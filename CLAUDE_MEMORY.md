...
[LOG — 2026-05-09]
- Action: Smoke test repair — Master `log_receipt` confirmation hit "Max iterations reached" with only Get Jobs recorded. Root cause: system prompt + tool description instructed Claude to forward `image_data` + `image_mime` from the user's message into the `log_receipt` tool_use input. Vision content blocks aren't accessible to the model as copyable text — the model attempted to emit a 200KB+ base64 string anyway, `max_tokens=2048` truncated the response, `stop_reason: "max_tokens"` fell to the agent loop's `else { break; }`, returned "Max iterations reached" before the confirm card could surface. Other confirm verbs (log_payment, submit_change_order) take no images, so they were unaffected.
- Fix: server-side image stash in the CONFIRM_TOOLS breakout. Removed the impossible "ALWAYS pass image_data + image_mime through" line from the system prompt RECEIPT FROM PHOTO section; rewrote the `log_receipt` tool description to say "Do NOT include image_data or image_mime — the system attaches the receipt photo automatically." New helper `extractLatestUserImage(currentMessages)` scans backward for the most recent user message with a `{type:"image", source:{type:"base64", data, media_type}}` block; in the breakout, if `confirmBlock.name === "log_receipt"` and no `image_data` is set, inject the extracted bytes into `pending_action.input` as `image_data`/`image_mime`. The confirmed-path executor (line ~917) consumes those keys unchanged, uploads to `job-receipts`, and stamps `receipt_url`. Only the log_receipt path was touched.
- Files: supabase/functions/ai-master-agent/index.ts (TOOLS log_receipt description, system prompt RECEIPT FROM PHOTO bullet, new extractLatestUserImage helper, CONFIRM_TOOLS breakout image injection).
- Decision: Round-trip the base64 image through the client via `pending_action.input` rather than introducing a server-side stash table or pre-uploading on breakout. The client already has the image in memory (it sent it up); ~200KB JSON round-trip is acceptable for v1. Pre-upload-on-breakout would orphan files when users cancel the confirm card; a stash table adds infra for one tool. Revisit only if payload size becomes a real problem.
- Architectural note: this is the first time we hit the wall where base64 payloads can't fit through model `tool_use` parameters. Vision content blocks reach the model for *reasoning* (the model can describe what it sees), but the bytes themselves aren't text the model can quote into structured output. Any future confirm verb that needs to bind a user-uploaded artifact to its action will need the same server-side stash pattern. Captured to backlog as "tool params can't carry payloads — generalize the receipt-photo stash if a second verb needs it."
- Open: Build verified (Vite passes); end-to-end smoke not yet exercised. Next session should paperclip a receipt → confirm → verify `job_transactions.receipt_url` populated and TransactionModal renders the photo. Quick smoke on log_payment + submit_change_order to confirm those confirm-flows weren't disturbed.

[LOG — 2026-05-09]
- Action: Master agent UX cleanup pass — two fixes in one commit. (1) Strip the redundant typed-confirm step from the receipt flow; the popup card IS the confirmation. (2) Always render dollar amounts with 2 decimal places ("$542.50" not "$542.5", "$1,000.00" not "$1000") across both agents and the frontend.
- Fix 1 (master prompt only): Tightened the HOW TO BEHAVE money-tools rule to forbid text-confirmation hedging ("do NOT ask the user to confirm via text first — the card IS the confirmation"). Added an explicit RECEIPT FROM PHOTO bullet: "Call log_receipt directly with the extracted fields once you've matched the job. The pending_action confirmation card surfaces automatically." Field agent's voice-only "Say yes to confirm" pattern intentionally untouched — voice has no card.
- Fix 2 (both agents + frontend): New `fmtMoney(n)` helper in both edge fns using `toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })`. Replaced all `Number(input.amount).toLocaleString()` call sites — `describeConfirmAction` / `describeAction` (the strings the confirm card displays) plus all field agent inline reply templates (log_payment reply, log_receipt action_label, submit_change_order body/reply/action_label). Updated `f$` in `avenstone-vite/src/lib/utils.jsx` to the same 2-decimal config. New rule added to both system prompts: "Currency formatting: ALWAYS write dollar amounts with two decimal places."
- Files: supabase/functions/ai-master-agent/index.ts, supabase/functions/ai-field-agent/index.ts, avenstone-vite/src/lib/utils.jsx, CLAUDE_MEMORY.md.
- Decision: Fix `f$` at the source rather than auditing every callsite — `f$` is the canonical helper, used app-wide; downstream callers either already want 2 decimals or were silently emitting "$542.5" too. Same approach in edge fns: one helper, all formatter sites updated. The system prompt rule is a belt-and-suspenders — even if the model bypasses fmtMoney by writing amounts inline in text, it should still emit two decimals.
- Voice TTS caveat: field agent replies now read "$500.00" which TTS may render "five hundred dollars and zero zero" — not ideal. Acceptable for v1 since the spec was explicit; revisit if voice replies sound awkward in the field.
- Open: Build verified (Vite passes). End-to-end smoke owed — paperclip receipt → popup card should appear immediately (no "Confirm?" text turn first), card should show 2-decimal amount. Quick check on log_payment + submit_change_order — amounts should also show 2 decimals; flows should still work.

[LOG — 2026-05-09]
- Action: Master Agent v1 build shipped — 15 commits. Ring buffer error tracking, reusable UI components, two DB migrations, full MasterAgent UI rewire with 5 verb flows, submit-bug-report edge function, two admin dashboards, comprehensive doc updates.
- Files (new): avenstone-vite/src/lib/bugContext.js, src/components/shared/ChipPicker.jsx, src/components/shared/JobChipPicker.jsx, src/lib/pendingTasks.js, src/components/shared/PendingTaskList.jsx, src/components/admin/BugReportDetailModal.jsx, src/components/admin/BugReportsScr.jsx, src/components/admin/PendingTaskOwnerScr.jsx, supabase/functions/submit-bug-report/index.ts, supabase/migrations/20260509004927_pending_tasks.sql, supabase/migrations/20260509005048_platform_owner_flag.sql, supabase/migrations/20260509005049_bug_reports.sql, supabase/migrations/20260509005100_tenants_notification_rules.sql.
- Files (modified): avenstone-vite/src/components/shared/MasterAgent.jsx (full rewire, 5 verb flows), avenstone-vite/src/App.jsx (initBugContext, pushBreadcrumb, profile select, 2 admin nav entries), avenstone-vite/src/lib/supabase.js (SUBMIT_BUG_REPORT_URL), avenstone-vite/package.json (html2canvas dep), CLAUDE.md (primacy note, AI component map, Master Agent v1 + Bug pipeline sections, schema reality, locked decisions), AVENSTONE_VISION.md (v2 rep scorecard arc, v2 bug pipeline auto-fix arc), EXECUTION_ARC.md (replaced with redirect), CLAUDE_ARCHIVE.md (execution-arc-2026-05 slug), OPUS_PROMPT_RULES.md (doc count discipline section).
- Decisions locked: (1) no auto-archive on pending_tasks — visible queue is discipline tool. (2) html2canvas screenshot at tile-tap, not resume time. (3) is_platform_owner flag on profiles (not hardcoded UUID). (4) paste-ready Claude prompt format in bug email is locked — 6-task structure, do not simplify.
- DB migrations verified: pending_tasks (17 cols ✓, 2 policies ✓, 4 indexes ✓), is_platform_owner (col ✓, kalin@avenstonekc.com row ✓), bug_reports (17 cols ✓, 4 policies ✓, 3 indexes ✓, bug-screenshots bucket ✓), notification_rules (col ✓).
- Open: (1) submit-bug-report needs env vars set in Supabase function secrets: RESEND_API_KEY, BUG_NOTIFY_EMAILS. (2) End-to-end smoke on all 5 verb flows (especially BugFlow → edge fn → email). (3) Notification rule cron alerting deferred to v2 — pending_tasks threshold alerts not wired. (4) Blake's email not yet set in is_platform_owner (email unknown). (5) Build passes locally; Vercel + Codemagic deploys auto-triggered on push.

[LOG — 2026-05-09]
- Action: Master Agent v1.1 — fixed two bugs surfaced by Test 2 (todo verb). Symptom: tile "Add to the todo list" → label "Call back Sarah Johnson Tuesday about countertop" → Save for later → Resume → walked the chips → agent said "note added," row landed in `job_notes`, not `todos`.
- Branch: B-1 confirmed by direct DB query — todos table empty in 2-hour window, single row in `job_notes` with content matching the label and a fabricated `job_id` of `test-flow-001`. ai-master-agent has 17 tools but no `add_todo` — only `add_note`. The model chose the closest match for any todo-shaped intent.
- Bug 2 surfaced during diagnosis: PendingTaskList `onResume` (MasterAgent.jsx ~1280) set `verb` and `pendingTaskId` but NOT `flowActive=true`. Render conditional `verb && flowActive && verb === 'todo'` failed; fell through to `verb && !flowActive && <QuickCapture>`. So Resume re-rendered the quick-capture step and Continue Now called `sbCreatePendingTask` a second time, creating a duplicate. This is why CHECK 3 (pending_tasks for verb='todo' in 30 min) returned [] — the original Save for later may have worked, but Resume was broken so the user diverted to chat.
- Fix 1 (ai-master-agent/index.ts): added `add_todo` tool schema + executor mirroring `sbCreateUserTodo` (writes to `todos` with type=user_task, source=manual). System prompt updated: WHAT YOU CAN DO list now includes "add todos (action items)"; HOW TO BEHAVE has a new disambiguation rule — todo (action item: "remind me", "follow up", "call back") routes to add_todo; note (passive context: "FYI", "for the record") routes to add_note; explicit "if user said todo, pick add_todo, never add_note."
- Fix 2 (MasterAgent.jsx): added `setFlowActive(true)` to PendingTaskList `onResume` callback. Resume now goes straight into the verb chip flow without re-creating the pending_task row.
- Cleanup: misrouted job_notes test row (id `06b66975-c72c-4d42-a2f4-c03963c84ed0`) deleted via Management API. Verified todos and job_notes both clean post-fix.
- Audited the other 4 verbs at the agent layer: receipt → log_receipt ✓, lead → create_job ✓ (status='lead'), CO → submit_change_order ✓, bug → no agent tool (intentional — BugFlow chip flow only, fires submit-bug-report edge fn directly). Only `todo` was missing.
- Files: supabase/functions/ai-master-agent/index.ts, avenstone-vite/src/components/shared/MasterAgent.jsx, CLAUDE_MEMORY.md (this entry + 2 symptom-index additions).
- Decision: did not add a "View in todos" link to the success toast (Branch A step 2). Toast already says "Todo added ✓" — wrong-wording was the agent path's bug, not the chip flow's. Adding the link would require threading `setPg` through MasterAgent; defer until users actually ask for it.
- Open: (1) end-to-end re-smoke owed — type "add a todo: test" freeform into the agent; row should land in `todos` with source='manual', not job_notes. (2) Re-test Save for later → Resume → chip walk on todo verb; only ONE pending_task row should be created.

[LOG — 2026-05-09]
- Action: Master Agent v1.1 — Test 3 lead-create fix. Symptom: `sprakle fish 9135552121 5010 e pony creek rd cleveland mo 64734 referral` lead failed Confirm with `duplicate key value violates unique constraint "jobs_po_number_tenant_unique"`. Routed to Branch C (auto-gen leaks into leads).
- Root cause: DB trigger `set_job_po_number` had two compounding bugs. (1) Issued po_number for ALL inserts including leads/proposals — but PO numbers belong to contracts and beyond, not pre-contract jobs. (2) Used `COUNT(*) + 1` for sequence numbering; deleting any job decremented the count, so the next insert collided with an existing po_number (existing job `test-flow-001` already had `26-002`, count was 1, new insert tried `26-002` again).
- Fix: migration `20260509130000_po_number_lifecycle_aware.sql`. (a) Status-aware trigger — `IF NEW.status IN ('lead', 'proposal') THEN po_number := NULL; RETURN NEW;`. (b) MAX-based numbering — `COALESCE(MAX(SUBSTRING(po_number FROM 4)::INT), 0) + 1` filtered to current year regex. Tolerant of deletes; never re-issues a stale number. (c) Replaced unique index with partial: `WHERE po_number IS NOT NULL` — explicit NULL-safe intent, survives any future `NULLS NOT DISTINCT` change.
- Verification: `pg_get_functiondef(set_job_po_number)` shows new body live; `pg_indexes.indexdef` shows `WHERE (po_number IS NOT NULL)` predicate. End-to-end test: inserted two leads back-to-back (`verify-lead-001`, `verify-lead-002`) — both succeeded with po_number=NULL. Inserted one contract (`verify-contract-001`) — got `26-003` correctly (existing max was 002). All three cleaned up post-test.
- Audit of analogous bugs: checked `create_job` in ai-master-agent (does not pass po_number) ✓; `sbSave` in supabase.js (does not pass po_number) ✓; `LeadFlow.handleConfirm` in MasterAgent.jsx (does not pass po_number) ✓. None of the agent/UI paths set po_number — this was purely a DB-layer bug. Side note: LeadFlow uses wrong column names (`customer_name`, `customer_phone`, `created`, `created_by`) which `sbSave` silently drops — separate cleanup, deferred.
...
[LOG — 2026-05-09]
- Action: Master Agent v1.1 shipped — smart label parsing on Resume + single-Confirm card with per-field Edit + Enter-key submit on agent text inputs.
- Files: `avenstone-vite/src/lib/labelParser.js` (new, 302 lines), `avenstone-vite/src/components/shared/MasterAgent.jsx` (4 flow components rewritten + render gating), `avenstone-vite/src/components/shared/JobChipPicker.jsx` (Enter on search → first result).
- Commits: `3a49122` (smart parsing + single-Confirm), `d864303` (Enter-key on JobChipPicker).
- Root cause of Resume redundancy: each flow component took an `initLabel` prop but never read it — Resume rebuilt the chip walk from scratch. Symptom: Smith Bath CO label `"Smith Bath extra electrical for vanity outlets $750"` didn't pre-fill anything; user re-typed every field.
- Approach (regex + keyword, no LLM): pure JS so the cost is zero and Resume is sync. Each parser returns `null` for unfindable fields (never partial-fills wrong). Field names match the JS write helpers exactly so seed state is a one-shot assign with no translation. CO confirm card reads back digits + spelled-out form (`$750 — seven hundred fifty dollars`) per VOICE_AGENT.md money-safety pattern.
- Project hint: `matchProjectHint` substring-matches against active jobs (status IN lead/proposal/contract/in_progress/final_touches) — 1 match auto-prefills, 0/many leaves JobChipPicker on screen. Highlighting matches at the top is deferred to v1.2.
- Todo verb rejoined the queueable QuickCapture pattern (`TodoQuickAdd` direct-write removed). Smart parser eliminates the friction that originally drove the kill — title now pre-fills from the label, due_hint from keyword scan, project_hint from address-y capitalized runs.
- BugFlow intentionally untouched per scope rule.
- Where the parser will miss (signal for v1.2): non-North-American phone formats, non-USD currency, multi-line labels, addresses without a street type, vendors that aren't capitalized in the source. None of these are blockers — Confirm card still asks for what's missing.
- Open: visual smoke test on the v1.1 verbs deferred — Vite build green, but no manual UI exercise this session. Test plan in resume point.

[LOG — 2026-05-09 — Master Agent v2 simplification]
- Action: Master Agent v2 shipped — chat-first architecture, queue retired, CONFIRM_TOOLS extended from 3 to 5 verbs.
- Files added: `supabase/migrations/20260509180000_drop_pending_tasks.sql`.
- Files deleted: `avenstone-vite/src/components/admin/PendingTaskOwnerScr.jsx`, `avenstone-vite/src/components/shared/PendingTaskList.jsx`, `avenstone-vite/src/components/shared/ChipPicker.jsx`, `avenstone-vite/src/components/shared/JobChipPicker.jsx`, `avenstone-vite/src/lib/pendingTasks.js`, `avenstone-vite/src/lib/labelParser.js`. Six files, ~1900 LOC.
- Files rewritten: `avenstone-vite/src/components/shared/MasterAgent.jsx` (1258 → 968 LOC; 1116 deletions, 142 insertions).
- Files modified: `avenstone-vite/src/App.jsx` (3 sites — import, NAV entry, render gate), `supabase/functions/submit-bug-report/index.ts` (dead `pending_task_id` branch removed), `supabase/functions/ai-master-agent/index.ts` (CONFIRM_TOOLS +2 verbs, amountToWords ported in, describeConfirmAction +2 branches +money read-back, system prompt extends confirm-card guidance to all 5 verbs, add_todo description loses the stale "auto-applies" line), `CLAUDE.md` (Master Agent v2 section rewrite + AI Component Map prune + Locked decisions rewrite + Schema reality block updated), `CLAUDE_MEMORY.md` (this entry + Schema reality additions + Symptom index entries).
- Commits: `12303ef` (drop migration), `ee5e3c0` (queue file removals + cleanup), `22d1185` (MasterAgent rewrite), `b1b2558` (CONFIRM_TOOLS extension), and the docs commit landing now.
- Migration applied: `DROP TABLE IF EXISTS pending_tasks CASCADE;` ran via Supabase Management API (PAT). Pre-drop row count was 8 — all smoke-test artifacts (`fuck it`, `sprakle fish`, two `call back mike davis` dupes, etc.). Post-drop verification: 3 information_schema / pg_policies / pg_indexes queries all returned zero rows. Resulting entities (job_transactions, change_orders, jobs, todos) live in their own tables and were unaffected.
- Build verification: `npm run build` from avenstone-vite/ exit 0 after Commits 3+4. 366 modules, 597ms. Same chunk-size warnings as before (heic2any + main bundle); not new.
- Decision: chat is now the input surface for Master Agent. Tiles are starter prompts via `TILE_PREFIXES`, not state-machine triggers. The Confirm card (`pending_action`) is the only commit point for write verbs. The agent infers verb + fields from freeform text. The trade-off: every captured intent now spends one Sonnet call. Acceptable because the alternative was 5 chip-flow components plus a queue table plus a 302-line parser plus a smart-Resume contract.
- Decision: bug submission stays inline (does not route through ai-master-agent). html2canvas + getSnapshot fire at tile-tap (the screen changes during chat); the description posts to submit-bug-report directly. Bug context is freeform — an LLM round-trip would not help.
...
[LOG — 2026-05-11 — assign_sub removed from ai-master-agent (job_subs dead-table fix)]
- Action: Removed `assign_sub` tool from `ai-master-agent/index.ts` — tool registration (`:295-306`), executor case (`:668-676`), and "assign subs" from the WHAT YOU CAN DO list in the system prompt (`:874`). Also fixed the secondary read in `get_job_details` (`:382`) that selected from the dropped `job_subs` table — now reads `job_sub_engagements` with profile join and filters out terminal statuses (`completed,declined,withdrawn,removed`).
- Files: `supabase/functions/ai-master-agent/index.ts`, `CLAUDE_MEMORY.md`.
- Decision: Option 2 (remove) over Option 1 (rewrite to canonical engagement model) or Option 3 (stub). Reason: chat-first capture works for atomic verbs; sub assignment is a 4–6 field decision (trade, bid_type, scope, budget, due_date, shared docs) better made in `AddSubToJobModal` where the rep sees the directory and budget side-by-side. Fix should match the scope of the finding (dead-table reference), not opportunistically rebuild a feature. If voice/chat sub assignment becomes a real need, it gets its own arc (probably under `VOICE_AGENT_ARC.md` or `SUB_WORKFLOW_ARC.md`) with proper picker enrichment + trade matching.
...
[LOG — 2026-05-12 — staff_messages migration applied (Branch C — not dead code, never-applied migration)]
- Action: Applied `supabase/migrations/20260415_staff_messages.sql` to live DB via Management API. Migration had been committed since April 15 but never applied. Two helpers (`sbLoadStaffMessages`, `sbPostStaffMessage` at `supabase.js:587-598`) had four live callers across `MessagesTab.jsx` (PM side) and `SubJobView.jsx` (sub side) — every send attempt for a month had been silently routing through `captureFailedIntent`. Drift detector caught it via the write-side helper, not the missing UI behavior.
- Files: `supabase/migrations/20260415_staff_messages.sql` (edited + applied), `CLAUDE_MEMORY.md`.
