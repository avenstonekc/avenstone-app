...
**SCOPE_TO_ESTIMATE agent-gate divergence** (logged 2026-07-11) — the selections lock `checkSelectionsConfirmed` (contract→in_progress gate) is enforced in `phaseGates.js` (the UI path, via PhaseAdvanceCard) ONLY. The `advance_phase` gate copies inlined in `ai-master-agent/index.ts` and `ai-field-agent/index.ts` (Deno can't import frontend code) do NOT mirror it — so an agent-driven `advance_phase` will move a job from contract→in_progress past the "N of M locked" selections lock the UI would block. Real enforcement hole, not cosmetic. Scoped out of C2 deliberately (C2 touched phaseGates.js only). Fix: mirror `checkSelectionsConfirmed` into both edge-fn gate copies when next touching agent `advance_phase` — rides with SCOPE_TO_ESTIMATE Phase D or a standalone ~1-prompt slice. Already documented in the `phaseGates.js` header divergence block (lines 1–11).
