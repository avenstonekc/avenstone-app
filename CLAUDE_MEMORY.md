...
- NEXT: P3 — `price_plan` edge mode: reads job_scope_answers → scopeTranslation → computePricingLines → returns priced_scope. No LLM call. Needs KALIN_QUEUE items j+k resolved first (rate authority). `vanity_swap` scope subset does not yet include Countertops trade — data gap to address at P3 or alongside.
- NEXT: P2 (translation layer — job_scope_answers → scope_details) needs KALIN_QUEUE j+k resolved first.
