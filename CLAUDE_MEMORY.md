...
- Visibility (one line): internal Files tab shows it (sbLoadJobFiles filters job_id/tenant/lifecycle only — no bucket filter); client portal shows it (ClientPortal.jsx queries job-documents + client_visible=true + active) — fine, same PDF was emailed. Nothing else from consultation leaks this surface: transcript is DB-only (never a file), consultation photos stay session-scoped (no job_files rows) — the recap is the only consultation job_files write.
- Verify: build green. The Documents tab (FilesTab/FilesTreeView) is an existing responsive surface already rendering 'Documents'-category rows across all three viewports; this adds a data row to it (no new UI), so viewport behavior is inherited. Not walked end-to-end because composing+sending a real recap emails a live client.
