...
- **CONSULTATION_MODE on-device TestFlight checklist (pending next build):** (1) hot-word mode switch ("avenstone, measure / ambient / what am I missing / end session") off the live partialResults stream; (2) spoken needs-list on demand + spoken end-gate before leaving; (3) 60-min+ continuous ambient session survives the ~60s SFSpeechRecognizer task ceiling via the auto-restart loop (no silent drop); (4) photo-during-talk (in-flow camera while ambient keeps running); (5) **pocket/screen-lock behavior — the load-bearing unknown:** does capture survive backgrounding with `UIBackgroundModes: audio`? If it drops when locked, fall back to keep-awake + dimmed pocket UI (dim UI already shipped). No audio persisted — verify only text lands.
