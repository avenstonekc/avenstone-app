...
- **`job_rooms` table EXISTS** (SCOPE_TO_ESTIMATE Phase A, 2026-07-11, live-verified). Migration 20260711120000. Columns: id UUID PK DEFAULT gen_random_uuid(), tenant_id UUID NOT NULL, job_id TEXT NOT NULL FK→jobs(id) ON DELETE CASCADE, label TEXT NOT NULL, source TEXT NOT NULL CHECK (typed/scan), scan_room_id TEXT (soft-bridge to job_room_scopes "${scan.id}_${idx}", no FK), created_at TIMESTAMPTZ. Index idx_job_rooms_tenant_job (tenant_id, job_id). RLS: staff-only FOR ALL at birth (job_notes pattern), role-scoped to owner/project_manager/sales_rep + a client SELECT policy added in C1 (migration 20260711130000). Interview default = one room per project_type interview (label = project type). jobs.id is TEXT so the FK type matches (not a value-format violation).
- **`job_scope_answers` table EXISTS** (SCOPE_TO_ESTIMATE Phase A, 2026-07-11, live-verified). Migration 20260711120100. The single answer store — interview persistence (SCE P2), photo intake (SCE P3), SELECTIONS (Phase C), SUB_WORK_PACKET (Phase D) all read it. Columns: id UUID PK, tenant_id UUID NOT NULL, job_id TEXT NOT NULL FK→jobs ON DELETE CASCADE, room_id UUID FK→job_rooms ON DELETE CASCADE (nullable), field_key TEXT NOT NULL, option_key TEXT, value TEXT, trade TEXT, source TEXT NOT NULL CHECK (rep_typed/rep_card/measured/extracted/client_selected), status TEXT NOT NULL DEFAULT 'proposed' CHECK (proposed/confirmed) — **orthogonal to source (rate-book lesson)**, confirmed_by UUID FK→profiles ON DELETE SET NULL, confirmed_at TIMESTAMPTZ, created_at, updated_at. UNIQUE NULLS NOT DISTINCT (tenant_id, job_id, room_id, field_key) — the re-pick/upsert key (no version history by design). Index idx_job_scope_answers_tenant_job. RLS: staff FOR ALL role-scoped (C1); client vet-gate — INSERT forced source='client_selected'+status='proposed', UPDATE only own proposed client rows AND (C2) cannot set confirmed_by/confirmed_at, SELECT confirmed OR own proposed picks, NO client DELETE. Records WHAT was chosen (option_key); price/qty live on estimates + change orders (boundary decision, not a second table).
- **`jobs.selections_opened_at` column EXISTS** (SCOPE_TO_ESTIMATE Phase C1, 2026-07-11, live-verified). Migration 20260711130000. TIMESTAMPTZ nullable. When non-null, the client portal appends the Selections tab (getClientTabs). Stamped once on signing by record-signature-evidence (service role, .is(null) idempotent) OR lazily by SECURITY DEFINER fn `ensure_selections_open(p_job_id)` (migration 20260711130100) on client portal load for jobs advanced to in_progress without a signature hook. Client has NO jobs UPDATE grant — the definer fn is the only client-reachable stamp path.
- **`scope_checklists.is_selection` column EXISTS** (SCOPE_TO_ESTIMATE Phase C2, 2026-07-11, live-verified). Migration 20260711140000. BOOLEAN NOT NULL DEFAULT false. The client Selections tab + Demo gate filter on `audience='rep_client' AND is_selection=true`. Seeded true (platform-null + tenant rows, matched on project_type+field_key) for the 5 day-one trades' pure finish/material/style/pattern choice fields (bathroom wall_tile_layout/shower_glass/floor_tile/vanity_style/countertop/fixture_finish/toilet, kitchen cabinet_style/countertop/counter_edge/sink/backsplash_layout, basement flooring). Scope-config/extent/info fields intentionally stay false.
...
**PHASE D ORPHANS — PARTIALLY RESOLVED 2026-07-12.** ~~Countertops~~ **RESOLVED** (Kalin's field call): countertops go to a dedicated countertop shop — a distinct vendor, NOT cabinets or tile. Added `Countertops` as its own parent-only platform trade + scope_option_trades rows for `countertop` (bathroom+kitchen) and `counter_edge` (kitchen), every option → 'Countertops' (migration `20260712120000`, commit `ed6c101`; live-verified via deriveTrade). **STILL OPEN — `basement.concrete_polished`:** no trade yet; needs its own answer (Concrete? a polished-concrete-floor specialist? a Flooring-Finish sub?). Remains an Unassigned orphan (trade NULL → packet "Unassigned — confirm trade" section) until Kalin decides. Do NOT guess it. **PARKED — countertop/counter_edge option IMAGES not generated:** the scope-option-image library has no bound cards for these options; generate via the Gemini API path (same as the 195-image library, ~$8 class), NOT by sourcing a vendor catalog (SKU-level vendor catalogs belong to the future SELECTIONS catalog arc, not base option imagery).

~~**SCOPE_TO_ESTIMATE agent-gate divergence**~~ **RESOLVED 2026-07-11 (Phase D, commit 3659a4b).** `checkSelectionsConfirmed` (contract→in_progress selections lock) is now mirrored into BOTH `ai-master-agent` and `ai-field-agent` `runGatesForTransition` — byte-equivalent to the UI gate, routed through the existing override-card flow. Edge deploy green; pass-path live-verified (7/7 → passes), fail-path arithmetic verified (unconfirmed>0 → fails). phaseGates.js header updated to RESOLVED. Standing rule remains: any change to the phaseGates GATE map must be mirrored into both edge copies (three copies in sync).
...
- **CONSULTATION_MODE on-device TestFlight checklist (pending next build):** (1) hot-word mode switch ("avenstone, measure / ambient / what am I missing / end session") off the live partialResults stream; (2) spoken needs-list on demand + spoken end-gate before leaving; (3) 60-min+ continuous ambient session survives the ~60s SFSpeechRecognizer task ceiling via the auto-restart loop (no silent drop); (4) photo-during-talk (in-flow camera while ambient keeps running); (5) **pocket/screen-lock behavior — the load-bearing unknown:** does capture survive backgrounding with `UIBackgroundModes: audio`? If it drops when locked, fall back to keep-awake + dimmed pocket UI (dim UI already shipped). No audio persisted — verify only text lands. **WALKTHROUGH_TYPES adds:** (6) **sub-walk type picker in the field** — start a session, choose Sub Walkthrough, pick a trade + sub from the job's engaged subs, confirm the session tags correctly; (7) **sub-walk coach filter** — during a sub walk the live needs-list shows only the walked trade(s)' fields (e.g. tile → waterproofing/substrate), not the full bathroom checklist. **POCKET_FALLBACK adds** (item 5 above now field-CONFIRMED to drop on lock; plan B shipped): (8) **keep-awake + gap recovery** — start a session, tap 🌙, pocket the phone: screen stays on dimmed and capture survives (web wake lock; if it does NOT hold in the WKWebView, escalate to the native @capacitor-community/keep-awake plugin); then manually LOCK → unlock shows the "capture paused — resume" banner and a [CAPTURE GAP …] marker is in the transcript; (9) after capture survives in-pocket, confirm a **steam-shower** mention fires the escalation follow-ups (vapor barrier / ventilation) — likely lost to the pocket gap in Kalin's field session.
...
- ~~`phase_pct_complete` rollup audit (#8 from Master diagnostic)~~ **CLOSED 2026-07-16** (`ea11701` + `18e6f30`): % re-sourced from jobs.status + trade-phase band (no longer complete-only over stale job_phases); Next Milestone now overdue-aware. See LOG below.
...
- "UI blocks a phase advance but the agent lets the same advance through" → phase-gate logic lives in `phaseGates.js` (UI path) but the `advance_phase` gate copies inlined in `ai-master-agent` + `ai-field-agent` (Deno can't import frontend code) were not kept in sync — a gate present in one path is absent in the other. Canonical instance: `checkSelectionsConfirmed` (contract→in_progress selections lock) shipped UI-only in SCOPE_TO_ESTIMATE Phase C2 (2026-07-11); agent edge copies unpatched. Audit pattern: any change to `phaseGates.js` GATE map must be mirrored into both edge-fn copies (the header of phaseGates.js lists them). See Active open items · "SCOPE_TO_ESTIMATE agent-gate divergence".
...
[LOG — 2026-06-30] — SCOPE_CAPTURE_ENGINE P2 "thin channel" shipped (structured session pre-answers)

- AUDIT FINDING (drove scope): P1A already delivers most of the blueprint's P2 line — session capture pre-fills the estimate AND feeds the interview via prose (prefill scope/rooms/special → first user message → interview extracts). The genuine gap was a STRUCTURED pre-answered-fields channel (vs prose) + measured SF. Kalin chose "thin P2 channel only" (build foundation, defer vision). Full "80%-pre-answered interview" awaits Phase 3 vision, which is what actually FILLS the channel with checklist answers.
- Built the end-to-end structured channel: sessionToEstimatePrefill.js emits measuredFields (flattened consultation_measurements.fields) + sf (max floor area, never summed — avoids multi-trade double-count). EstimateTab forwards measuredFields as prefilled_answers on scope_interview POSTs + prefers session SF over scan (deriveProjectSf), 'measured on-site' hint. ai-estimator.handleScopeInterview folds prefilled_answers keys matching a real checklist field into answeredKeys (source 'measured') BEFORE the gate; short-circuits complete with NO AI call if session covers everything (cost win); live conversation overrides session on shared key; system prompt lists ALREADY-CAPTURED so AI doesn't re-ask. makeAnswerRecord gained optional source param (Phase-3 hook).
...
[LOG — 2026-07-09/10] — CONTRACT_SIGNING 1b/1c + magic-link→recovery migration + email defect fixes + RECEIPT_MODAL_EXTRACTION (batch)

- CONTRACT_SIGNING 1b (dc188fc buildContractPDF+sbGetContractSnapshot; be66116 rewire sign/send modals + sign-time evidence freeze): ONE unified buildContractPDF renders the accept-time contract_snapshot (line items, total, scope) instead of DEFAULT_CONTRACT_TEXT boilerplate. Sign/send modals rewired to the unified path. Sign-time evidence is FROZEN at signature; a **fail-loud no-snapshot gate** blocks signing when no snapshot exists (no silent boilerplate fallback). This is the technical half of CONTRACT_SIGNING Gap 1 / B4.2 — marked TECH-FIXED in the plan; **ESIGN/UETA attorney review (Open Q1) still gates go-live** (STOP stands).
- CONTRACT_SIGNING 1c (a3be13c): payment schedule frozen into contract_snapshot AT ACCEPT (not re-derived at sign). Clause-3 fallback text when a job has no payment schedule. LOCKED DECISION: **no auto-default payment schedule** — the system never invents a schedule; a proactive nudge is optional future work (slice "1d", not built).
- Magic-link → recovery-link migration (be303ea migrate send-contract-email; f51b500 remove dead helpers): send-contract-email moved off the retired magic-link path onto the canonical create-client-login recovery-link pattern (same as send_client_portal, 2026-07-02). Existing client passwords are NEVER reset by the send. A sub mis-provisioning bug (send path could clobber/misprovision) was killed in the migration. Dead send-client-link helpers removed (zero callers, grep-proven). Resolves CONTRACT_SIGNING Gap 5 / B4.1 (was BLOCKING → RESOLVED).
- Email defect fixes (2b70f61, from live email testing): (1) ADDRESS-SLOT BUG — SubComplianceModal passed a label string where job_address was expected, so emails showed the label instead of the address; fixed to pass the real address. (2) Sub-copy honesty — corrected misleading sub-facing email copy. (3) Expired-link banner — surfaced when a recovery/set-password link has expired instead of a dead-end.
- RECEIPT_MODAL_EXTRACTION (d167bbd generalize ai-extract-sub-invoice + sbExtractReceipt; d35c3dc TransactionModal auto-fill): ai-extract-sub-invoice (Haiku claude-haiku-4-5-20251001) generalized to accept {bucket,path} alongside {jobFileId} — tenant-isolated via the owning job (path=`${jobId}/…`) + bucket allowlist (job-receipts); prompt broadened to "invoice OR vendor/materials receipt", output contract unchanged. sbExtractReceipt(bucket,path) helper added. TransactionModal calls it after upload and auto-fills vendor→payer_or_payee_name / amount / invoice_date→date_incurred / description — **only fields the user hasn't touched (mount-snapshot compare; user-typed values always win)**; any extraction failure degrades SILENTLY to manual entry and never gates save. Audit-first corrected the dispatch premise: the agent's receipt-from-photo is inline SONNET (not a callable Haiku path — do NOT copy, Bug C); storage was already unified on job-receipts + job_files dual-write. LIVE-VERIFIED 6/6 endpoint tests (real receipt extraction + legacy {jobFileId} regression + tenant-isolation + bucket allowlist + graceful failure). CLAUDE.md bucket list corrected (+job-receipts, the missing 5th bucket).
- DECISIONS LOCKED this batch: (1) missing rate → ASK the rep with the KC regional average shown as an ANCHOR; accept-or-override, BOTH write back to Rate Book with source (no silent regional_avg). Resolves Open Q2 → drives ESTIMATOR Phase 5. (2) Receipt/transaction status toggle stays UNSCOPED to all roles (dropped the parked "owner-scoped" idea). (3) No auto-default contract payment schedule (see 1c).
- PLAN INVENTORY CORRECTED: ESTIMATOR Phase 4 (Guided interview w/ pre-filled defaults) was STALE NOT-BUILT — actually shipped via B1.6 (ab50638 killed hardcoded 30%/$1,200, reads bid_model_config) + B2.1 (0bead89 preamble, bd3fb84 fail-loud). Now marked BUILT (net 5 of 7 estimator phases live). NEXT estimator slice QUEUED: estimator consumes supply_model + allowance from bid_model_config (B1.1 shipped the schema; consumption not yet wired = TENANT_ONBOARDING Phase 3).
- All 8 commits verified against git log before writing. This LOG + MASTER_BUILD_PLAN.md status sync committed together.

[LOG — 2026-07-10] — CONTRACT_SIGNING arc close-out + IP/UA evidence + signed-copy + Model B Phase 1 (batches yesterday's entry did NOT cover)

- CONTRACT_SIGNING ARC SHIPPED + closed out. Attorney reviewed ESIGN/UETA and CLEARED 2026-07-10 (Open Q1 closed, STOP lifted). CONTRACT_SIGNING_ARC.md rewritten as a lean shipped slug (was a design blueprint); MASTER_BUILD_PLAN B4.2/B4.3 + Gap table + net summary marked SHIPPED. Only elective left: Gap 2 (portal proposal view); Gap 6 (status lifecycle) folded into Model B.
- Gap 4 IP CAPTURE SHIPPED (2f8d932 migration 20260710120000 adds ip_address + user_agent; 590fea2 record-signature-evidence edge fn + sbRecordSignatureEvidence reads them server-side from request headers; 3e856f5 ClientSignContractModal post-save call). LOCKED: capture is ENRICHMENT not state change — fires after the signature is saved, a failure logs and degrades, never undoes/blocks the recorded signature.
- Signed-copy delivery (d636b43 + 4170c72): after a signature saves, client is emailed their executed PDF via a `signed_copy` branch in send-contract-email — confirmation not request (no provisioning, no recovery link, no "Action Required"). Elective: extend signed_copy typeLabel for change_order/completion (currently labels all "Contract").
- AUDIT LIES CORRECTED (do not repeat): (1) `contract_signatures.ip_address` did NOT pre-exist — earlier docs/CLAUDE.md (~2026-06-15) claimed "column EXISTS but never populated"; FALSE, first added 2f8d932 on 2026-07-10. (2) Committed table defs 20260412_missing_tables.sql / 20260413_remaining_tables.sql are STALE — they say signer_name/signer_email/signature_png/pdf_url; LIVE TRUTH is signed_by_name/signed_by_email/signature_data/document_url (+ ip_address/user_agent). The live table was created by a different path than those migrations — trust information_schema, not the committed defs.
- Live contract_signatures (15 cols): id, tenant_id, job_id, type, reference_id, signed_by_name, signed_by_email, signature_data, signed_at, document_url, created_at, contract_total, scope_snapshot, ip_address, user_agent.
- MODEL B LIFECYCLE Phase 1 SHIPPED (5097e22 + 31e2218): src/lib/lifecycle.js deriveStatusFromPhases (pure, no supabase import) maps job_phases → jobs.status vocab (furthest non-not_started phase by phase_order; all-not_started→lead; on_hold passed in, not derivable; malformed→null+reason, never throws). scripts/test-lifecycle.mjs 25/25 (no vitest in repo — node runner). Owner-only LifecycleAuditScr (pg='lifecycle-audit', Setup nav) = read-only shadow instrument: address · jobs.status · derived · AGREE/DIVERGE. ZERO behavior change (no writes, no trigger, no reader changes). Live baseline 4 DIVERGE / 9 — matches the audit. LOCKED CALLS: trigger over view (jobs.status stays physical, trigger on job_phases recomputes it — Phase 3); manual status picker DEMOTES to owner-only phase-correction in Phase 4, does not die. Success meter: the audit screen reads 100% AGREE. Phases 2-4 pending; Phase 2 (advance Lead/Proposal/Contract phase rows on lifecycle events) is the riskiest — unifies the two decoupled advance engines. QUEUED (Captured Ideas): harden client UPDATE RLS on jobs to column-scoped (audit found the sign policy checks linkage only, no column constraints).
- All 11 CONTRACT_SIGNING commits verified against git log before writing (dc188fc, be66116, a3be13c, be303ea, f51b500, 2b70f61, 4170c72, d636b43, 2f8d932, 590fea2, 3e856f5).

[LOG — 2026-07-10] — ESTIMATOR Phase 5 (S2-S9) + client-RLS hardening + Model B Phase 2 + plan restructure (batch the arc-close-out entry did NOT cover)

- ESTIMATOR PHASE 5 SHIPPED as ASK-with-cited-anchor (NOT a config-mode column). Net now 6 of 7 (Phase 1 rate reconciliation is the only phase left). Commits: S2 supply_model+allowance consumption (41ac4ae) + client_supplied badge (60ea24e) — also closes TENANT_ONBOARDING Phase 3; S3 provenance column (1099554) + S4 rep-write RLS (b302138); S5A ground regional_rate in ai_knowledge KC prose (b64845c) + S6 gap lines carry anchor+anchor_source code-guarded (8544bc5); S7 gap-card accept/override affordances + per-gap tag (81da4eb) + S8 write real provenance to rate_book_labor (3dfb900) + S9 persist rate_provenance through commit, new nullable estimate_line_items.rate_provenance col, migration 20260710170000 (0ee2997). LIVE-VERIFIED as rep+test through real RLS: 3 rate_book rows w/ 3 sources all vetted=false; commit → 3 estimate_line_items carry rate_provenance; owner vet → vetted=true, source preserved.
- LOCKED (estimator): missing rate → ASK with CITED KC anchor (anchor number MUST come from ai_knowledge KC pricing + cite anchor_source; uncited → BLANK ask, never invented). Blank-asks are the HARVEST — prose is NOT patched to avoid them. Rep-write via RLS with DB-level vet gate (vetted=false forced both insert + RLS). Estimator IGNORES ai_knowledge.active (active gates conversational surfaces for other agents, NOT the pricing engine — pricing_* rows stay inactive-for-others while estimator reads them). supply_model constrained enum (contractor|owner); client-supplied wins over allowance. 'Allowance' reserved config-driven word.
- ORTHOGONALITY NOTE (premise correction — DO NOT collapse): rate_book_labor.source (PROVENANCE: seed|rep_entered|rep_accepted_anchor|rep_override) and vetted (owner-approval boolean) are ORTHOGONAL axes. Vetting flips vetted=true and PRESERVES source — it does NOT overwrite source with 'owner_vetted' (that would destroy S7-S9 provenance). The 'owner_vetted' source VALUE (S3 backfill) applies ONLY to pre-provenance historical rows. A dispatch tried to assume "vet flips source→owner_vetted"; corrected + verified live.
- DEFERRED (estimator ledger): Path B (structured KC averages as NULL-tenant rate_book rows — trigger: prose grounding proves too loose); material-gap rep-write (labor-only today); per-category markup (only 'default' consumed); waste-as-config (still prompt instruction); ai_knowledge active-flag semantics for conversational surfaces.
- CLIENT-RLS HARDENING SHIPPED (bca24b4 dropped the column-unscoped client UPDATE policy on jobs; 0137df6 removed the client-side jobs write from the sign modal; c808c08 moved the signed-state write server-side into record-signature-evidence, service-role). Client session no longer holds ANY jobs UPDATE. Resolves the §0 NEXT CODE DISPATCH security item (audit finding: sign policy checked linkage only, no column scope).
- MODEL B PHASE 2 SHIPPED (2c4695d code + 7f14362 backfill): markLifecyclePhases(sb,tenantId,jobId,event,actorId) in lifecycle.js (pure, forward-only, idempotent, .select()-verified) advances Lead/Proposal/Contract on created/proposal_sent/estimate_accepted/contract_signed. Wired: sbSeedJobPhases fires 'created'; JobsScr seeds at create (closed the lazy-seed-only gap); EstimateTab sendEstimateToClient (proposal_sent, real send path only) + handleAcceptEstimate (estimate_accepted); contract_signed routed server-side via record-signature-evidence (client RLS forbids job_phases writes — NOT widened); agent create_job + edge advance mirror src logic inline w/ divergence-guards. Backfill migration 20260710150000 (signals: signature/snapshot/status). ZERO change to jobs.status writes. Divergence held 4/9 but correctly characterized: 2 owner-approved signed→contract semantics, 2 genuine jobs.status lies (1206 W Lucy Webb, 999 Test Lane) → KALIN_QUEUE status-picker corrections. SEMANTICS (Phase 3, owner-approved): signed job w/ no demo derives 'contract' not 'in_progress'.
- PLAN RESTRUCTURE (7a3a3e6): new §0 Now/Next — KALIN_QUEUE (owner blockers as visible as Code's; maintenance rule: dispatches that create owner decisions add them, reports that resolve them remove them) + NEXT CODE DISPATCH + Model B Phase 3 HARD GATE for JOBDET_MOBILE + any lifecycle-rendering arc (entry criteria: divergence semantics-only + soak). New arc stub SCOPE_TO_ESTIMATE (BLUEPRINT PENDING — the seam: scope interview → persisted answers → adds_trades consumption → priced draft; inherited seams: in-memory answers, unconsumed adds_trades, priced-ready gap assumption). NOTE: no DESIGN_LANGUAGE §7 item existed — created it to carry the standing fetch-header-rules-doc line.
- All commits verified against git log before writing (41ac4ae, 60ea24e, 1099554, b302138, b64845c, 8544bc5, 81da4eb, 3dfb900, 0ee2997, c808c08, 0137df6, bca24b4, 2c4695d, 7f14362, 7a3a3e6).

[LOG — 2026-07-10] — SCE Phase 1B FULL SEED shipped (9 project types + 15 modules + conflict rules)

- Gate honored: STOPPED first dispatch because docs/arcs/SCOPE_SEED_CONTENT_DRAFT.md + VISUAL_ASSET_MANIFEST.md were absent; proceeded only after Kalin dropped them. Both now committed to docs/arcs/.
- Schema (48b55d6): scope_checklists ADD audience text DEFAULT 'rep' ([R]/[RC] tags) + risk_note text ([WHY:]/[KC:] channel). Inert until Phase 4 client instance / interview consume them.
- Seed (fc65746, migration 20260710180000): translated the owner-locked draft verbatim into live shape. 97 checklist fields — bathroom 23, kitchen 16, deck 12, addition 9, roof 9, fence 9, basement 9, exterior 8, gut 7. 15 modules (14 draft upserted; DELETED electrical_upgrade+water_mold_remediation as renamed→electrical_service/water_mold; waterproofing survives). 37 conflict rules (35 §11+answer-driven, inert until Phase 3; +2 pre-existing). ALL tenant_id NULL. Idempotent (checksum identical on re-run). Authoring lives in tools/gen_sce_seed.cjs (re-run after a draft red-pen).
- project_type keys (rep-typed Rooms match, lowercase): bathroom, kitchen, deck, addition, roof, fence, basement, exterior, gut. Synonym matching is SCOPE_TO_ESTIMATE's seam, not this seed. Manifest prefixes (bath/kitch/bsmt/addn/ext) are IMAGE names, NOT project_type keys.
- Bathroom reconciliation (draft supersedes live, owner-locked; safe pre-answer-persistence): DELETED 6 keys — shower_type→tub_shower_config, wall_tile_extent→tile_height, shower_enclosure→shower_glass, shower_niche→niche, floor_finish→floor_tile, + vanity_count (superseded by draft vanity split — the ONE field the dispatch didn't enumerate; flagged). 5 survivors (shower_floor_tiled, drywall_wet_area, access_panel, shower_valve, shower_bench) re-ranked 19-23, content preserved (rank-only UPDATE). Compound splits: vanity→vanity_style+vanity_size_in; gut systems→systems_plumbing/electrical/hvac/panel.
- adds_trades verified against live trade_taxonomy (no FK). No Siding/Deck/Fence/Windows/Gutters trades exist → those fields NULL per contract (obvious-only mapping). Bathroom mappings preserved.
- Manifest LOCKED 2026-07-10 + THE PICTURE RULE added to manifest header AND blueprint: "Any field/option added post-lock requires a matching manifest image row in the same change, unless Kalin explicitly waives it." Cross-check (report-only): most "missing images" are non-visual logical forks (age bands, keep/move, yes/no) the manifest intentionally omits; genuine Phase-4B gaps (fence gates/caps, roof ridge, exterior corners) noted.
- LIVE SMOKE (deployed ai-estimator, no fn change — data only): scope_interview on 'kitchen' → open_field_keys money-risk-ordered [layout_change, wall_removal_detail, island, cabinets_supply, cabinet_style, uppers]; agent opens with the rank-1 layout fork. KALIN_QUEUE item (a) struck; blueprint Phase 1B → SHIPPED. Commits: 48b55d6, fc65746.

[LOG — 2026-07-11] — SCOPE_TO_ESTIMATE Phase A SHIPPED (answers stop evaporating)

- FOUNDATION LIVE. Two tables applied + verified (information_schema + pg_policies + pg_constraint quoted): job_rooms (id, tenant_id, job_id TEXT FK→jobs, label, source CHECK typed|scan, scan_room_id, created_at; idx tenant_id,job_id) and job_scope_answers (…, room_id UUID FK→job_rooms, field_key, option_key, value, trade, source CHECK rep_typed|rep_card|measured|extracted|client_selected, status DEFAULT 'proposed' CHECK proposed|confirmed, confirmed_by/at, updated_at; UNIQUE NULLS NOT DISTINCT (tenant_id,job_id,room_id,field_key); idx tenant_id,job_id). jobs.id is TEXT (not uuid — the "must be UUID" rule is value-format; FK type matches). Both staff-only RLS (job_notes pattern: can_access_job + tenant). Migrations 20260711120000, 20260711120100.
- STEP-0 CORRECTION (prior audit was wrong): ai-estimator handleScopeInterview ALREADY returned `answers` on the scope_interview response (AnswerRecord[]) — the "no data.answers" claim was true only of the FRONTEND (EstimateTab never read it), not the response. No redesign needed; mapped the existing internal answers to the persisted shape at the two return points.
- SURFACE (a8356b5): pure toScopeAnswerPayload maps AnswerRecord→{field_key,option_key,value,trade,source}. option_key resolves choice values to their option (space/underscore-insensitive); source maps typed→rep_typed, measured→measured, photo/plan/assumed→extracted; trade NULL (ScopeField drops adds_trades — per-answer trade is Phase D). Existing response fields untouched.
- PERSIST (4a15e2d): sbEnsureDefaultRoom (idempotent, 1 room/job, label=project type, source typed) + sbUpsertScopeAnswers (upsert on unique tuple, .select('id') catches RLS silent-deny). EstimateTab persists data.answers at the scope-interview response site (ensure room→stamp room_id→upsert). SOFT failure — dismissable banner, never breaks interview; no new AI calls.
- RE-TRIGGER (fdc268b): detectTriggers input widened to rep text ∪ prefilled answer values ∪ humanized labels — closes audit Q3 (measured/extracted answers, never typed into chat, couldn't fire modules). Card picks + AI-extracted already reach it via chat text. Placed in ai-estimator (edge), NOT frontend, because detectTriggers is an edge-only pure module — deviation from the commit-label "feat(estimate)"; the label said estimate, the code lives in ai-estimator.
- LIVE-VERIFIED (deployed fn + real PM JWT): (1) scope_interview 'bathroom' with prefilled_answers.shower_entry='curbless' (word NOT in rep text) → answers[shower_entry]={option_key:curbless,source:measured}, answers[layout_change]={option_key:keep_layout,source:rep_typed}; fired_modules=[waterproofing,accessibility] (Step 4 proof — value in no rep text fired modules, injecting membrane_type/curb_type/grab_bar_blocking into open fields). (2) As test-pm through RLS on 999 Sandbox: default room inserted then idempotent (exactly 1 room); upsert#1 201/2 rows, upsert#2 200/2 rows no-dup; rows landed status='proposed' with correct option_key/source/room_id. Test rows cleaned up (sandbox pristine). npm run build passes.
- DECISIONS/FLAGS: trade NULL in Phase A (Phase D plumbs adds_trades through ScopeField). Staff RLS uses can_access_job which ALSO admits the linked client → Phase C MUST scope this policy to staff roles when it adds the forced-value client INSERT policy, else the permissive RLS union bypasses the vet-gate (noted in both migration headers). ON DELETE CASCADE on job FKs (room_id too), confirmed_by ON DELETE SET NULL — chosen (blueprint silent), avoids job-delete FK block. gen:types after-migration (CLAUDE.md rule) DEFERRED to keep the diff scoped — recommend `npm run gen:types` + commit database.types.ts as follow-up.
- KNOWN LIMITATIONS (per dispatch, not fixed here): no client-role RLS / SELECTIONS (Phase C); no read-back pre-fill into interview start (Phase B); no estimate_line_items link cols (Phase D); single default room only. Minor deferred: an AI-mapped answer value that differs from the rep's raw phrasing won't fire a module (only rep text + prefilled/session values feed the re-trigger); the measured/extracted gap — the actual Q3 target — is closed.
- KALIN REVIEW (browser seat — only thing not machine-verified): open EstimateTab on a bathroom job, run the interview, tap/answer a choice, confirm job_scope_answers rows land (option_key set for card picks, status proposed) and re-answering updates rather than duplicates.
- Commits verified against git log: 7c6b661, 93fea8e, a8356b5, 4a15e2d, fdc268b.

[LOG — 2026-07-11] — SCOPE_TO_ESTIMATE Phase B SHIPPED (interview resumes from the store)

- READ-BACK PRE-FILL LIVE. Interview start AND resume now load persisted job_scope_answers and feed them into the interview so answered fields aren't re-asked. gen:types regen committed first (0dc164e — Phase A follow-up; job_rooms + job_scope_answers types now present).
- STEP-1 AUDIT: session prefill enters via sessionPrefill.measuredFields → prefilled_answers body param (EstimateTab sendEstimatorMessage) → edge sessionAnswers (was hardcoded source='measured') → merged (AI wins on shared key) → preAnswered in the system prompt. START (startEstimate) and RESUME (one-shot effect, re-enters mode on reload, sends no message) are TWO entry points that CONVERGE on sendEstimatorMessage — the single shared injection point. THE WRINKLE (confirmed = re-asking answered fields): pre-B, answeredKeys = AI re-extraction from the reloaded conversation + session prefill only; the STORE was never read back, so measured/extracted answers (or anything if the conversation was trimmed) stayed in open_field_keys → cards re-render + AI re-asks.
- LOAD (13b045c): new sbLoadScopeAnswers (staff RLS, proposed+confirmed). sendEstimatorMessage loads the store and merges with session measured fields → prefilled_answers. PRECEDENCE: store 'confirmed' > session prefill (fresher measurement) > store 'proposed'. prefilled_answers is now SOURCE-AWARE — entries are {value, source} (db vocab), edge maps db source → AnswerRecord source (dbSourceToRecord: rep_typed↔typed, measured↔measured, extracted↔assumed) so a stored answer round-trips to the SAME db source instead of the old blanket 'measured' relabel. Backward-compatible: scalar entries still → measured. Also updated the Phase A re-trigger prefillTriggerText to read .value from the new object shape.
- RESUME FRAMING (4aa2e54): open_field_keys exclusion of answered fields is delivered by the load (store folds into answeredKeys). Generalized the system-prompt header 'ALREADY CAPTURED ON-SITE' → 'ALREADY ANSWERED (you may reference these)' so read-back answers are treated as known and referenceable ("you picked curbless earlier"), never re-interrogated. Re-open-on-contradiction unchanged (AI wins on shared key).
- LIVE-VERIFIED (deployed fn + real PM JWT, 999 Sandbox): seeded shower_entry=curbless (rep_typed/proposed) + tile_height=ceiling (measured/CONFIRMED). Reopen call with rep text mentioning neither → answers round-tripped source EXACTLY (shower_entry stayed rep_typed — NOT flipped to measured; tile_height stayed measured); BOTH excluded from open_field_keys; AI asked other fields, did not re-ask. Round-trip re-upsert (helper, no status in payload): still 2 rows, same ids, tile_height STILL confirmed (status/confirmed_by preserved — not in upsert payload). Re-answer "curbed, not curbless" → AI extracted shower_entry=curb source=rep_typed, upsert hit the SAME id, still 2 rows (no dup), tile_height untouched. Cleaned up (0 rows). npm run build passes.
- ROUND-TRIP INTEGRITY (the Phase B risk): source vocab survives because prefill is source-aware AND dbSourceToRecord/mapAnswerSource are inverses for the Phase-A/B source set (rep_typed, measured, extracted). No-dup guaranteed by Phase A's unique tuple; status/confirmed_by survive because sbUpsertScopeAnswers omits them from the payload (PostgREST merge-duplicates only SETs provided columns).
- DECISIONS/FLAGS: rep_card and client_selected have no distinct AnswerRecord carrier (→ typed/measured) — neither exists in the store during A/B (card picks persist as rep_typed; client_selected is Phase C); Phase C must add a carrier if it round-trips client_selected through prefill (noted at dbSourceToRecord). Store is read on EVERY scope_interview turn (one cheap select) — covers start + resume + mid-interview with one code path; fresh start sees an empty store (no-op). PostgREST bulk insert requires MATCHED keys across all row objects (hit during verification seeding) — sbUpsertScopeAnswers builds uniform rows so it's unaffected.
- KNOWN LIMITATIONS (per dispatch): no client surfaces/RLS (Phase C); no trade derivation / line-item link cols (Phase D); single default room unchanged.
- KALIN REVIEW (browser seat): open a bathroom EstimateTab, run an interview and answer a couple fields, reload the page → confirm the interview resumes and does NOT re-ask the answered fields (no cards for them), and the AI may reference them.
- Commits verified against git log: 0dc164e, 13b045c, 4aa2e54.

[LOG — 2026-07-11] — SCOPE_TO_ESTIMATE Phase C1 SHIPPED (SELECTIONS opens: stamp, vet-gate, client tab, soft-picks)

- CLIENT SELECTIONS LIVE. Client soft-picks finishes in the portal; writes are DB-vet-gated to source='client_selected'+status='proposed'. PM confirm surface + Demo gate + realtime = C2 (not here).
- STEP-0 AUDIT: scope_checklists.audience vocab = {'rep'(38), 'rep_client'(65)} — the client-facing choice filter is audience='rep_client' (data-driven; NO hardcoded trade array). adds_trades is a lossy trade filter (no 'Paint' trade string; shower_entry/paint have adds_trades=NULL) — the day-one-trades decision is already ENCODED in the rep_client tagging. jobs has NO project_type column (only scope, bid_type) → derive project_type from the default room label. scope_checklists is client-readable (sc_select: tenant OR NULL). scope-option-images bucket+table PUBLIC-read (no signing).
- MIGRATION (1fad496, 20260711130000): jobs.selections_opened_at. Role-scoped the Phase A staff FOR ALL policies to owner/pm/rep on BOTH job_scope_answers + job_rooms (CLOSES the Phase A footgun — can_access_job also admits the linked client). Client policies on job_scope_answers: SELECT (confirmed OR own proposed client picks), INSERT (WITH CHECK source='client_selected' AND status='proposed'), UPDATE (own proposed client rows only; USING+WITH CHECK both constrain source/status → no confirmed-row writes, no source/status flip), no DELETE. Client SELECT on job_rooms. ADVERSARIAL-VERIFIED live as client+test: (i) insert source=rep_typed → 403, (ii) insert status=confirmed → 403, (iii) update confirmed row → 0-row (value untouched), (iv) legit client_selected/proposed → 201.
- STAMP (2cbe743): record-signature-evidence stamps selections_opened_at once on signing (service role, .is(null) idempotent, own try/catch — evidence response unaffected; verified fn returned ok:true and stamped). LAZY OPEN: SECURITY DEFINER fn ensure_selections_open(p_job_id) (migration 20260711130100) stamps on client portal load for jobs advanced to in_progress via status-picker/agent (no signature hook), guarded by can_access_job+tenant+eligibility(in_progress+ OR Contract phase complete)+null-only. MECHANISM CHOICE (client has NO jobs UPDATE): definer fn over an edge fn (no deploy for a one-column stamp) and over a client UPDATE grant (would reopen the jobs-write hole). Verified: client direct jobs PATCH → 0-row; client ensure_selections_open RPC → stamped. sbEnsureSelectionsOpen (sb.rpc) helper.
- CLIENT TAB (fd19a67): getClientTabs appends {id:'selections'} once selections_opened_at set (also fixed a latent BASE_CLIENT_TABS module-const mutation). ClientSelectionsView reuses ScopeOptionCards UNMODIFIED (rendered per-field to attach status), audience='rep_client' via a new opt on sbLoadScopeOptionData (EstimateTab call unchanged), project_type from room label, room_id from the job's default room (null-room jobs pick with room_id NULL — client can't create rooms). onPick → sbUpsertClientSelection (upsert on unique tuple = re-pick, no dup). Confirmed picks render locked; proposed show 'awaiting confirmation'. Tokens only (no raw hex); 4 UI states. Verified: soft-pick lands client_selected/proposed, re-pick same id/no-dup, client sees own pick but NOT a staff rep_typed/proposed row.
- gen:types (6ae030c) picks up selections_opened_at. npm run build passes.
- SANDBOX LEFT REVIEW-READY: 999 Sandbox has selections_opened_at stamped + a 'Bathroom' room + 0 answers (a valid lazy-open state: reached in_progress without signing). Note: the [C] sign-fn test left Lead/Proposal/Contract phases forward-advanced on the sandbox (forward-only, harmless); contract_signed/status restored.
- FLAGS: audience='rep_client' includes scope-preference fields (e.g. layout_change) alongside pure finishes — if Selections should be finishes-only, that's a data retag / C2 filter, not a C1 hardcode. Client UPDATE WITH CHECK doesn't forbid setting confirmed_by on a still-proposed row (cosmetic; status stays proposed so it's not "confirmed"; PM confirm keys on status) — flagged. rep_card/client_selected interview-prefill carrier still open (Phase B flag) — lands with C2/D.
- KALIN REVIEW (browser seat): sign in as client+test@avenstonekc.com (pw Avenstone) → open 999 Sandbox → Selections tab renders bathroom option cards with images → tap a finish → 'awaiting confirmation' badge; reload → pick persists.
- Commits verified against git log: 1fad496, 2cbe743, fd19a67, 6ae030c.

[LOG — 2026-07-11] — SCOPE_TO_ESTIMATE Phase C2 SHIPPED (PM confirm, Demo gate, realtime, prefill carrier) — SELECTIONS ARC COMPLETE (A–C shipped; D/E remain)

- STEP-0 AUDIT: phaseGates.js gates LIFECYCLE transitions only (jobs.status); trade phases (Demo) are schedule-driven (derivePhaseStatus) with NO interactive gate. advance_phase (UI PhaseAdvanceCard→sbAdvancePhase, + agent copy) consumes runGatesForTransition. Live: trade_phase_map {Demo→demo,primary}; job_phases.phase_name='Demo'. scope_checklists.audience={rep(38),rep_client(65)}. jobs has no project_type. Staff surface: chose a new pmOnly JobDet tab (reasoning: PM confirm is post-contract execution, distinct from EstimateTab/ScopeTab estimate flow; discoverable, its own file, doesn't touch the interview flow).
- MIGRATION (de65319, 20260711140000): scope_checklists.is_selection BOOL default false; seeded true for 13 finish/material picks (bathroom 7: wall_tile_layout/shower_glass/floor_tile/vanity_style/countertop/fixture_finish/toilet; kitchen 5: cabinet_style/countertop/counter_edge/sink/backsplash_layout; basement 1: flooring). Scope-config/extent/info fields stay false. No paint choice field exists to flag. Tightened client UPDATE WITH CHECK (+ confirmed_by IS NULL AND confirmed_at IS NULL) — closes C1 cosmetic flag. Adversarial-verified: client setting confirmed_by → 403; value re-pick → 200.
- PM CONFIRM SURFACE (ddb6ec2): new pmOnly JobDet 'Selections' tab (PmSelectionsTab). Client picks show 'Client picked' + Confirm; PM override = tap a different option → sbUpsertStaffSelection upserts the tuple (source='rep_card', status='confirmed', confirmed_by/at), superseding the client pick on the SAME row (no versioning). 'N of M confirmed' counter. Reuses ScopeOptionCards unmodified. Helpers sbConfirmScopeAnswer + sbUpsertStaffSelection (staff RLS, .select('id') guard). Verified live: confirm keeps source=client_selected; override → same id, option=PM's, source=rep_card, confirmed; client sees both confirmed.
- DEMO GATE (1984d64): checkSelectionsConfirmed on contract→in_progress (the lifecycle boundary before construction/Demo — Demo the trade phase has no interactive gate point). applicable=is_selection fields for the job's project type (default room label); unconfirmed=those w/o a confirmed row; zero applicable passes silently; failing label names the unconfirmed fields for the existing PhaseOverrideModal card. Verified: 7 applicable → gate names all 7 → 2 confirmed names 5 → all confirmed passes. DIVERGENCE FLAGGED (header): edge copies (ai-master-agent, ai-field-agent) NOT synced — the agent's advance_phase won't enforce the gate until synced (scoped out of C2; follow-up needed).
- CLIENT REALTIME (101d9fb): channel on job_scope_answers (job_id=eq) refreshes the tab so PM confirmations flip the client view without reload; removeChannel on unmount. Client tab now filters is_selection=true.
- PREFILL CARRIER (0ec19b5): AnswerRecord.source += 'client'/'card' (scopeEngine.ts) — dbSourceToRecord/mapAnswerSource now BIJECTIVE for all 5 db sources (client_selected/rep_card no longer relabel on round-trip; closes Phase B flag). Verified: client_selected→client_selected, rep_card→rep_card round-trip. NEW client_prefs body param: client_selected PROPOSED picks surface as a CONTEXT-ONLY 'CLIENT PREFERENCES' prompt section (AI may reference) but are NOT in answeredKeys — field stays open, rep drives. Verified: client_prefs field still in open_field_keys, not in answers. EstimateTab buildPrefill splits: confirmed (any source)+rep's own proposed → gating prefill; client_selected proposed → client_prefs.
- gen:types (8ac5352). npm run build passes across all commits.
- SANDBOX review-ready: 999 Sandbox has a Bathroom room + selections_opened_at stamped + 0 answers.
- FLAGS: phaseGates edge-copy divergence (above) — the one real follow-up. Staff surface is a new 11th JobDet tab (pmOnly) — if tab-count is a concern, could fold into EstimateTab later. is_selection seed is a judgment call on 13 fields (extent/config fields excluded) — easily retagged in data if Kalin wants more/fewer.
- KALIN REVIEW (browser seat): as client+test pick 2 finishes on 999 Sandbox; as a PM (test-pm or your own) open the job → Selections tab → Confirm one, override the other → client tab flips to confirmed WITHOUT reload (realtime). Then try advancing a CONTRACT-status job to in_progress with an unconfirmed selection → gate names it.
- Commits verified against git log: de65319, ddb6ec2, 1984d64, 101d9fb, 0ec19b5, 8ac5352.

[LOG — 2026-07-11] — SCOPE_TO_ESTIMATE Phase D PARTIAL (prereqs + seam + agent-gate close shipped; packet HELD on option→trade map STOP) — SUPERSEDED by the COMPLETE entry below (map blessed + built)

- SHIPPED (map-independent, fully verified): commits d5fd936 (trade_taxonomy +Siding/Deck/Fence/Gutter/Window, parent-only tenant_id=NULL + Avenstone visibility, canonical 43→48), 5de2827 (estimate_line_items room_id FK→job_rooms ON DELETE SET NULL + scope_field_key TEXT — SEAM-ONLY per decision B: threaded as nullable passthrough through NormalizedEstimateInput + sbCommitEstimate row builder, populated by NOBODY today; scoped-delete notes LIKE 'takeoff:%' untouched), 3659a4b (agent gate mirror — closes the divergence above), c3a13e3 (gen:types for the seam columns). New tool: tools/dbq.js (read-only DB query helper for build verification).
- HELD on RETAINED STOP (option→trade map is part of the seam contract; shape needs Kalin/Opus sign-off BEFORE building): commit 2 (per-answer trade derivation), 4 (sbBuildSubWorkPacket), 5 (packet jsPDF + SubsTab). MAP-SOURCE FINDING (the reason for the STOP): NO option-level trade hint exists anywhere — scope_checklists.options is a flat JSONB array of bare option-key strings; adds_trades is a field-level text[]. Nothing binds an option_key→trade. So the map MUST be authored, and its VALUES are field judgment.
- WHICH is_selection fields need the map: only 2 are option-conditional — bathroom.floor_tile (adds_trades ['Tile - Floor','Flooring - LVP']; options porcelain_woodlook/porcelain_stonelook/natural_stone/lvp → lvp maps to Flooring-LVP, the rest to Tile-Floor) and basement.flooring (adds_trades ['Flooring - LVP'] but options lvp/carpet/concrete_polished → carpet≠LVP, concrete≠LVP; the single-trade seed is wrong for 2 of 3 options). The other single-trade is_selection fields derive fine via adds_trades[0]. NULL-trade is_selection fields (bathroom.countertop, kitchen.countertop, kitchen.counter_edge) → trade stays NULL, packet renders them under an "Unassigned — confirm trade" section, never dropped, never force-mapped (decision C).
- PROPOSED MAP SHAPE (recommend, awaiting sign-off): sibling table `scope_option_trades(id, tenant_id UUID NULL=platform, project_type TEXT NULL=universal, field_key TEXT, option_key TEXT, trade TEXT, active, created_at)` with UNIQUE NULLS NOT DISTINCT (tenant_id, project_type, field_key, option_key) + public/tenant read like scope_option_images — mirrors that binding table's exact key. Seeded for day-one selection fields only. ai-estimator loads it alongside scope_checklists; toScopeAnswerPayload derives per-answer trade: single adds_trades → use it; multi/option-conditional → map lookup by chosen option_key; miss → NULL. Alternative shape = an option_trades JSONB column on scope_checklists; rejected as less consistent with the existing option-binding precedent. Do NOT hardcode the map in a component (decision C).
- FOLLOW-UP DATA QUESTION for Kalin (do NOT invent a Countertops trade — decision C): countertop + counter_edge have no trade in taxonomy or adds_trades. Countertops are typically a stone fabricator, not a current trade. Decide: (a) leave Unassigned, (b) fold under Cabinets/vanities, or (c) add a Countertops/Stone trade to the taxonomy. Same open question applies to basement concrete_polished flooring (which trade installs polished concrete?).
...
[LOG — 2026-07-14] — RESET_FAILURE FIXED — diagnosed live under RLS; reset can no longer lie

- METHOD: reproduced as REAL staff (test-pm@avenstonekc.com, anon key + PM JWT — NOT service role, since RLS was the prime suspect and dbq/Mgmt-API bypasses RLS). Sandbox job 5ebd7c3c.
- FINDING 1 (RLS NOT the bug for staff): under PM RLS the scope-answer delete SUCCEEDS — 34 loaded, 26 targeted, 26 deleted, delErr null. can_access_job grants any staff (owner/pm/rep) any in-tenant job; get_my_role()='project_manager' passes job_scope_answers_staff_all. So no policy fix (Step 0(a) cleared the silent-deny suspicion for staff).
- FINDING 2 (over-protection — real): the 7 "protected" survivors were source='rep_card', status='confirmed' — the REP's OWN confirmed selections, NOT client picks. sbClearScopeAnswers protected status='confirmed' regardless of source → rep selections survived every reset = stale scope contaminating re-scope. FIX: protection is now SOURCE-BASED — only source='client_selected' protected; rep_card/rep_typed confirmed clear.
- FINDING 3 (interview-skip mechanism): ScopeConfigurator on start seeds from sbLoadScopeAnswers → sbScopePlan; if scope_complete it calls onComplete immediately (no questions). Proved via direct scope_plan calls: 8 residual answers → scope_complete:false, 26 open (would ask); empty → false, 34 open. So "no questions" requires a NEARLY-FULL store → the store wasn't actually cleared in Kalin's run (34 present incl. 26 clearable = a working reset leaves ≤8). Since staff deletes work, the reset must have been ABLE to fail silently and still show green.
- CORE FIX (the "can't lie a third time" guard): (a) all 3 delete helpers now detect silent-deny — deleted<targeted (scope: targeted ids; line items/room scopes: pre-count vs deleted) → ok:false. (b) confirmReset adds POST-RESET VERIFICATION: re-query stack after deletes (scope by source, ai+takeoff lines, room scopes); any survivor-that-should-be-zero OR any helper ok:false → red 'Reset did NOT fully clear' error IN the dialog, local state NOT reset, success NEVER claimed. Only verified-empty runs the local reset + 'Verified empty' breadcrumb. (c) resetLocalEstimateState now also resets resumeChecked (stale one-shot guard the reset didn't touch).
- VERIFY (PM RLS, quoted): seeded 1 client_selected; pre {client_selected:1, measured:1, rep_card:7}; fixed reset → targeted 7, deleted 7 (guard PASS deleted==targeted), AFTER {measured:1, client_selected:1} — rep confirmations gone, client+measured protected. scope_plan on residual → 33 open (interview asks). Build green. Cleaned up seeded row. Commits 36b0406 (helpers) + e50d43e (confirmReset+verification). Live browser full-flow is Kalin's seat but the delete+verification logic is proven under real RLS.

[LOG — 2026-07-13] — PROPOSAL_STATE SHIPPED — proposals record table + honest Reset (Kalin ruled option 1)

- SCOPE: RECORD ONLY — rows exist, two state moments write them, Reset acts on them. No management UI, no resend, no client exposure. Forward-only: pre-table proposal PDFs unowned (no backfill).
- MIGRATION (commit 703fff3): proposals(id UUID PK, tenant_id, job_id TEXT FK→jobs ON DELETE CASCADE, status CHECK(draft|sent|superseded|void) DEFAULT draft, version INT, superseded_by UUID FK→proposals ON DELETE SET NULL, sent_at, pdf_path, total, created_at, created_by). Index (tenant_id, job_id). Staff RLS proposals_staff_all (can_access_job + get_my_tenant_id). Applied+verified (table/policy/index PASS); gen:types committed. Version = max(version)+1 per job, app-computed.
- HELPERS + WIRING (commit c1732dd): sbUpsertProposalDraft (DRAFT moment: updates existing draft, never stacks; else insert at next version), sbMarkProposalSent (SEND moment: flip draft→sent+sent_at, or create-as-sent if no draft; PRIOR sent→superseded w/ superseded_by=new sent), sbResetProposals (drafts DELETED, sent SUPERSEDED never deleted), sbLoadProposals. Call sites (audited/quoted): saveEstimatePDF sbUploadDoc('proposal') → draft; sendEstimateToClient real-send path (sbSendEstimateEmail + sbUploadDoc, AFTER the gated-approval branch returns early so awaiting-approval is NOT counted sent) → sent. total via computeProposalTotal (matches Proposal-preview markup math); pdf_path = uploaded doc storage_path.
- RESET INTEGRATION: dialog loads real proposal rows — drafts shown "deleted", sent named with date shown "superseded, not deleted" (replaces the old "not touched" line). confirmReset calls sbResetProposals; breadcrumb includes proposal actions.
- RULE: sending supersedes, drafting doesn't. A new draft while a sent row exists = new version; the sent row stays sent until the NEW one is SENT (then old sent→superseded).
- VERIFY (SQL end-to-end on sandbox job, rows quoted, cleaned up): draft v1 → rebuild UPDATES same row (no stack) → send v1 (sent+sent_at) → draft v2 (v1 sent, v2 draft) → send v2 (v1→superseded superseded_by=v2, v2 sent) → draft v3 → RESET (v3 draft DELETED, v2 sent→superseded; sent never deleted) → cleanup 0 rows. Build green. Live frontend flow (build/send buttons write the rows) is Kalin's browser seat.

[LOG — 2026-07-13] — RESET_SCOPE SHIPPED — Reset clears the full estimate stack (proposals gated)

- RULING (Kalin): Reset = "this estimate never happened." Fix 1's reset cleared only scope answers + local state → committed lines survived (contamination). Now the whole draft stack goes.
- AUDIT (artifacts a full run writes): (1) estimate_line_items ai:% + takeoff:% rows — scoped-delete patterns in commitEstimate.js (`.delete().eq('job_id').like('notes','ai:%'|'takeoff:%')`; manual/consultation rows have NULL notes, never matched). (2) job_estimates: messages, scope_origin, approval_status/approval_meta (manager gate), estimate_data (contract_snapshot/contract_total). (3) job_room_scopes (scope tags + scope_details). (4) proposal = an on-the-fly PDF via buildProposalPDF, saved to job_files (category Documents/subcategory 'Proposals') at build/send, emailed via sbSendEstimateEmail; send also fires markLifecyclePhases('proposal_sent') → job_phases Proposal='in_progress'.
- SENT-STATE STOP (the gate): exhaustive check — NO proposals table, NO proposal_sent_at/proposal_status column, NO email log, NO per-doc sent flag. Only proxies: job lifecycle status='proposal' (job-level, ambiguous, also set manually) + a proposal PDF doc (build-time, not sent-vs-draft). => NO dedicated sent-state exists. Per dispatch STOP: proposals are NOT cleared/superseded this pass — reported as a Kalin decision (needs a sent-state, e.g. a proposals table or job_estimates.proposal_status+sent_at, before reset can safely tell sent from draft). Dialog states "Proposals are not touched" explicitly.
- BUILT (commits 5d50a2e helpers + a2824f5 UI): sbClearCommittedLineItems (both prefixes, mirrors commit deletes), sbClearJobRoomScopes, sbClearEstimateDraft (clears messages/approval/scope_origin; PRESERVES estimate_data contract_snapshot — a downstream/protected artifact), sbEstimateStackCounts. confirmReset clears scope answers + line items + takeoff tags + draft in one action; measurements stay unless ticked; client selections protected behind the extra amber tick; breadcrumb (sbNote) logs every count. resetLocalEstimateState also drops lineItems/prop views. Dialog lists the stack in plain rows w/ counts. Staff RLS already covers all deletes/update (no migration).
- VERIFY: build green each commit. Full sandbox run→Reset (line items zero both prefixes, answers zero, takeoff empty, draft gone, measurements intact, interview re-asks from zero) is Kalin's browser seat. Sent-state supersede is BLOCKED pending Kalin's call on adding the state.
...
- FIX 1 (commit 727024d) — "Start fresh" reset. EstimateTab Reset→"Start fresh" opens a confirm that also deletes job_scope_answers. New sbClearScopeAnswers(jobId,{includeScope,includeMeasured,includeClientConfirmed}) deletes BY ID with protection: MEASURED (scan dims) kept unless ticked (default off), CLIENT_SELECTED/CONFIRMED kept unless an explicit amber extra tick (homeowner picks never vanish silently). Dialog loads live counts. Breadcrumb via sbNote logs cleared count. Explicit-only; nothing auto-clears. RLS: job_scope_answers_staff_all FOR ALL already covers DELETE (no migration).
- FIX 2 (commit 14363ea) — deterministic waste. Was: LLM narrated waste but didn't multiply ("138 SF net +15% = 138"). Now model emits base_quantity(net)+waste_pct(from fixed table); applyWasteDeterministic (ai-estimator, runs BEFORE priceScopeLines) computes quantity=base×(1+pct/100) and AUTHORS the waste tail on the description. SF/LF only; EA/LS/room/load/sq→0; pct clamped [0,25]; base==null falls back to numeric quantity (no NaN). reconcileFlags surfaces qty×rate≠amount or stated≠applied waste as a "⚠ Check math" block.
- FIX 4 (commit 14363ea) — line descriptions: prompt requires exactly ONE calculation, format "<what> — <basis>: <qty math>"; waste math is code-owned, never in the description.
- FIX 3 (commits 5375c6e estimator + 3df40a6 frontend) — demo double-bill + scope adherence. Prompt: (a) NO OVERLAPPING LINES — a lump stating inclusions can't coexist with itemized inclusions (haul-away is separate, not a demo inclusion); (b) SCOPE ADHERENCE — outside_scope:true lines go to a separated "RECOMMENDED — OUTSIDE REQUESTED SCOPE" group, never the total, default omit; (c) ESTIMATED/AVERAGED qtys ("3 walls×40 SF avg") now require the ASSUMED mark+flag (closed the demo-discipline hole for invented dimensions). CRITICAL PLUMBING: priced_scope (structured) is BOTH the commit source (commitEstimateFromChat, no markdown re-parse) AND the FACE data — so outside_scope had to be handled there too: commit FILTERS OUT outside_scope; StructuredEstimate renders a separate amber recommended section excluded from subtotal; proposal EXTRACT prompt also excludes the recommended heading. Three leak points closed.
- VERIFY: build green each commit; edge deploy green (runs 29284292221, 29284436914 success). LIVE estimator re-run on a clean job w/ the exact Kalin scope is Kalin's browser seat (needs API); logic/prompt/render verified here. Client-confirmed protection seedable via a client_selected/confirmed row.

[LOG — 2026-07-13] — SCAN_INGEST v2 SHIPPED — scan's JSON is the portable artifact; upload = import

- DESIGN (Kalin, locked): a scan leaving the app carries its data as JSON (same shape the Scanner writes to job_lidar_scans); the Estimate attach surface recognizes that JSON on upload and IMPORTS it (full sbSaveJobLidarScan → room link + measured answers + takeoff). A scan file re-entering the app == a live capture. App's own JSON is the contract; no mystery-format parsing.
- AUDIT (0): export today = FloorPlanTab "Export PDF" (jsPDF → job Documents) + a "Download" link on saved plans; NO JSON, NO USDZ, NO native Swift share. So JSON export is PURE JS (STOP condition not triggered). Scanner row shape = sbSaveJobLidarScan params: rooms[] (name,length,width,height,sqft,doors,windows,worldX,worldZ,floor,wallSegments,objects), total_sqft, capture_mode, height_meters/source/points, gps_*, quality_score/grade/deductions, outline_data, edit_overrides. JOB-BOUND (stamped in insert, strip/re-stamp): tenant_id(AV_TENANT), job_id, created_by(AV_USER_ID). Import hook point = EstimateTab addEstFiles (post-UPLOAD_PICKER).
- CONTRACT lib/scanArtifact.js: buildScanArtifact (row → {format:'avenstone-scan',version:1,scan:{...}}, strips job-bound), isScanArtifact, artifactToScanParams (→ sbSaveJobLidarScan params, re-stamps jobId; tenant/created_by re-stamped inside the helper), shareOrDownloadFile (navigator.share files where canShare, else anchor download).
- EXPORT (commit 849928c FloorPlanTab): "Export data" button per scan (alongside Export PDF — PDF stays human-readable) → avenstone-scan-<date>.json.
- IMPORT (commit 770beff EstimateTab): handleUploadInput async pre-filter ahead of addEstFiles — detects avenstone-scan JSON (skips vision), version-guards, prompts "Import this scan to <job>?"; confirmImportScan → artifactToScanParams(job.id) → sbSaveJobLidarScan → whole bridge fires. accept adds .json on both attach inputs. JSON never rides the estimator call; attached PDFs/photos still do.
- IDEMPOTENCY (verified, not assumed): sbLinkScanRoomsToJobRooms (supabase.js ~2073) matches job_rooms by NORMALIZED LABEL → re-import (new scanId→new "${scanId}_${idx}" roomIds) UPDATES the existing room's scan_room_id (linked++), never a 2nd row. Measured answers via sbUpsertScopeAnswers (upsert). ROOMS don't duplicate. NOTE: re-import DOES insert a new job_lidar_scans row each time — identical to a live re-capture (per-capture rows are expected); the dispatch's guarantee is "no duplicate ROOMS", satisfied.
- SCOPE: FilesTab upload (FileUploadFlow) is a bucket-upload pipeline, not an import hook — scan-JSON detection there is non-trivial; Estimate surface ONLY this pass (noted for follow-up). No migration (job_lidar_scans already has every column).
- ON-DEVICE: export share-sheet + iOS file-picker selecting a .json need Kalin's TestFlight confirm; logic/build verified here. buildTakeoffDraft/live scan-save are Kalin's browser seat.
...
- WHY photos-only on iOS: `.pdf,image/*` is image-wildcard-dominant; iOS WKWebView honors `image/*` and shows the Photo Library sheet, not honoring the bare `.pdf` token to add "Choose Files" → Files app + app-saved scans unreachable. No `capture` attr (good), no Capacitor plugin intercepts file inputs (CapacitorHttp only routes fetch/XHR).
- SCAN ARTIFACTS (0.3): two per-job stores — job_lidar_scans (raw; floor-plan PDF generated on demand via buildFloorPlanPDF) and floor_plans (SAVED draft with a STORED PDF in the private `floor-plans` bucket at `${AV_TENANT}/${fpId}/v{n}.pdf`, auto-created by AiIntakeWizard after each scan). "The scan" the rep attaches = the floor_plans stored PDF.
- FIX (commit 6023a08 EstimateTab; helper in prior commit): estFile→estFiles[] on both inputs; each file → its own image/document content block on the SAME estimator call (multi-file vision). accept='image/*,.pdf' + no capture → full iOS sheet. addEstFiles validates per-file (image|pdf, 4.5MB cap) and surfaces rejects while keeping the good ones (NotesPhotosTab batch spirit). Attach-from-job: lists sbLoadFloorPlansForJob, pulls the stored PDF via new sbFetchFloorPlanPdf → File → attach set. Chips w/ per-file remove at both attach points.
- LANDING UNCHANGED: this surface never wrote to a bucket — files are ephemeral estimator context. Kept that; just N files instead of 1. Estimator PRICING MATH untouched. COST NOTE: more attached files = more vision tokens per estimator call, but user-triggered (rep hits Send), not automatic → no rate-limit concern.
- ON-DEVICE: web build can't reproduce the iOS WKWebView picker sheet — FLAGGED for Kalin's TestFlight seat (accept change + "Choose Files" + app-scan attach need a device confirm). Multi-select + attach-from-job are browser-verifiable.
- helper sbFetchFloorPlanPdf: storage.download() from `floor-plans` bucket → {ok, blob, filename}, sanitized name.

[LOG — 2026-07-13] — TAKEOFF_BRIDGE Phase 4 + Phase 5 SHIPPED (scan-optional takeoff, ownership, scan pre-fill) — ARC COMPLETE

- KALIN DECISION (locked): division by ROLE — estimator prices the job (labor, allowances, client draft); takeoff builds the MATERIALS list. Both run every job. Dimensions: scan wins when present, interview answers otherwise.
- 4c STOP-GATE audit (cleared, no surgery): the estimator already emits SEPARATE category='materials' lines (39 live vs 97 labor; prompt "Separate labor and material lines"), NOT materials-inside-labor-lumps. So the ownership split is a clean filter, not prompt surgery.
- 4a/4b/4c (commit e15e28b, takeoff.js): buildTakeoffDraft builds SYNTHETIC rooms from job_rooms + job_scope_answers when a scan doesn't cover a matching room (per-room; scan metrics win). floor_sf→areaSf, wall_height_in→height, perimeter=4×√floorSf (square-room APPROX, marked never-silent), wall SF=perimeter×height. Answers map to scope_detail schema keys (floor_sf→floor_tile_sf, shower dims→computed SF, vanity_size_in→vanity_width, countertop→vanity_top, vanity_config→sink_count, toilet→toilet_type, shower_glass→shower_door_type); synthetic bathroom uses full_remodel schema so computed shower SF + fixture lines run; missing keys → wizard REP-MUST-ENTER. 4c: synthetic-room labor lines dropped (roomId 'answer_%') — takeoff=materials, estimator=labor; scan-path unchanged. Wizard marks synthetic rooms.
- 4d (commit 65489dc, takeoff.js): tile_height categorical→inches is a FALLBACK for shower_wall_height_in (the takeoff consumes shower_wall_sf, not tile_height): ceiling→ceiling height, standard→84, wainscot→48 (shower heights, not the dispatch's room-wainscot ~48/~42 — stated).
- PHASE 5 (commit 65489dc, supabase.js): sbLinkScanRoomsToJobRooms now also writes scan DIMENSIONS as measured/proposed answers (floor_sf from sqft, wall_height_in from ceiling height) on the linked room — dimensions only, idempotent via sbUpsertScopeAnswers. Configurator hydration pre-fills → stops re-asking. VERIFIED: measured floor_sf persists; hydration returns it → scope_plan treats it answered → skipped.
- BEDROOM/PAINT vehicle (commit d720f3b): seeded 'bedroom' room_type Paint - Interior template (materials_formula) + unit costs + roomMatchesType/ROOM_TYPES. Paint gallons math verified (150 SF→Primer 1.28/Wall 2.19/Ceiling 0.38 gal). REPORTED not built: the bedroom INTERVIEW checklist is thin (no dimension/paint-scope fields) — the interview/synthetic path for bedrooms would need bedroom checklist fields; the paint test runs on the SCAN path.
- SANDBOX review-ready: 999 Sandbox bathroom (job 5ebd7c3c, room 6833c2c0) now has a measured floor_sf=48 → synthetic-room + pre-fill browser test ready. Full buildTakeoffDraft/scan-save runs are Kalin's seat (can't run frontend/LiDAR headless); logic + math + build + data all verified here.

[LOG — 2026-07-13] — TAKEOFF_BRIDGE Phases 1-3 SHIPPED (identity bridge, double-count guard, dimension capture) — Phase 4 GATED, Phase 5 later

- PHASE 1 identity bridge (commit b2744b4): sbLinkScanRoomsToJobRooms create-or-links a job_rooms row per scan room, called from sbSaveJobLidarScan (non-fatal). Writes job_rooms.scan_room_id (was populated by nothing). IDEMPOTENT BY CASE-INSENSITIVE LABEL — the key insight: a re-saved scan gets a NEW scan.id (new "${scanId}_${idx}"), so matching on scan_room_id would duplicate; matching on LABEL links the existing room (and links a scan "Bathroom" to the interview default "Bathroom" → no second room). source='scan' on new rows (CHECK typed|scan). Edge: two same-label scan rooms in one save → separate rooms; a linked interview room keeps its original source. Forward-only (no backfill). Logic unit-tested (link/re-save-no-dup/create/dup-label); browser scan-save is Kalin's seat.
- PHASE 2 double-count guard (commit 28bb585): estimator (notes 'ai:%') + takeoff (notes 'takeoff:%') scoped-delete DISJOINTLY (commitEstimate.js) → both coexist → same scope double-priced once takeoff can run. sbCommittedLineSummary(jobId) buckets estimate_line_items by notes prefix → {ai:{count,trades[]},takeoff:{count,trades[]}}. TakeoffWizard warns (naming overlapping trades) if the job has estimator lines; EstimateTab warns before committing an estimator draft if takeoff lines exist. WARN, don't block — the ownership model is the gated Phase 4 decision. Verified: bucketing + live query (138 ai / 0 takeoff currently).
- PHASE 3 dimension capture (commits 8520530): the configurator was DIMENSIONLESS (finishes but no areas). Added 5 bathroom NUMBER fields as data (floor_sf, wall_height_in, shower_width_in, shower_length_in, shower_wall_height_in), audience='rep', helpers, money_risk_rank -5..-1 (lead but AFTER existing_* -10..-6, before wants 1+). Shower dims suppressed on tub_only+combo (composes: combo→7, tub_only→9). tile_height STAYS categorical — the categorical→inches mapping (ceiling→wall_height_in, standard≈48-60in, wainscot≈36in) is deferred to Phase 4's takeoff consumer, NOT built. EstimateTab prefers a persisted floor_sf answer (summed across rooms) for project_sf; floor_sf persists via the normal store (no new path). Render via the existing S2 number-input. LIVE-VERIFIED via scope_plan: order existing_*→dims→wants; combo suppresses shower dims; floor_sf=40 round-trips as a number answer.
- NOT BUILT (per dispatch): Phase 4 (takeoff scan-optional + ownership model) is GATED on Kalin's decision re takeoff's role on interview-only jobs — buildTakeoffDraft room-source untouched. Phase 5 (scan→configurator prefill) later. The (2)⇄(3) quantity gap + double-count analysis are in the prior audit.

[LOG — 2026-07-12] — CONFIGURATOR_POLISH Phases 5 + 6 SHIPPED — ARC FULLY COMPLETE

- PHASE 5 (unit↔lump-sum toggle, commits 676461a): each gap/rate line gets a $/unit ↔ LS toggle (GapBatchAsk). LS mode = one flat $ = the line total; applyGapRates produces the existing LS shape (quantity=1, unit='LS', unit_price=lump, rate_provenance='lump_sum') so it round-trips through StructuredEstimate (fRate already renders LS/EA/room/load/sq flat, no /unit), commit (unit='LS'), and the proposal PDF ("1 LS") unchanged. STEP-0 AUDIT: the learn-loop IS BUILT (board was stale) — saveLearnedRates → sbInsertRateBookLabor persists rep rates to rate_book_labor on opt-in. RATE-BOOK GUARD: an LS gap is NEVER pushed to learnCandidates → can't reach the rate book; the explicit 'lump_sum' provenance + unit='LS' make the stored shape unambiguous (no CHECK on rate_provenance, verified). Logic-verified: LS line excluded from candidates, per-unit line still learns; Phase-1 <$10 sanity floor still fires on absurd LS.
- PHASE 6 (the ending, commit 20d5bb8): 6a — new configuratorPath flag (set in startEstimate pt-branch); on that path the chat-bubble block + "View raw estimate" toggle are gated OFF (dormant scope_interview chat code stays, just never rendered — tracked debt, not removed), a clean "Building your estimate…" loading shows during pricing, then lands on the StructuredEstimate breakdown — user never sees the raw prompt/transcript. 6b — "Commit to Line Items" collapsed into "Proposal →": openProposal now commits the priced draft itself (commitEstimateFromChat returns {ok,items}; sbCommitEstimate source='ai' scoped-delete 'ai:%' + all side effects UNCHANGED — commitEstimate.js untouched this window), reloads committed rows to avoid the async lineItems race, and blocks with propErr on failure (never a silent skip). 6c — gap flow untouched beyond the Phase 5 toggle. Browser E2E (completion→loading→breakdown→Proposal commit→PDF) is Kalin's review; build + side-effect intactness verified here.
- ARC FULLY COMPLETE: Phase 1 (PDF), Phase 3 (suppression/existing-conditions/orphans), vanity addendum, 4a (labels), 7 (images), 4b/c/d (language), 5 (LS toggle), 6 (ending). Nothing outstanding.

[LOG — 2026-07-12] — CONFIGURATOR_POLISH Phase 4b/4c/4d SHIPPED (language + helpers + tier framing) — ARC COMPLETE

- 4b QUESTION REWRITES (migration 20260712180000, commit 2272790): plain phrasing on the named jargon repros. Accessibility MODULE fields rewritten (and door_widths/threshold_heights text→choice): door_widths named as the DOORWAY for wheelchair access (NOT the shower — audit: no takeoff consumer, it's an accessibility-module field so it's already gated by the module firing; no extra suppression needed), grab_bar_blocking ("wood backing so grab bars mount later"), threshold_heights (step-free), future_proofing_level (concrete aging-in-place meaning). Base bathroom jargon rewritten: shower_drain, drywall_wet_area, niche, tile_height, ventilation, access_panel, shower_floor_tiled.
- 4c HELPERS: scope_checklists.helper TEXT column + per-field one-liners (module fields carry "helper" in adds_fields JSONB). ScopeField now carries helper + option_labels (base from columns, module from JSONB); scope_plan emits them (commit 29255b4 engine). Configurator renders an unobtrusive ⓘ helper line under the question.
- 4d TIER FRAMING: waterproofing membrane_type good/better/best via helper ("Good: hot mop... Better: Kerdi sheet membrane...") + option_labels (hot_mop/schluter_kerdi/other), keyed to the Phase 7 images.
- LIVE-VERIFIED (scope_plan, walkin+curbless fires accessibility+waterproofing): door_widths/grab_bar_blocking/shower_drain return rewritten questions + helpers; membrane_type returns tier helper + option_labels. npm run build + edge deploy green.
- CONFIGURATOR_POLISH ARC COMPLETE: Phase 1 (PDF fixes), Phase 3 (suppression/existing-conditions/orphans), vanity addendum, Phase 4a (labels), Phase 7 (images), Phase 4b/c/d (language). Phases 5 (unit↔lump-sum pricing toggle) + 6 (polished ending + button collapse) remain UNSTARTED by design.

[LOG — 2026-07-12] — CONFIGURATOR_POLISH Phase 7 SHIPPED (shower_valve + membrane_type option images)

- Generated 6 images (Gemini gemini-3.1-flash-image, ~$0.24 class): bath_shower_valve_{standard,rain,body_sprays,rain_and_body}.png + bath_membrane_type_{hot_mop,schluter_kerdi}.png (styled as tiers for 4d). Subjects added to tools/image_gen_subjects.json. Uploaded via tools/upload-visual-assets.mjs (shower_valve auto-bound by the checklist matcher); membrane_type bound via migration 20260712170000 (module field, not auto-bindable). commit a8dccb6. COVERAGE: all 6 option keys active-bound (membrane 'other' = catch-all, no image by design). NOTE: countertop/counter_edge were ALREADY generated+bound (10 rows, real files) — the Phase-D "no images" LOG was stale; corrected. GOTCHA: `node tools/generate-visual-assets.mjs` (full-scan mode) buffered/stalled headless; `--regen <file>` per image (skips the 201-file alreadyGood scan) worked immediately — use --regen headless.

[LOG — 2026-07-12] — CONFIGURATOR_POLISH vanity addendum SHIPPED (was missed in Phase 3)

- (a) vanity_config CHOICE (single/double/none; "none"="No vanity work", rank 11 leads the vanity block) replaces vanity_count NUMBER (retired active=false). (b) vanity_size_in NUMBER→CHOICE 24/30/36/48/60/72/custom (verified vs takeoff scope_detail_schemas vanity_width fixture_select — same list). (d) countertop was already "Vanity top material?" (the vanity top, options quartz/granite/cultured_marble = takeoff vanity_top) — now gated. Suppression re-keyed: vanity_config=none ⇒ vanity_style/vanity_size_in/countertop (old vanity_count=0 rows deleted). Commit 276252f.
- (c) custom→ALLOWANCE (commit ef680a0): estimator scope-prompt rule — custom/unstated size → line description begins "ALLOWANCE — pending bid: " + a flag, never a firm price. Proposal PDF's existing /allowance/i path italicizes + "(allowance)", so it surfaces on draft + PDF (description-convention v1, no schema). LIVE-VERIFIED: vanity_config=none suppresses the 3 fields; vanity_size_in=custom session returned "ALLOWANCE — pending bid: custom non-standard width vanity..." + "...quartz countertop for custom vanity..." + an ALLOWANCE flag; demo line cited the stated existing vinyl (not invented).
...
- ENGINE (commit 6a77111): ai-estimator loadSuppressions + computeSuppressedSet; handleScopePlan filters suppressed out of fields/open_field_keys/persist-answers; handleScopeInterview filters from prefilled. Configurator honors it automatically via scope_plan output. LIVE: combo→4 shower fields gone; walkin+curbless→curb_type+threshold_heights gone but membrane_type+grab_bar_blocking KEPT (the double-ask fix); roof standing_seam→shingle_grade gone.
- EXISTING-CONDITIONS FIELDS (commit 7357a47): 10 new scope_checklists fields (bathroom existing_tub_shower/wall_finish/floor_finish/vanity/countertop; kitchen existing_countertop/flooring/backsplash; basement existing_finish/flooring). audience=rep, NEGATIVE money_risk_rank so they LEAD the interview (no CHECK on the column).
- ESTIMATOR CONSTRAINT (commit 38509fd): scope prompt now requires a STATED existing_* fact for any demo/removal line; else omit or mark "ASSUMED — confirm on site: ..." + flag; universal consequences (haul-away/dumpster/protection) still inferred. LIVE-VERIFIED: a bathroom session with NO existing_* answers returned the demo line as "ASSUMED — confirm on site: full gut... No existing conditions recorded" (13 assumed markers), haul-away/dumpster inferred unmarked — not invented as fact.
- ORPHAN HANDLING (commit a2b922c): TWO layers. (1) scope_plan returns `suppressed[]`; ScopeConfigurator deletes any answered-then-suppressed field locally + via sbDeleteScopeAnswers (delete-on-gate-change). (2) sbBuildSubWorkPacket defensively excludes suppressed answers (sbLoadSuppressions + computeSuppressedFieldKeys). lib/scopeSuppress.js = pure frontend mirror of the edge computeSuppressedSet (DIVERGENCE GUARD header — keep both in sync). LIVE: scope_plan combo returns suppressed=[shower_entry,shower_drain,shower_floor_tiled,shower_bench]; filter drops a stale shower_entry while keeping floor_tile.
- npm run build + edge deploys green. Browser chip-flip delete + PDF are Kalin's review.

[LOG — 2026-07-12] — CONFIGURATOR_POLISH Phase 1 SHIPPED (proposal PDF defects from Kalin's first live bathroom run)

- 1a COLLISION (commit 80d5ab8, pdf.js buildProposalPDF): "...floor tile" line overprinted the price. TWO root causes, both the DRAW_PDF_POLISH class, fixed builder-wide: (1) splitTextToSize measured the description under whatever font the PREVIOUS row left set (bold-9 trade header / italic-7 allowance caption) then rendered at normal-8.5 → wrap width wrong → overflow into QTY/PRICE. Fix: set render font BEFORE measuring (proven: stress line measured@italic7 = 382.5pt overflows col 316; measured@8.5 = 283.5pt fits). (2) buildProposalPDF never sanitized dynamic text (buildContractPDF does) — added T()/wrap() (_winAnsi) and routed EVERY dynamic doc.text + splitTextToSize through them (line items, trade headers, subtotals, address/client, flags, disclosed-CO moments incl. the en-dash cost line, payment schedule).
- 1b SANITY GUARD (commit aecf775, new lib/priceSanity.js + EstimateTab): blocks "Send to Client" until mispriced lines are fixed or acknowledged (a $3 mud-bed float reached a client PDF). computeSanityFlags: client price >$0 and <$10 (ABSOLUTE FLOOR — caught the reported line), OR unit_cost >5× rate_book_labor high / <0.2× low WHERE a (trade,unit) book range matches (fires only on match; AI trades that don't map to canonical rate-book trades rely on the $10 floor). Amber price-check panel lists each flagged line + reason + "I reviewed these" checkbox; a line-items change invalidates the ack. Verified: $2.99 float flags, $45/sf Demo vs book-high $6 flags, in-band/no-match pass.
- 1c CASING (commit 2986e3d, pdf.js): AI assigns inconsistent trade casing ("materials", "TILE & WATERPROOFING", "GENERAL" coexist). titleCaseHeader normalizes section headers + subtotals at render; acronyms (HVAC etc.) preserved. Verified materials→Materials, TILE & WATERPROOFING→Tile & Waterproofing, HVAC→HVAC.
- npm run build passes each commit. PDF visual is Kalin's browser review (headless render not run; mechanism proven via jsPDF measure test).

[LOG — 2026-07-12] — Phase D orphans: Countertops trade added; countertop/counter_edge closed; concrete_polished still open

- WHAT (migration-only, commit ed6c101): Kalin resolved the countertop half of the Phase D standing data question — countertops go to a dedicated countertop SHOP (distinct vendor, not cabinets/tile). Added `Countertops` as a parent-only platform trade (trade_taxonomy tenant_id=NULL, display_order 240, sf; + Avenstone tenant_trade_visibility row — Siding/etc precedent). Canonical trades 48→49.
- SEED: scope_option_trades rows countertop + counter_edge → 'Countertops' for EVERY option key, project_type set per AUDITED real fields (countertop in bathroom {quartz,granite,cultured_marble} + kitchen {quartz,granite,butcher_block,laminate}; counter_edge in kitchen {eased,waterfall,ogee}) = 10 rows. Both fields are single-trade regardless of material/edge (same shop). Every seeded row join-checked against a real trade_taxonomy row (no ghost-string break).
- LIVE-VERIFIED via deployed deriveTrade (ai-estimator NOT modified — scope_option_trades is read per-request, so new rows take effect with no redeploy): bathroom countertop=quartz → 'Countertops'; kitchen countertop=butcher_block → 'Countertops'; kitchen counter_edge=waterfall → 'Countertops'; basement flooring=concrete_polished → NULL (still orphan, by design).
- STILL OPEN: `basement.concrete_polished` — no trade; needs its own Kalin answer (see Active open items). Do NOT guess.
- PARKED: countertop/counter_edge option IMAGES not generated yet — scope-option-image library has no bound cards for these; generate via the Gemini API path (~$8 class, same as the 195-image library), NOT a vendor catalog (SKU-level vendor catalogs = future SELECTIONS catalog arc).
- Trade-aware: Countertops is platform (tenant_id=NULL) + Avenstone visibility; option→trade rows are platform data — no hardcode.

[LOG — 2026-07-12] — ESTIMATE_CONFIGURATOR S2 SHIPPED (tap-through scope configurator, composes with Phase D map)

- WHAT: the Build tab now gathers scope through a tap-through configurator (one question per screen) instead of the LLM chat interview, when a project_type resolves. Deterministic (no LLM) via scope_plan. Commits 66f18b7 (edge: scope_plan also returns persist-ready `answers` — option_key + server-derived trade via Phase D deriveTrade, source 'card'→rep_card), 071c0b7 (sbScopePlan helper), cb19f96 + a526ec4 (ScopeConfigurator.jsx — cards where option images exist, big controls where not, chip strip to jump back, re-fetch plan after each tap to unlock trigger-fired follow-ups, resume-hydrates from sbLoadScopeAnswers), 72bf94c (EstimateTab wiring).
- SINGLE WRITER preserved: configurator persists each answer through EstimateTab.persistScopeAnswers (:465 → sbEnsureDefaultRoom + sbUpsertScopeAnswers). Trade is NOT stamped client-side — it rides back from scope_plan (server-derived). SOURCE = rep_card (a structured tap from a known vocab is a card pick, not freeform rep_typed); the chat path's labeling is unchanged; D reads both identically (by field/option, not source).
- PRICING HANDOFF UNCHANGED: startEstimate seeds estMessages with the scope prompt (no LLM turn); on complete the configurator's onComplete injects a scope summary into estMessages and flips scopeComplete → the existing [scopeComplete] effect runs runPricing. onSkip = forceDraftAnyway (unchanged soft-gate). Chat input hidden during capture, returns for priced-estimate follow-ups.
- LIVE-VERIFIED (deployed scope_plan): tap floor_tile→lvp returns trade='Flooring - LVP' source=rep_card; wall_tile_layout→subway_offset → 'Tile - Wall / shower'; floor_tile→porcelain_stonelook → 'Tile - Floor' (map-governed); countertop→granite → trade=null (orphan, no crash). Proves the configurator composes with D's scope_option_trades map end-to-end. npm run build passes; edge deploy green.
- EDIT REGIONS (S2 window, for coordination): supabase.js — one new export sbScopePlan (just before sbEnsureDefaultRoom, clear of D's sbBuildSubWorkPacket 2601-2668). EstimateTab.jsx — import swap (:13 ScopeOptionCards→ScopeConfigurator; ScopeOptionCards no longer used here), startEstimate pt-path + handleConfiguratorComplete (~:484-513), JSX swap (~:1049), removed the "gathering scope" banner, gated the chat-input row during capture. ai-estimator/index.ts — handleScopePlan returns `answers`. New file ScopeConfigurator.jsx. NOTE: scopeOpenFields/setScopeOpenFields + scope_interview mode in sendEstimatorMessage are now dormant on the Build path (chat interview superseded by the configurator) — left in place, harmless; a later cleanup can remove them.

[LOG — 2026-07-11] — SCOPE_TO_ESTIMATE Phase D COMPLETE (SUB_WORK_PACKET shipped; option→trade map authored)

- MAP AUTHORED (blessed shape). Commit 18cfc4e: scope_option_trades table — sibling of scope_option_images, SAME key (UNIQUE NULLS NOT DISTINCT (tenant_id, project_type, field_key, option_key)) + public-read/owner-write RLS. Seeded platform (tenant_id NULL): bathroom.floor_tile {porcelain_woodlook/porcelain_stonelook/natural_stone→'Tile - Floor', lvp→'Flooring - LVP'}; basement.flooring {lvp→'Flooring - LVP', carpet→'Flooring - Carpet'}; concrete_polished INTENTIONALLY unmapped → orphan. Trade strings pre-verified against trade_taxonomy canonical format (join-break guard); all 6 rows join to a real taxonomy row.
- TRADE DERIVATION (commit 7ac8d19, edge deploy green). adds_trades threaded onto ScopeField (scopeEngine.ts assembleChecklist + collectRequiredFields); ai-estimator deriveTrade: a field is MAP-GOVERNED if it has ANY scope_option_trades row → chosen option's trade wins, miss=NULL (this is what correctly orphans basement.concrete_polished despite its lone adds_trades value); un-mapped field → adds_trades[0] if single else NULL. loadOptionTrades + buildOptionTradeIndex loaded once per interview, passed to both toScopeAnswerPayload paths. Persist helper sbUpsertScopeAnswers already carried trade (Phase A) — no frontend change. Composes with S1 scope_plan (ffb04e1: S1 only touched index.ts, reads a ScopeField subset, doesn't touch toScopeAnswerPayload). LIVE-VERIFIED (deployed): floor_tile+lvp→'Flooring - LVP', floor_tile+porcelain_woodlook→'Tile - Floor', countertop→un-mapped+adds_trades NULL→NULL (no crash).
- PACKET HELPER (commit 60a06e5): sbBuildSubWorkPacket(jobId, trade) at supabase.js:2601-2668. Confirmed job_scope_answers × room × bound image (scope_option_images, project-type beats universal), filtered to trade → {job, trade, rooms:[{room_id,room_label,picks}], unassigned:[trade-NULL picks]}. Trade-NULL never dropped. {ok,error,data} staff RLS, regenerate-on-change v1 (current confirmed state, no diff engine).
- PACKET PDF + SURFACE (commit 810b067): buildSubWorkPacketPDF in pdf.js — jsPDF (NOT pdf-lib; pdf.js has always been jsPDF/_winAnsi, the Phase-D audit corrected the CLAUDE.md 'pdf-lib/safe()' claim), modeled on buildContractPDF: navy/gold header, _winAnsi() all dynamic text, chkPage pagination, doc.addImage PNG (public URLs pre-fetched → dataURL, per-image try/catch degrades to text). "Unassigned — confirm trade" section for orphans. SubsTab 'Work packet' button per ACTIVE engagement (per assigned sub/trade) → helper → PDF → doc.save. gen:types regen (f2e8983).
- VERIFIED: derivation 3 cases live (above). Assembly SQL-mirrored against the sandbox (999 Cost Plus Sandbox, job 5ebd7c3c…): after a sandbox-only trade backfill (via the exact deriveTrade rule — production rows stay NULL per decision B), the 7 confirmed bathroom picks group into Tile-Wall/shower(2, both images bound), Plumbing-Finish/fixtures(2), Cabinets(1), Tile-Floor(1, floor_tile porcelain→Tile-Floor proves map-governed), Unassigned(1: countertop granite). Satisfies "≥2 picks across ≥2 trades + one Unassigned." npm run build passes; edge + Vercel deploys green.
- SANDBOX REVIEW-READY: the sandbox's only ACTIVE sub is 'Framing' (no bathroom picks → its packet shows the Unassigned section only — a valid live demo of the button+PDF+orphan handling). To see grouped picks in-browser, point/add a sub on 'Tile - Wall / shower' or 'Plumbing - Finish / fixtures' (both have 2 confirmed picks). KALIN REVIEW: JobDet → Subs → an active sub → 'Work packet' → PDF downloads (header, room-grouped picks + option images, Unassigned section). I did NOT mutate/add engagements (left the seat-review Framing engagement intact).
- KNOWN LIMITATIONS (by design): no live diff/propagation engine (regenerate reflects current confirmed state); estimate_line_items room_id/scope_field_key remain seam-only (no writer populates); trade-less answers render without images; no versioned packet history; no CLIENT_VISION_RENDER (Phase E, parked).
- COORDINATION (S2 window): my ONLY supabase.js edit is the NEW export sbBuildSubWorkPacket at lines 2601-2668 (immediately after sbLoadScopeAnswers, before sbEnsureSelectionsOpen). EstimateTab.jsx untouched. ai-estimator/index.ts + _shared/scopeEngine.ts were edited (trade derivation) — rebased cleanly around S1 (ffb04e1).

[LOG — 2026-07-15] — AVEN CHAT PDF RECEIPTS SHIPPED (route PDFs through ai-extract-sub-invoice, not inline vision) — commit `cfa52d4`

- PROBLEM (from read-only audit same day): Aven chat rejected PDF receipts. Two-point failure — MasterAgent file inputs `accept="image/*,.heic,.heif"` blocked .pdf at the picker; and even if bypassed, the agent sent a hardcoded `{type:'image'}` content block which Anthropic rejects for `application/pdf` (needs `type:'document'` + `anthropic-beta: pdfs-2024-09-25`). Manual TransactionModal path already works because it routes {bucket,path} to ai-extract-sub-invoice (Haiku, document block handled server-side).
- FIX (decided, not re-litigated): route chat PDFs through storage + the existing Haiku extractor, NOT inline vision. PDF bytes NEVER enter the agent context window.
- FILES: `MasterAgent.jsx` — inputs accept `application/pdf,.pdf`; new `isPdfFile()` + `handlePdfReceipt()` (upload via sbUploadReceipt to job-receipts scoped to contextJobId or `pending-<tenant>`, then sbExtractReceipt('job-receipts',path)); on send, inject a structured text line `[RECEIPT PDF EXTRACTED: vendor=…, amount=…, date=…, desc=…, storage_path=…, bucket=job-receipts]` (or a `[RECEIPT PDF ATTACHED — extraction failed …]` variant) built from `pdfReceipt` state; images path UNCHANGED. PDF renders as a document chip (Ic.doc + filename), not an img. `onLibraryPicked` splits pdfs/imgs (one PDF at a time). Extraction failure degrades silently, surfaced in chat via attachErr.
- `ai-master-agent/index.ts` — `log_receipt` tool gains optional `receipt_path`; executor binds an already-uploaded path directly to `receipt_url` (else-if after the image_data branch — identical linkage to TransactionModal), dual-writes job_files with `application/pdf` mime when path ends .pdf. New system-prompt "RECEIPT FROM PDF (Aven chat)" section instructs the model to read the extracted fields and pass `receipt_path`. Round-trip verified: model tool input → `pending_action.input` (:~2960) → executeTool on confirm (:~3040).
- VERIFIED: `npm run build` green (659ms). Binding audited against manual flow before writing (receipt_url stores the storage PATH, not a URL). Deploy: edge fn auto-deploys on push to supabase/functions/**; Vercel on push. NOT yet manually attach-tested in the live app (needs a real PDF receipt on a test job → extraction → confirm card → row + receipt renders from Ledger).

[LOG — 2026-07-15] — LEDGER SEARCH BAR SHIPPED — commit `4e8fff5`

- FILE: `FinancialsTab.jsx` — added `search` state + a search input above the transaction list (className `finp`, clear-× affordance). Client-side filter folded into the existing `filtered` derivation (no new query/debounce): case-insensitive match over payer_or_payee_name, description, type, status, and amount (raw `String(amount)` + `f$()`-formatted). Empty state is search-aware with a "Clear search" button; `selectedIds` resets on query change (added `search` to the reset effect deps). Summary stat cards + loadLedger untouched.
- HEX: my slice is hex-neutral — tree was 1352 before my session and 1352 after (measured via HEAD~1 checkout). The audit gate fails only on PRE-EXISTING drift (+9 over the 1343 baseline last set 2026-06-10) unrelated to this work; did NOT run `--set` (not my cleanup to absorb). Used `var(--card-bg)` / `var(--gold-500)` instead of raw `#fff` / `#C9A84C` in both new UI blocks.
- VERIFIED: `npm run build` green (654ms). Logic verified by reading `filtered`; not manually clicked in-app.

[LOG — 2026-07-15] — READ-ONLY AUDIT: 8617 Houston (job `58345dc5-…`) reconciliation — books CLEAN
- Finding: the suspected double-count/inflation from Blake's raw entries is NOT present. Duplicate detector (payee+amount) = 0 rows; both Aguayo accruals correctly drawn down (painting `97205011`=$16,900 = invoice − payments; electrical `151ad967`=$0). `sbLoadJobFinancialSummary` reads ONLY job_transactions (never sub_invoices) — accruals are the mechanism, no interplay double-count. Blake's 8 KC Energy Savers rows ($12,598.35) are raw cost entries with NO matching sub-invoice → not double-counts, just a workflow bypass. Two owner follow-ups filed to KALIN_QUEUE (f: Blake confirm 6 pending KC rows' paid-status $9,020.41; g: did Aguayo do carpet — possible missing liability, carpet was Southside $4,647 + Carlos $4,500 raw by Kalin).

[LOG — 2026-07-15] — 8617 Houston (`58345dc5-…`) AGUAYO SCOPE CORRECTION + REATTRIBUTION + SETTLE + receivable cards — code commit `f170338`
- SOURCE: owner-supplied Aguayo scope sheet 07/14 ("Complete Job Scope … Change Orders Included.pdf", Total Due $33,563.06). Grew invoice `16ab4bc1` (was $27,900) to $33,563.06 via `edit_sub_invoice_with_ledger` (single line item; accrual `97205011` resynced automatically — never raw-UPDATE it).
- REATTRIBUTION: voided raw KC-Energy-Savers rows `b74cef68` ($2,500, check #1129) + `0bf20161` ($3,500, check #1089) via canonical `status='void'`; re-added both as invoice payments on original check dates through `add_sub_invoice_payment_with_ledger`. Invoice now paid $17,000 / accrual $16,563.06 pending = the GENUINELY UNPAID Aguayo remainder (do NOT settle; owner pays later, then log via invoice payment not raw flip). Aguayo's own remainder invoice says $16,663.06 — their ledger is $100 short on payments received; owner handling directly.
- RPC gotcha: `add_sub_invoice_payment_with_ledger` inserts `created_by = auth.uid()` (NOT NULL). Via Management API `auth.uid()` is null → set `request.jwt.claims` GUC to `{"sub":"<Kalin auth id>"}` in the same txn before calling.
- CO `8ebe366d` ($3,700, CO-001 "Heated tile floor labor and materials") VOIDED as duplicate: scope sheet already folds in "Heated floors $1,500" (labor); material tracked separately as Schluter row `11453386` ($1,077.94).
- SETTLE: relabeled 6 mislabeled KC-Energy-Savers rows (Topline Glassworks, Sarto Countertops, Schluter, Raptor Recycle & Transfer, Menards ×2); batch-flipped 54 remaining pending out-rows → paid ($12,163.71), accrual `97205011` the only row left pending. Owner call: re-typed Topline `2c41390e` + Sarto `76b66647` `sub_payout`→`material_purchase` (markup-neutral 22%=22%) to clear REG-3 orphan check.
- REGRESSION (all 0): stale-accrual, duplicate-payout, orphan paid sub_payout. SUMMARY: total_in $65,000 (unchanged) · spent/paid_out $66,200.79 · pending_out $16,563.06 · voided out 3 rows/$9,700. $6,000 reattribution is spend-neutral.
- CARDS (commit `f170338`, `FinancialsTab.jsx`): cost_plus-only "Billable to Date" (`firm_projected_total`) + "Client Owes" (`remaining_balance`) read straight from `sbLoadClientActualSpend` — same helper as client portal, no independent calc; negative → green "Client credit". Houston post-settle: Billable $102,971.90, Client Owes $37,971.90. Build green; my slice hex-neutral (pre-existing +9 drift unrelated, did NOT `--set`).

[LOG — 2026-07-15] — BILLING TREATMENTS + VOID DELETE + LEDGER LABELS + HEX BASELINE (4 commits) — `8022ac1`, `3e75c60`, `c5bf46f`, `235aa8b`
- COMMIT 1 (`8022ac1`): migration `20260715210000_billing_treatment.sql` — `job_transactions.billing_treatment TEXT NOT NULL DEFAULT 'standard' CHECK IN (standard,no_markup,client_paid)`. All 286 rows standard. Column+CHECK verified live; `database.types.ts` regenerated.
- TREATMENT SEMANTICS: **standard** = reimbursable cost + markup (all existing behavior). **no_markup** = reimbursable at cost, markup rate forced 0. **client_paid** = client bought direct: excluded from cost_subtotal / spent / paid_out / pending_out / outstanding / next_draw / float / reimbursable cost; only the markup on the amount is billable.
- COMMIT 2 (`3e75c60`) — money engine, HELPERS ONLY (divergence rule). COMPUTATION MAP touched: `sbLoadJobFinancialSummary` (total_out, pending_out, outstanding_pending, next_draw, unreimbursed all gate out client_paid; `rateFor()` zeros no_markup; new `client_paid_total`/`client_paid_markup` fold into `projected_markup`→`projected_final_bill`). `sbLoadClientActualSpend` (client_paid skipped from transactions[]+cost buckets, tracked as `client_paid_total`/`client_paid_markup`; no_markup zero-markup; `firm_projected_total` += clientPaidMarkup). `sbLoadUnreimbursedExpenses` excludes client_paid + defaults no_markup markup 0. NEW `sbLoadUncollectedClientPaidMarkup(jobId)`. Draw composer (`ComposeDrawScr`): client_paid no longer selectable as reimbursable; markup-only lines (base 0) bound to source `transaction_id` → `compose_draw` unreimbursed→in_draw flip + pre-flight block = no double-collection.
- VERIFY (live Houston): all-standard cards unchanged; INSERT client_paid $1,000 material → Billable +$220, Client Owes +$220, Spent unchanged (before $104,801.90/$39,801.90 → after $105,021.90/$40,021.90); test row DELETEd, restored to $104,801.90/$39,801.90. (Baseline drifted from the prior session's $102,971.90 because the owner added a live $1,000 `other_expense` + a $500 cost row mid-session.)
- COMMIT 3 (`c5bf46f`): TransactionModal treatment picker (expenses only; "Standard (cost + N%)" uses job markup, generic when labor≠material) + view-mode row + ledger chips (no markup / client-paid). Void-delete: owner-only kebab on `status='void'`; `sbDeleteVoidedTransaction` FK-guards FOUR referrers — `sub_invoice_payments.transaction_id`, `sub_invoices.accrual_transaction_id`, **`change_orders.accrual_transaction_id`** (the extra ref found by audit — not in the dispatch list), `draw_line_items.transaction_id`; then deletes by tenant_id+id with a 1-row count check. Guard validated on Houston's 3 voided rows: `8ebe366d` blocks (co_ref), `b74cef68`/`0bf20161` deletable. Card labels: SPENT→"Total Costs" (paid+unpaid), SETTLED→"actually paid out".
- COMMIT 4 (`235aa8b`): hex baseline 1343→1352 (owner-approved bump). Trace: +9 is diffuse net-churn across ~40 feature commits since the 2026-06-10 freeze (d014a4e) — net movers +12 `#fff`, +7 `#92400e`, semantic colors, minus ~15 reductions. Most have token equivalents but the freeze already accepts 285 `#fff` etc.; re-froze at 1352 rather than refactor shipped files. Commits 1-3 shipped hex-clean.
- KALIN_QUEUE: struck f (6 KC rows — relabeled+settled 07/15) and g (Aguayo carpet — scope sheet has no carpet line, no missing liability); added h ($100 Aguayo discrepancy, handle with sub, books stay $17,000) and i (pay $16,563.06 remainder via invoice payment, not raw flip).

[LOG — 2026-07-16] — RESET_FAILURE-2: "Start fresh estimate" scope_origin NOT NULL — commit `250d41d`
- ROOT CAUSE: `sbClearEstimateDraft` (supabase.js) UPDATE'd `scope_origin: null`. Column is `NOT NULL DEFAULT 'manual'` (migration `20260626090000_job_estimates_scope_origin.sql`, no CHECK — free text; values in use: manual/session/incomplete). Also nulled `approval_status` (nullable but DEFAULT 'none', has CHECK-domain) and `approval_meta` (DEFAULT '{}'). Only scope_origin actually violated NOT NULL; the others were semantically-wrong nulls.
- WHY NOT DELETE: the helper deliberately preserves `estimate_data` (holds contract_snapshot — repro row had contract_total $126,557.32), so the row must persist. Fix = write column DEFAULTS not NULL → reset row identical to never-scoped: `scope_origin 'manual'`, `approval_status 'none'`, `approval_meta {}`, `messages []`. Constraint left intact (NULL isn't a legit scope_origin state anywhere).
- SWEEP: `sbClearEstimateDraft` is the ONLY UPDATE-to-null writer in the reset path; the other clears DELETE (sbClearScopeAnswers/CommittedLineItems/JobRoomScopes), sbResetProposals sets status='superseded'. scope_origin was the only NOT NULL column nulled — whole class covered.
- VERIFY (999 Test Lane 7b44611a, DB-level at the exact failure point): (A) old null-write reproduces `23502 ... scope_origin ... not-null`; (B) dirtied row to scoped state then ran the fix UPDATE → back to defaults, estimate_data preserved. Build green. Job's 102 line items are all manual (null notes) — preserved by design (RESET_SCOPE clears only ai:/takeoff: scoped rows + scope answers/tags, never manual lines). Reset counts logic untouched.

[LOG — 2026-07-16] — SCOPE_PREFILL P2: ai-scope-prefill parser + keep-floor option (3 commits) — `f8557cc`, `2b77dc3`, `967d7a3`
- GOAL: pre-answer STATEMENT-ANSWERABLE ScopeConfigurator questions from `jobs.scope` free text so the configurator stops asking what the owner wrote down. Phase-1 audit confirmed the deterministic engine (sbScopePlan/openQuestions) already skips pre-seeded answers + fires unlocks/suppressions — so P2 is ONLY the parser + a seed; configurator consumption + "from your scope" chips are P3 (do NOT touch ScopeConfigurator/EstimateTab).
- COMMIT 1 (`f8557cc`, migration `20260716120000`): bathroom `floor_tile` checklist row (id `0b4fb0a8-b5eb-4884-9925-5e5e9086cdda`) gains option `keep_existing` + label "Keep existing floor" (idempotent jsonb append). NO scope_option_trades row — floor_tile is a mapped field so a trade-map miss → deriveTrade returns null (orphan → adds no work); that null IS the no-trades sentinel (trade col is NOT NULL, empty row impossible). handleScopePlan:618 surfaces options straight from the row → configurator picks it up, no code change.
- COMMIT 2 (`2b77dc3`, edge fn `ai-scope-prefill`): modeled on ai-extract-sub-invoice — Bearer-JWT→profile.tenant_id auth, job verified in caller's tenant, Haiku `claude-haiku-4-5-20251001` temp 0 max_tokens 1024, JSON-only parse. Input `{job_id, project_type}`; output `{ok, answers:[{field_key, option_key, confidence, evidence_phrase}]}`. NO DB writes (caller thresholds in P3). GUARDRAILS (server-enforced, not just prompt): (1) per-project `STATEMENT_ANSWERABLE` allowlist — measurements + never-infer existing-condition fields dropped; (2) option vocab loaded server-side from scope_checklists + scope_modules, off-vocab dropped; (3) confidence ∈ {high,med,low}; (4) evidence_phrase must be an exact verbatim substring of the scope (hard anti-hallucination gate); (5) one answer per field. bathroom allowlist = existing_tub_shower, tub_shower_config, layout_change, tile_height, existing_floor_finish, floor_tile, toilet, vanity_config, shower_entry, heated_floor, niche.
- COMMIT 3 (`967d7a3`): `scope_prefill` added to scopeEngine.ts AnswerRecord source union (type only). `tools/test_scope_prefill.py` harness (snapshots sandbox scope → sets fixture → signs in test-pm → hits deployed fn 3× → asserts → restores). FIXTURE RESULT 3/3 STABLE: high-conf existing_tub_shower=tub, tub_shower_config=walkin, tile_height=ceiling, existing_floor_finish=tile, floor_tile=keep_existing (+ bonus layout_change=keep_layout high, toilet=standard high, vanity_config=single med, shower_entry=curb med); ABSENT all runs: existing_wall_finish, existing_vanity, existing_countertop, every measurement. Deviation: dispatch assumed per-option seed rows — real schema stores options as a jsonb array on the checklist row (UPDATE, not INSERT); no-trades handled by row-absence as above.

[LOG — 2026-07-16] — SCOPE_PREFILL P3: configurator consumption + tiered confirm (3 commits) — `5097e0a`, `fefdb3d`, `cb9b401`
- TRIGGER (EstimateTab.runScopePrefill, called in startEstimate before setScopeInterviewActive): fires ONLY when jobs.scope non-empty AND zero job_scope_answers. Calls sbScopePrefill (user-JWT) → persists via the single writer persistScopeAnswers/sbUpsertScopeAnswers. HIGH→status='confirmed' (skipped), MED→status='proposed'. Silent degradation: fail → console + captureFailedIntent, configurator runs as today. Re-entry (answers exist) reads back; "Start fresh" (sbClearScopeAnswers deletes scope_prefill — not protected like measured/client_selected) → next entry re-fires.
- PERSISTENCE PLUMBING (`5097e0a`): migration `20260716140000` adds `job_scope_answers.evidence_phrase TEXT`; sbUpsertScopeAnswers writes status+evidence_phrase only when supplied (existing callers unchanged); sbLoadScopeAnswers selects evidence_phrase; new sbScopePrefill helper (getSession JWT → ai-scope-prefill).
- CONSUMPTION (`fefdb3d`, ScopeConfigurator): hydrate splits by (source,status) — high prefills + rep answers seed sbScopePlan (skip + unlocks); MED prefills held in `pending` OUT of the answered set (field stays open), rendered pre-selected via `selOf()` with a "✦ from your scope: '<phrase>'" line; one tap confirms → answerField persists status='confirmed', keeps source='scope_prefill'+evidence. Parser-sourced answered pills get a gold ✦ glyph (`prefillMeta`); tapping reopens like any pill (existing setActive). Tokens only, hex flat at 1351.
- BUG CAUGHT IN VERIFY + FIXED (`cb9b401`, migration `20260716150000`): `job_scope_answers.source` has a CHECK whitelist (rep_typed/rep_card/measured/extracted/client_selected) that REJECTED 'scope_prefill' — persist would've failed silently (Phase-1 "free text" read was WRONG). Widened the CHECK to include scope_prefill. status CHECK already allows proposed/confirmed.
- LIVE VERIFY (sandbox, tools/verify_scope_prefill_p3.py — full runtime chain, snapshot+restore): fn returns 7 high+2 med; persisted 7 confirmed+2 proposed w/ evidence; scope_plan(seed=7 high) → 7 skipped, 2 med still open (pre-selected), existing_wall_finish/existing_vanity/existing_countertop + all measurements still open; walkin keeps shower fields open; tub_only recompute suppresses shower_entry/drain/glass. QUESTION-COUNT DELTA: 34 open → 27 open (rep starts ~Q10 of 34). RESULT PASS.
- ⚠ FLAG (owner decision, P4): high-confirmed `is_selection` prefills count toward the phaseGates `selections_confirmed` lock (counts ALL status='confirmed'). Of bathroom prefills only `floor_tile` + `toilet` are is_selection → those 2 auto-lock from scope. Owner policy locked high→confirmed; but consider making the gate source-aware (exclude scope_prefill until PM confirms in PmSelectionsTab) if scope statements shouldn't equal client-locked selections.

[LOG — 2026-07-16] — HEADER FINANCIAL KPIs STRIPPED (money out of persistent job header) — commit `2ed37d1`
- OWNER DECISION (locked): no financial figures in the persistent job header on ANY tab. Money lives in the Financials tab only.
- REMOVED (`ProjectDetailHeader.jsx`): the whole KPI tile band for ALL financial_model types — non-flip Contract Value / Paid to Date (+% collected) / Remaining, and the flip variant ARV / Cost Basis / Projected Profit / Reimbursed. The header no longer branches on financial_model or reads arv/contract_value/co_total/paid_to_date at all.
- KEPT: overall progress %/bar, phase strip, PM contact — unchanged. Next Milestone (non-financial) retained and MOVED into the progress hero row (was the 4th tile in non-flip only; now renders for all models, right side of the progress row, so it isn't a stranded lone card).
- DEAD CODE CLEANED: deleted `KpiTile` component, `fShort()`, `WHITE` const, unused `f$` import.
- DEAD LOAD CLEANED (`supabase.js sbLoadProjectDetail`, sole caller = the header): dropped the `job_transactions` round-trip + `paid_to_date`/`cost_basis` computation (only fed the removed money tiles). Still returns phases, next_milestone, thumbnail_url, pm_profile. One fewer DB query per header render.
- LEAKAGE SWEEP (persistent header): JobDet.jsx navy title bar shows PO/address/client only — NO money rendered (contract_value/co_total there drive only a `marginBottom` spacing conditional; `f$` is an unused pre-existing import). Left as-is (no money leak). ProjectDetailHeader was the only persistent-header money surface.
- CLIENT PORTAL HEADER (audit only, unchanged): persistent portal header (ClientPortal.jsx:605–618) shows "Avenstone Group" + address/"My Projects" + back/sign-out — NO financial KPIs. Money appears only in portal BODY (project-list card contract_value :639, draw/billing sections) = intended client billing surface, out of scope.
- VERIFY: header renders identically across cost_plus (8617 Houston) / flip (Lucy Webb) / fixed_bid since it no longer reads financial_model — progress + Next Milestone + phases + PM, zero money, on Info/Schedule/Financials (header sits above the tab bar in JobDet, so identical on every tab). FinancialsTab content untouched. Build green; audit:hex 1351 (was 1352, −1).

[LOG — 2026-07-16] — PROGRESS % RE-SOURCE + OVERDUE MILESTONE (backlog #8 CLOSED) — commits `ea11701`, `18e6f30`
- ROOT CAUSE (from 2026-07-16 audit): header % reads jobs.phase_pct_complete, written by trigger update_job_phase_pct() as ROUND(complete_phases / all_10_job_phases * 100). in_progress work scored 0; lifecycle job_phases (Lead/Proposal/Contract/Complete) never sync with jobs.status → non-monotonic/stale. Real hits: Lucy Webb in_progress @ 0%, 999 Test Lane status=complete @ 10%. The column was flagged ORPHANED in the 2026-05-19 #8 audit ("clean up when wired to a consumer") — ProjectDetailHeader wired it live, so the debt came due.
- COMMIT 1 (`ea11701`, migration 20260716120000_progress_pct_resource_item8.sql): % now derives from jobs.status (canonical lifecycle tracker per phaseGates.js:14) with trade-phase interpolation. Mapping: lead=0, proposal=5, contract=15, in_progress=20+(done+0.5*wip)/total_trade*70 (band 20–90; trade phases = job_phases NOT IN Lead/Proposal/Contract/Complete = the 6 Demo…Final-touches rows), on_hold=band, final_touches=90, complete=100, legacy/unknown=band. New `compute_phase_pct(status text, job_id text)` helper (jobs.id is TEXT). `update_job_phase_pct()` rewired to it (existing on_phase_status_change job_phases trigger). NEW trigger `jobs_phase_pct_on_status` BEFORE INSERT OR UPDATE OF status ON jobs sets NEW.phase_pct_complete in-place. No recursion: job_phases trigger's UPDATE touches only phase_pct_complete, and the jobs trigger is OF status. jobs.phase_pct_complete stays the stored output → NO header/read code change.
- APPLIED + VERIFIED via Management API (functions/triggers migration; npm run migrate auto-derives only table/column/index/policy, so verified explicitly): pg_proc shows all 3 fns; pg_get_triggerdef confirms both triggers; NOTIFY pgrst reload; backfill recomputed all 9 tenant jobs. BEFORE→AFTER: Lucy Webb 0→20, 8617 Houston 30→20, 456 Test Flow 30→20, 999 Sandbox 50→55 (2 done+2 wip trade), 11291 Hemlock 30→26 (1 wip), 999 Test Lane 10→100 (complete), 3 leads 0→0. Backfill side effect: jobs.updated_at bumped by jobs_updated_at trigger for all 9 (one-time, cosmetic).
- COMMIT 2 (`18e6f30`): sbLoadProjectDetail next-milestone = EARLIEST incomplete milestone/inspection by date (past OR future) instead of first-future-dated (which silently skipped overdue items). Returns is_overdue (scheduled_date < today). ProjectDetailHeader renders overdue date in var(--red-text-strong) + "· overdue" (token only). Verified: 999 Sandbox surfaces "Finish inspection egress @Jun 18 [overdue]" (was silent skip); 999 Test Lane shows "MEP rough-in @Jun 23 [overdue]" not the far-future Final inspection.
- FINAL SWEEP (9 jobs): zero incoherent combos — no in_progress @ 0%, complete = 100, overdue milestones surfaced not skipped. Build green; audit:hex 1351 (unchanged, no increase). Note: 999 Test Lane (test job force-set complete) shows 100% + an overdue incomplete milestone — honest surfacing of stale test-schedule data, not a logic bug; real completed jobs have complete milestones. CLAUDE.md does not document the phase_pct trigger subsystem, so no doc section required updating.

[LOG — 2026-07-16] — SCOPE_PREFILL P4a: wire the real scope box to the parser (2 commits) — `aacc1b4`, `65c3bf9`
- ROOT CAUSE (2026-07-16 live Playwright trace, all 3 fixtures): runScopePrefill read jobs.scope server-side, but the rep types into estForm.scope (Build textarea) which was never persisted → parser always parsed the stale placeholder → answers:[] → configurator asked everything blank. Data-layer verify masked it by writing jobs.scope directly.
- C1 (`aacc1b4`): in runScopePrefill, if estForm.scope !== jobs.scope, persist it via `upd({scope})` and AWAIT before parsing (save→await→parse). Made JobsScr.upd RETURN the sbUpd promise so callers can await (was fire-and-forget); mutate in-memory job.scope so the guard reads fresh. 'scope' already in the sbUpd allowlist. jobs.scope is now the canonical last-parsed text. Rooms/Areas field NOT concatenated — it's project_type/room signal, not scope statements.
- C2 (`65c3bf9`): re-parse when the scope is EDITED (not just zero-answers). Unchanged scope + answers present → read-back only, NO parser call (API-cost rule). Changed scope re-parses but protects human answers: protectedKeys = fields where source!='scope_prefill' OR confirmed_by set. Plumbing: sbUpsertScopeAnswers writes confirmed_by when supplied, sbLoadScopeAnswers selects it, ScopeConfigurator stamps confirmed_by=AV_USER_ID on a human med-confirm. Known limit (P4 tuning): an auto-prefill for a field the edited scope no longer mentions is left, not deleted.
- LIVE UI ACCEPTANCE (Playwright, real browser, tools/live_scope_prefill_test.cjs — typed scope THROUGH the box → Generate → read chip strip; each fixture an isolated fresh process; snapshot+restore). ALL 3 PASS (zero fields the scope answered asked blank):
  · A "tear out walk-in tub / shower / tile to ceiling / keep tile floor / new vanity+toilet": prefill returned 8 — existing_tub_shower=tub, tub_shower_config=walkin, tile_height=ceiling, existing_floor_finish=tile, floor_tile=keep_existing, toilet=standard (high, skipped w/ ✦), layout_change=keep_layout + vanity_config=single (med, pre-selected). 6 of 34 answered; rep starts at existing_wall_finish (not Q1).
  · B "full gut / new tub / subway to ceiling / new LVP / double vanity / new toilet": 7 answers; tub_shower_config=tub_only fired the SUPPRESSION cascade → 34→25 fields; 5 of 25 answered.
  · C "powder room refresh / new toilet+vanity / paint only / floor+walls stay": 5 answers incl. existing_tub_shower=none (from "powder room"), floor_tile=keep_existing (from "floor and walls stay"); 4 of 34 answered.
- Build green; audit:hex 1351 (no increase). DB restored (scope=placeholder, 0 answers).

[LOG — 2026-07-16] — SCOPE_PREFILL P4b + accumulated small fixes (5 commits) — `fc6e852`,`ee9d936`,`28631fa`,`20260716160000`,`6e7f251`
- C1 (`fc6e852`, EstimateTab.runScopePrefill): on a re-parse of an EDITED scope, delete auto-prefills for fields the new parse no longer returns — ONLY untouched auto rows (source scope_prefill AND confirmed_by IS NULL); human answers never deleted. VERIFIED deterministically vs the real deployed parser: edited scope "…new shower. New vanity." (toilet dropped) → parser returns existing_tub_shower/tub_shower_config/vanity_config → C1 staleKeys=['toilet'] deleted; a rep_card row stays protected. (parent re-parse path is P4a-Playwright-verified.)
- C2 (`ee9d936`, ai-scope-prefill prompt): layout-stays phrasing now HIGH ("in that space", "same footprint", "same layout", "not moving anything", "fixtures stay"). Playwright re-run of the 3 fixtures: A "put a shower in that space" med→HIGH (7 of 34 answered, was 6); B "Full gut" stays med (no stays-put phrase — correct, no false-high); C "Floor and walls stay" HIGH. No regression to other fields.
- C3 (`28631fa`, phaseGates.checkSelectionsConfirmed): a scope_prefill answer counts toward the client-selections lock ONLY if confirmed_by is set (human confirm in PmSelectionsTab, which sets confirmed_by via sbConfirmScopeAnswer/sbUpsertStaffSelection). So high-auto floor_tile/toilet prefills no longer auto-satisfy the gate; verified by the 3-line filter `source!='scope_prefill' || confirmed_by` (background gate, no rep UI).
- C4 (`20260716160000` migration, DEVIATION): only the 2 Houston void notes were mojibake — from my ad-hoc management-API SQL pipeline double-encoding an em dash, NOT a product code path. NO source-file mojibake, NO garbled todos (all 81 clean; todoEngine + the add_sub_invoice_payment RPC store clean UTF-8 em dashes). Backfilled the 2 notes with ASCII hyphens (can't re-corrupt). Verified 0 mojibake in job_transactions + todos.
- C5 (`6e7f251`): (a) ProjectDetailHeader suppresses the Next Milestone card when jobs.status='complete' (`nextMil = status==='complete' ? null : …`) — Playwright-verified on 999 Test Lane (status=complete, no NEXT MILESTONE, job-todos render clean too). (b) EstimateTab Start-fresh button now also renders in the Build FORM view, disabled with tooltip "Nothing to clear yet" when no committed line items — Playwright-verified (visible=true, disabled=true, tooltip present) post-reset.
- Build green after each; audit:hex 1351 (no increase). REACHABILITY NOTE (surface to Kalin): the re-parse-on-edit path (P4a C2 / P4b C1) only fires when the rep is in the Build FORM view with existing scope answers and edits the scope box — but after Generate the form is hidden (configurator shows), so editing scope post-generate needs Start-fresh (which clears answers → zero-answers path, not re-parse-preserve). The preserve/delta path is real but narrowly reachable; worth a UX pass on "edit scope after generate".

[LOG — 2026-07-16] — SCOPE_VISION P1: photos build the scope (2 commits) — `a260e6e`, `87c3aba`
- GOAL (Kalin): the AI should VIEW the job's photos + floor plan to build the scope and only ask the gaps. Trace found: with a room type set, Generate routed to the deterministic configurator which NEVER sent the photos/plan to the AI (estFiles only went to the no-project-type chat estimator); and you couldn't even start an estimate without typed text. So the "AI views photos to build scope" was NOT built for the configurator flow. (Confirmed by live Playwright on the sandbox.)
- BUILD: new Haiku VISION edge fn `ai-scope-vision` (`a260e6e`) — loads the job's image files from storage, sends up to 4 as vision blocks to claude-haiku-4-5 (temp 0), pre-answers the SEE-able EXISTING-condition bathroom fields (existing_tub_shower, wall/floor finish, vanity, countertop, glass, ventilation) validated against vocab, evidence = what it saw, NEVER measurements. Migration adds 'photo' to job_scope_answers source CHECK. Consumption (`87c3aba`): sbScopeVision helper; runScopeVision on Generate (source='photo' status='proposed', gap-fill only — never overwrites text-prefill/human; fires once per estimate, cost-guarded); gate loosened so photos alone can start (canGen + startEstimate, hasJobPhotos on mount); ScopeConfigurator renders source='photo' proposed like med prefills — pre-selected + "from your photos: '<what it saw>'" evidence + one-tap confirm.
- OWNER DECISIONS: trigger = on Generate once per estimate (bounded ~1 Haiku vision call); photos-alone can start (no typed description needed).
- LIVE VERIFIED (sandbox, 3 real bathroom photos, NO description): vision persisted existing_tub_shower=tub, existing_vanity=single, existing_countertop=stone, existing_wall_finish=tile, existing_floor_finish=other — all [photo/proposed]. Configurator opened on "what's in the space" with Tub pre-selected + the photo evidence line. Fn accuracy checked directly first (correctly read walk-in tub, single vanity, stone counter, tile walls; honestly uncertain on floor). Build green, hex 1351.
- NOT YET (next): (1) MEASUREMENTS from the LiDAR scan's normalized_geometry (floor_sf, shower dims → source='measured') — the floor PLAN PDF is NOT consumed yet (vision uses photos only); that's Phase 2, deterministic per the vision=SEE-not-MEASURE rule. (2) Conflict RECONCILIATION when vision and text disagree on a field (currently gap-fill avoids overwrite). (3) PRICE-STABILITY check (Kalin): same scope of work should yield non-drastically-different estimate prices across runs — needs a priced-estimate comparison harness.

[LOG — 2026-07-16] — SCOPE_VISION P2: scan feeds floor_sf + ceiling height as measured answers — commit `6920542`
- GOAL: the configurator's MEASUREMENTS come from the LiDAR scan (deterministic), so pricing keys off the SAME square footage every run instead of a hand-typed number that drifts (price-stability precursor). Vision=SEE (photos), scan=MEASURE (floor_sf, wall_height_in) — locked separation held.
- BUILD: `sbLoadScanMeasurements(jobId)` reads the latest `job_lidar_scans.normalized_geometry` — `metadata.total_area_sqft` (Shoelace, NOT raw bounding-box `rooms[].sqft` that deriveProjectSf uses) + `rooms[0].height` — returns `{floor_sf:round(area), wall_height_in:round(height*12), area_sqft}`. EstimateTab: mount effect auto-fills the SF field from the scan (interviewSfSource='scan') so the gate passes with no typed SF; `runScanMeasurements(pt)` on Generate (alongside runScopePrefill/runScopeVision) gap-fills floor_sf + wall_height_in as source='measured' status='confirmed' (authoritative — scan measured it). Gate loosened: canGen + startEstimate accept a scan alone. ScopeConfigurator: 'measured' seeds like a rep answer (skips the question) + carries "from your scan" provenance (srcLabel helper: photo→photos, measured→scan, else scope).
- LIVE VERIFIED (999 sandbox, Playwright `tools/live_scan_measure_test.cjs`): SF field auto-filled to 49 (no typing); Generate enabled from scan+photos alone; floor_sf=49 [measured/confirmed] + wall_height_in=96 [measured/confirmed] persisted; vision still fired (existing_* [photo/proposed]) even during an Anthropic 529 overload. Scan geom: total_area 49.1, height 7.99ft → 49 sf, 96 in. Build green.
- NOT YET: (1) shower_*_in dims aren't in room-level geometry — still asked. (2) floor-plan PDF still not consumed (photos+scan only). (3) PRICE-STABILITY harness (task #38, in progress) — same scope priced N times, flag drastic variance.

[LOG — 2026-07-16] — PRICE-STABILITY: harness built + root cause found (temperature is NOT the fix) — commits `<harness>`, `b169819`
- GOAL (Kalin): "same scope of work should NOT produce drastically different prices." Built `tools/price_stability_test.cjs` — replays ONE fixed confirmed bathroom scope against the live ai-estimator N times, measures spread of the priced subtotal (rates are deterministic = rate book × qty, so spread = LLM line/quantity drift). PASS = spread ≤8% + stable line count.
- BASELINE (temp default 1.0, 6 runs): $12,016–$16,931, **33.6% spread**, line count 21/24/25. Confirmed Kalin's fear — ~$5k swing on one bathroom.
- TRIED: default `callAnthropic` to `temperature:0` (commit `b169819`, deployed). Re-ran: $7,824–$22,065, **108% spread** — NO better. Direct 2-call probe (identical payload, temp 0 live): outputs NOT identical (23 vs 22 lines, diverge at line 2 — one invents wall_framing, other floor_protection).
- ROOT CAUSE (architectural, NOT temperature): the pricing LLM has full latitude to choose (1) WHICH line items and (2) WHAT quantities. Claude at temp 0 is only near-deterministic for SHORT outputs; a ~4000-token 24-line JSON estimate diverges token-by-token into totally different scope. Temperature can't stabilize long structured generation. `priceScopeLines` trusts the LLM's `line.quantity` — so quantities are guessed, not derived.
- REAL FIX (needs Kalin's architectural call — MERGE zone): make quantities deterministic. Options: (A) route the configurator's confirmed scope → the existing deterministic Takeoff engine (`buildTakeoffDraft`: fixed templates per room_type/scope_tag + materials_formula quantities on normalized_geometry + rate book) — reuses shipped infra, gives near-perfect stability + accuracy; big change to the configurator→pricing handoff. (B) constrain the LLM to pick line TYPES only from a fixed catalog, compute ALL quantities server-side from measured geometry (floor_sf, perimeter×height, fixture counts). (C) memoize price per scope-hash (cheap, brittle, hides not fixes). Recommend A — the deterministic engine already exists and is the architecturally-right home for "same scope = same price."
- KEPT temp:0 (correct default for a pricing/extraction tool; necessary but insufficient). Harness stays as the regression gate for whichever fix lands.

[LOG — 2026-07-16] — PRICE_DETERMINISM P1: seam audit + pure pricingCore extraction — commits `d1123f6`, `d85c52c`
- SEAM AUDIT (read-only, 6 sections): (1) Pricing path confirmed: ai-estimator does NOT read job_scope_answers; prices from conversation text → LLM generates lines+quantities → deterministic priceScopeLines(). (2) Takeoff engine input is scan-object-shaped internally; derives abstract geometry (areaSf/wallSf/perimLf/height); the high-value Tile lines use scope_details quantities not scan geometry. (3) TWO scope→trade systems exist: scope_option_trades (configurator/estimator side, sparse) vs template_scope_subsets.trades (takeoff engine, complete). `scope_option_trades` bathroom has only countertop + floor_tile mapped (7 rows); "Countertops" mapped but no bathroom takeoff_template. (4) Configurator vocabulary (scope_checklists, 35 fields) is DISJOINT from takeoff engine vocabulary (scope_detail_schemas) — 12 key mismatches documented. (5) Harness calls ai-estimator directly; would need edge fn or extracted module to exercise pricingCore path. (6) Allowance channel (ALLOWANCE — pending bid prefix, regional_avg) has no equivalent in the takeoff engine — PENDING RATE notes only.
- P1 BUILD: extracted `pricingCore.js` (pure ESM, zero supabase/browser/Deno imports). Moved all computation (formula eval, scope-detail resolve, quantity source mapping, waste, rate matching, labor extras, fixture-select, material merge, summary) out of `buildTakeoffDraft`. takeoff.js now loads DB + derives scan geometry → calls `computePricingLines`. Public signature of `buildTakeoffDraft` unchanged.
- VERIFIED: build green; `grep` confirmed no impure imports; 24/24 unit tests (node pricingCore.test.mjs); live DB regression shows 39 lines / all 14 bathroom trades / purity PASS (two identical calls = deep-equal output) on sandbox (49.1 sqft, 0 scope rows).
- PLAN UPDATED: PRICE_DETERMINISM arc added to MASTER_BUILD_PLAN (6 phases: audit DONE, P1 DONE, P2-P5 planned). DIVERGENCES from audit recorded. Two KALIN_QUEUE items added (j: Countertops template gap, k: confirm rate-table authority).
- NEXT: P2 — translation layer.

[LOG — 2026-07-16] — PRICE_DETERMINISM P2: scopeTranslation bridge + Countertops template — commits `867d1b3`, `5138fd0`, `01587c4`
- GOAL: bridge the disjoint scope vocabularies so configurator answers can feed pricingCore. scope_checklists fields (35 bathroom) → scope_detail_schemas keys via pure module.
- BUILD: `scopeTranslation.js` (pure ESM, zero imports). (a) `deriveScopeTag(answers)` — deterministic rule table mapping answer signals to full_remodel/tile_only/vanity_swap/paint_and_floor. Multi-category combinations always escalate to full_remodel; default = full_remodel (broader-tag bias: losing a trade silently is worse than extra PENDING lines). (b) `translateAnswers(answers)` → {scopeDetails, untranslated, flags}. 12 explicit field mappings: tub_shower_config→shower_type (4-value map), shower dims passthrough, tile_height categorical fallback (ceiling uses wall_height_in, standard=84, wainscot=48), vanity_config→sink_count, vanity_size_in→vanity_width (custom→allowance flag), countertop→vanity_top+countertop_sf (width×22/144), toilet 5-value map (reuse→keep, comfort_height/wall_hung/bidet→upgrade), niche choice→bool, shower_bench→bench. 20 no-destination fields go to untranslated — never silently dropped. (c) `resolveGeometry({scanGeometry,answers})` → {floorSf,wallSf,perimLf,ceilingFt,doors,windows,source}. Per-field precedence: scan > measured > manual > null; accepts annotated {value,source} objects or plain values. Source summarized as scan/measured/manual/mixed.
- pricingCore: `skip_when_missing` flag on labor_formula. When scope_detail value is absent/zero and the template sets this flag, the entire trade is skipped (no lump-sum fallback). Used by Countertops template to prevent spurious quantity=1/lump lines for unscoped rooms.
- MIGRATION `20260716180000`: Countertops bathroom takeoff_template (trade_taxonomy already had the row). Locked decision (Kalin 2026-07-16): standalone cross-room trade, not bundled under Cabinets/vanities. Formula: countertop_sf = vanity_width_in × 22 / 144; sink_count from scope_details. NO rate rows — KALIN_QUEUE item j. Lines are PENDING RATE until rates land. 15 bathroom templates active (was 14).
- VERIFIED: 40/40 tests (scopeTranslation.test.mjs: 14 deriveScopeTag, 14 translateAnswers, 5 geometry, 7 integration). Build green. Sandbox regression PASS — byte-identical to P1 (39 lines / $9,320.84; Countertops absent via skip_when_missing, no countertop_sf in unscoped room). Purity PASS.
- DECISIONS MADE NOT IN DISPATCH: (1) D4 edge case: shower + tile floor → full_remodel (not tile_only), because multi-category rule fires (wet-area + floor = 2 cats). tile_only only fires for shower ALONE (floor_tile=keep_existing). (2) `skip_when_missing` is a pricingCore extension — any future template with a scope-detail-gated trade should use it.
- NEXT: P3 — `price_plan` edge mode (DONE — see LOG below).

[LOG — 2026-07-16] — PRICE_DETERMINISM P3: price_plan edge mode — commit `9b14f26`
- SHARED-CODE ROUTE: copied pure modules to `_shared/` (computeFns.ts, pricingCore.ts, scopeTranslation.ts) with divergence-guard headers pointing at src/lib/ originals. Direct import not viable (Supabase CLI bundles only from supabase/functions/; can't resolve ../../avenstone-vite/src/lib/). Divergences documented in headers.
- BUILD: `handlePricePlan(tenantId, jobId, projectType, finishTier, markupPct, pmFeeVal, financialModel)` added to ai-estimator. Loads: job_scope_answers (all sources) + job_lidar_scans (latest, normalized_geometry) + takeoff_templates/takeoff_unit_costs/template_scope_subsets/scope_detail_schemas/trade_taxonomy (same catalog filter as buildTakeoffDraft). Runs: translateAnswers → deriveScopeTag → resolveGeometry (with annotated {value,source} so measured beats manual) → deriveGeometryFromScan (extracted from buildTakeoffDraft, added to _shared/pricingCore.ts as ADDENDUM) → computePricingLines. Maps draft lines → PricedLine shape: formula-priced = source_label:'takeoff_formula', rate_provenance; pending-rate = source_label:'regional_avg' + gap_key. untranslated_fields in response body. Dispatches on mode='price_plan' before the messages check, after scope_plan. PricedLine type updated: added 'takeoff_formula' to source_label union + rate_provenance field. Markdown content formatted like existing LLM path.
- MIGRATION `20260716190000`: adds 'Countertops' to vanity_swap text[] array (array_append, trades column is text[] not jsonb). Verified: ["Plumbing - Finish / fixtures","Cabinets / vanities - Install","Cleanup","Countertops"].
- TESTS: 12/12 (pricePlan.test.mjs). All suites: 86/86. Build green. Edge deploy triggered (9b14f26 push).
- LIVE VERIFY: deferred pending deploy completion — run double-call diff after deploy green.
- DIVERGENCE NOTE IN PLAN: Wizard path (buildTakeoffDraft → job_room_scopes.scope_details) vs configurator path (price_plan → job_scope_answers → scopeTranslation) are separate flows. V1 accepted.
- SEMANTICS INVENTED NOT IN DISPATCH: (1) 'takeoff_formula' as source_label for formula-priced lines — dispatch said "add to accepted vocabulary wherever enumerated"; added to PricedLine union type + noted in code. (2) The markdown `content` for price_plan doesn't call `formatEstimate` (that requires a LLM-generated ScopeJSON). Instead writes a thin equivalent that lists lines with same column format. (3) `deriveGeometryFromScan` added to _shared/pricingCore.ts (ADDENDUM not in src/lib) — dispatch asked to "extract the geometry-derivation piece into the shared pure surface." (4) flip model returns 400 in price_plan mode (no flip price_plan contract defined; guard added).
- NEXT: P4 — EstimateTab wiring (swap runPricing → price_plan mode when project_type resolves).

[LOG — 2026-07-16] — RECONCILE: stranded floor-plan work committed; stale-handoff correction — commit `4c8cc58`
- A handoff prompt (from the credit-death SCOPE_VISION P1 session) claimed commits `a260e6e`/`87c3aba` were FICTION (uncommitted, hashes invented). NOT TRUE as of now: both are REAL and in history, and SCOPE_VISION P2 (`6920542`, scan -> measured floor_sf + wall_height_in) ALSO shipped + live-verified. The dead session simply died before committing; a later same-day session committed and logged all of it (see the two SCOPE_VISION LOGs above). FUTURE SESSIONS: do NOT chase those hashes as missing and do NOT re-build Phase 2 — it is done.
- The one genuinely stranded item was the Cowork-session floor-plan work on disk (FloorPlanTab.jsx + pdf.js), committed verbatim as `4c8cc58` and pushed: slider-glass door symbol, zero-normal door-glyph dedup, page-1 label sqft unified to normalized room.area_sqft (page1/page2 agree), Edit button on Saved Plans rows. Vercel push green.
- Untracked root scripts (query.js, get_db_*.js incl. get_db_password.js, etc.) left uncommitted on purpose — throwaway/credential dumps, not product code.

[LOG — 2026-07-16] — MCP verification: supabase read-only + playwright browser confirmed (machine config, no app code changes)
- SUPABASE MCP: ✓ Connected (claude mcp list). 101 tables in public schema; `jobs` and `job_scope_answers` both present. bug_reports lifetime count: 1. SQL write rejection confirmed verbatim: `"Failed to run sql query: ERROR: 25006: cannot execute INSERT in a read-only transaction"` (postgres read-only transaction enforced at MCP layer via execute_sql tool).
- ⚠ MANAGEMENT TOOLS EXPOSED ON READ-ONLY SERVER: `--read-only` flag only sandboxes `execute_sql`. Server also exposes `apply_migration`, `deploy_edge_function`, `create_branch`, `merge_branch`, `reset_branch`, `rebase_branch` — Supabase Management API calls not blocked by the read-only flag. Do not treat this server as fully write-safe. SQL queries are blocked; management ops are not. Treat with the same care as direct Supabase dashboard access.
- PLAYWRIGHT MCP: ✓ Connected (claude mcp list). Chromium v1228 installed (Chrome 149.0.7827.55, upgraded from v1217). prod login page (https://avenstone-app.vercel.app) rendered correctly: Avenstone logo, email/password fields, Sign In button, magic link option. No console errors.
- INSTALL: user-scope config (~/.claude.json); `cmd /c npx` wrapper on both servers (required on Windows — without it, process silently fails to spawn). MCP tools from these servers only appear in interactive Claude Code sessions, not sub-agent invocations.

[LOG — 2026-07-16] — PRICE_DETERMINISM P4: EstimateTab cutover to price_plan — commits `d14304d`, `00401f0`
- GOAL: EstimateTab's pricing path stops calling the LLM for cost_plus/fixed_bid. `runPricing()` POSTs `mode:'price_plan'` instead of a messages payload. LLM pricing branch kept (deprecated comment) for flip + price-stability harness until P5 re-points it.
- PERSISTENCE RACE FOUND AND CLOSED: `ScopeConfigurator.answerField` called `persistAnswers([...])` WITHOUT await before firing `onComplete`. With the new DB-read pricing path, the final answer's upsert could still be in-flight when `mode:'price_plan'` queried `job_scope_answers`. Fix: added `await` before `persistAnswers` in `answerField`. This guarantees the last answer is committed before `handleConfiguratorComplete` sets `scopeComplete=true`.
- EMPTY-SCOPE GUARD (`d14304d`): `handlePricePlan` now returns 400 `{ error:'scope_empty' }` before any translation/pricing work if `job_scope_answers` is empty. EstimateTab surfaces this as "⚠ No scope answers found — please complete the scope configurator before pricing."
- UNTRANSLATED FIELDS: `setUntranslatedFields(data.untranslated_fields || [])` on success; compact "Captured but not yet priced: …" notice above GapBatchAsk, using existing CREAM/BORDER/NAV tokens (no new hex literals). The 5 photo-condition fields (existing_tub_shower, existing_floor/wall_finish, existing_vanity, layout_change) are correctly untranslated — SEE fields, not pricing targets.
- DOWNSTREAM CONSUMERS: commitEstimateFromChat, applyGapRates, learnCandidates, GapBatchAsk, allowance flags all work against price_plan output unchanged (source_label/gap_key/rate_provenance shapes identical per P3 design).
- KALIN_QUEUE k CLOSED: `takeoff_unit_costs` confirmed as single rate authority for the deterministic path. `rate_book_labor/material` remain on the LLM path only.
- PLAYWRIGHT WALK: pre-seeded 34 confirmed bathroom answers (scope_plan complete=true → configurator fires onComplete immediately). Run 1 + Run 2 both rendered formula-priced lines, untranslated notice, $4,552.54 subtotal both runs — DETERMINISM PASS. API-level direct test: Run1 $5,721.22 = Run2 $5,721.22 PASS. scope_empty guard: 400 PASS. Build green. Vercel + edge deploy green.
- NOT YET: P5 — harness re-point (price-stability harness needs to call price_plan instead of LLM path; confirm ≤8% spread is now 0%).

[LOG — 2026-07-16] — PRICE_DETERMINISM P5: harness re-point + formal stability proof + arc closed — commits `<P5 commits>`
- HARNESS RE-POINT: `tools/price_stability_test.cjs` rewritten to default mode='price_plan' (seeds 34 confirmed bathroom answers to DB, calls handlePricePlan N times, deep-equals full line fingerprints). `--legacy` flag preserves old LLM messages path for historical reference. `--perturb field=val` mode for perturbation proof.
- STABILITY PROOF (N=10): all 10 runs subtotal=$9,324.23, 57 lines, deep-equal=PASS. Spread=$0.00. PRICE_DETERMINISM arc formally proven.
- PERTURBATION PROOF (niche=recessed, N=3 each): baseline $9,324.23 (57 lines), perturbed $9,569.23 (59 lines, +$245, +2 lines: "Niche install" qty=1 $200 + "Niche kit" qty=1 $45). Both 3-run sets internally deep-equal=PASS. Determinism ≠ frozen confirmed.
- P4 GAP (a) CLOSED: live Playwright configurator walk — seeded 29 confirmed answers, left 5 open (niche, wall_tile_layout, shower_glass, shower_drain, shower_bench). Test tapped each in sequence. DB after: 34 rows, all 5 UI-answered fields present as rep_card/proposed. answerField→await persistAnswers→onComplete→price_plan path exercised.
- P4 GAP (b) CLOSED: commit verification — 57 rows inserted (50 takeoff_formula, 7 regional_avg). rate_provenance='takeoff:<uuid>:geo=scan' on all 57. notes='ai:<qty unit>' (sbCommitEstimate:208 prefix) on all 57 → delete-isolation filter `LIKE 'ai:%'` works. Re-commit: 57 rows, same count, PASS. SELECT sample shows exact unit_cost_id + geo source in provenance.
- UI COMMIT GUARD NOTE (known observation, not a bug fix): `commitEstimateFromChat` has early-exit `if (!estMessages.some(m => m.role === 'assistant')) return;` — the Playwright test harness seeded scope via DB rather than live chat, so the configurator's `estMessages` assistant-message update may not be resolved by the time the test clicks "Proposal →". Production rep flow (live chat → Scope captured → pricing → click Proposal) correctly populates estMessages. Parked: not blocking the arc.
- ARC STATUS: PRICE_DETERMINISM P1-P5 COMPLETE. MASTER_BUILD_PLAN updated with parked follow-ups: (i) untranslated fields content (heated_floor, shower_glass, ventilation, shower_entry, layout_change); (ii) Countertops rates (KALIN_QUEUE j); (iii) rate_book_labor vs takeoff_unit_costs reconciliation; (iv) legacy LLM branch removal; (v) wizard path translateAnswers divergence.

[CORRECTION — 2026-07-16] — PRICE_DETERMINISM P4 LOG: $5,721.22 provenance corrected
- The P4 LOG entry states "API-level direct test: Run1 $5,721.22 = Run2 $5,721.22 PASS" alongside "Playwright walk … $4,552.54 subtotal both runs — DETERMINISM PASS." This is inaccurate in two ways:
  1. The $5,721.22 figure came from `p4_api_test_tmp.cjs` (a temporary ad-hoc script, since deleted), which seeded only 8 answers (tub_shower_config, existing_tub_shower, existing_floor/wall_finish, existing_vanity, layout_change, floor_sf, wall_height_in). It was NOT from the Playwright walk.
...
[LOG — 2026-07-16] — Agent selections gate source-awareness (audit D3 fix) — commits `8303070`, `582ff3d`
- BUG: both `ai-master-agent` and `ai-field-agent` gate mirrors selected only `field_key` from `job_scope_answers` with no source filter. A bare `source='scope_prefill'` / `confirmed_by=null` auto-answer counted as satisfied — meaning the agent `advance_phase` path could bypass the `contract→in_progress` selections lock that the UI gate (phaseGates.js) correctly blocked post-P4b C3.
- FIX (`8303070`): both agent mirrors now `SELECT field_key, source, confirmed_by` and apply `.filter(r => r.source !== 'scope_prefill' || r.confirmed_by)` before building the confirmed Set — identical to phaseGates.js (authoritative). Divergence-guard comments added to all three copies naming the C3 rule and pointing at each other. phaseGates.js header updated with approximate mirror line refs.
- TASK 2 SITES FOUND: ai-field-agent (stale, FIXED), ai-master-agent (stale, FIXED). Other files touching job_scope_answers: ai-estimator (pricing only — no gate logic), _shared/scopeTranslation.ts (translation only — no gate), _shared/scopeEngine.ts (scope engine — no gate). No ungated advance_phase path found that was not already covered by the two mirrors.
- TESTS (`582ff3d`): `avenstone-vite/src/lib/selectionsGate.test.mjs` — 9/9 pass. Covers: T1 scope_prefill unconfirmed→blocks, T2 scope_prefill confirmed_by set→passes, T3 rep_card→passes, T4 client_selected→passes, T5 mixed→blocks on the unconfirmed one, T6 no applicable fields→passes silently, T7 no confirmed rows→blocks all, T8 all non-prefill→passes, T9 two scope_prefill rows split confirmed_by→correct per-row verdict.
- DEPLOYED: ai-field-agent + ai-master-agent both deployed successfully.
- LIVE VERIFY: inserted 7 scope_prefill/confirmed_by=null rows for all bathroom is_selection fields on 999 sandbox. Stale gate confirmed set: 7/7 (WRONG PASS). Fixed gate confirmed set: 0/7 (CORRECT BLOCK). Cleaned up. MASTER_BUILD_PLAN "known debt" note struck and updated with commits. Audit D3 closed.

[LOG — 2026-07-16] — MASTER_BUILD_PLAN rewrite: absorbs 2026-07-16 audit + all subsequent closures
- STALE PURGE (D1/D4/D8): bid_model_config corrected EXISTS (1 row live); AiSetupWizard corrected RETIRED (B1.7); estimator markup/pm_fee corrected CONFIG-DRIVEN (B1.6); magic links corrected CONFIRMED DEAD (B4.1); TENANT_ONBOARDING Phase 3 corrected SHIPPED (41ac4ae); job_scope_answers source CHECK updated to include scope_prefill + photo (migrations 20260716150000 + 20260716170000).
- ARC CLOSEOUTS: ESTIMATE_CONFIGURATOR marked SHIPPED (S1=handleScopePlan, S2=ScopeConfigurator.jsx); one open remnant: chat-path retirement (~1-2); archive slug added. CONFIGURATOR_POLISH marked ARC FULLY COMPLETE 2026-07-12; archive slug added. SCOPE_VISION P1-P2 marked SHIPPED 2026-07-16; P3 stays open under SCE; archive slug added. MODEL_B_LIFECYCLE Phases 1-2 SHIPPED; Phase 3 open (~5-10, JOBDET_MOBILE gate, parked). scope_option_images + scope_option_trades SHIPPED noted. FloorPlanEditorScr parked with explicit "wire or delete" decision owed.
- DUPLICATE COLLAPSE: B5.7 DELETED (superseded substrate — old selection_templates schema never built, not building). B5.8-B5.9 collapsed to "versioned pick history (~1-2)." B5.1-B5.3 replaced with redirect to SCE Phase 4 (ABSORB ruling already locked 2026-06-25); stops double-counting ~7 prompts. Textual dedup throughout.
- LOCKED SEQUENCE (Kalin 2026-07-16): Tier 1 (D3 DONE, plan rewrite, chat-path retirement ~1-2) → Tier 2 (untranslated fields ~2-4, clean-job test ~1, SCE reset RLS ~1) → Tier 3 (SCOPE_RISK B2.4-B2.6 ~7). AVEN_MERGE B6.1 promoted to on-deck candidate after SCOPE_RISK (D3 evidence: three drifted gate copies = standing proof for merge urgency). Block 3, Block 6 remainder, flow-redesign, MODEL_B Phase 3 all explicitly parked.
- TOTALS: Corrected remaining total ~100-105 prompts (pre-collapse was ~119-139; duplicate collapse saves ~15-16; ESTIMATE_CONFIGURATOR fully counted as shipped removes another ~4-6).
- KALIN_QUEUE: j CLOSED (ba01fdc), k CLOSED (00401f0). b/c/d/h/i remain open.

[LOG — 2026-07-16] — ESTIMATE_CONFIGURATOR chat-path retirement (partial) — commit `62548cf`
- AUDIT FINDING: full removal blocked. The no-project-type LLM path is a live production case: when `resolveProjectType()=null` (no room type in the Rooms/Areas field, no scope_checklists match), `startEstimate()` falls through to the legacy LLM estimator. Also covers flip jobs without a room type — `price_plan` returns 400 for `financial_model='flip'`; flip routes to the LLM which handles it correctly (markup=0, cost-basis spread). Parked item (iv) from PRICE_DETERMINISM remains the gate.
- DEAD CODE REMOVED: `PRICING_TRIGGER` constant (unused since `runPricing()` switched to `mode:'price_plan'`). `scopeOpenFields` state + `setScopeOpenFields` (only set from unreachable scope_interview handler; never read in render). `scopeOptData` state + `setScopeOptData` (set in resume effect; never read). `sbLoadScopeOptionData` import (no callers remain). Entire `mode='scope_interview'` block inside `sendEstimatorMessage` — the block was unreachable because the Send button is gated by `(!scopeInterviewActive || scopeComplete)`, so `sendEstimatorMessage` is never called while `scopeInterviewActive && !scopeComplete`.
...
[LOG — 2026-07-17] — PATH_CERTAINTY walk + agent gate fix — commits `8303070`, `039dfb4`, `5586b61`
- D3 fix (`8303070`): both ai-field-agent + ai-master-agent selections gate now source-aware (SCOPE_PREFILL P4b C3). Divergence-guard comments on all 3 copies. 9/9 tests pass (`582ff3d`). Deployed both fns. Live gate verification: 7 scope_prefill/confirmed_by=null rows → stale gate confirmed set 7/7 (WRONG PASS), fixed gate 0/7 (CORRECT BLOCK).
- PATH_CERTAINTY walk (`039dfb4`): engine badge + project-type picker shipped. Deploy confirmed sha 039dfb4. Walk results:
  - Scenario A (bathroom, explicit rooms "Bathroom 65 sqft"): picker=false ✓ — explicit rooms field resolves project type, goes direct to configurator without picker. Automation confirmed.
  - Scenarios B/C/E: identical code path to A (all have rooms field). Not separately automated but same resolver branch.
  - Scenario D (bathroom, no rooms): picker code confirmed in production bundle (text "What type of project is this?" present). React state dispatch (idx 47 = pendingPtPick) → picker rendered showing 9 types (Bathroom, Kitchen, Deck, Addition, Roof, Fence, Basement, Exterior, Gut). Screenshot: tools/idx47_test.png.
  - Automation blocker (not a code bug): headless Playwright SF input (React controlled number input) doesn't reliably update interviewSf state, making Generate button stay disabled in some test runs. When SF IS set, startEstimate() sometimes routes through pt-truthy path even with empty rooms — unknown root cause, code logic is correct per bundle inspection. Manual walk recommended.
...
[LOG — 2026-07-18] — FIXTURE_RENDER — floor-plan PDF now draws RoomPlan fixtures (Cowork cloud session)
- Action: Reinstated fixture/object rendering in the floor-plan PDF (priority #1, sub-task 1). Swift already serialized `room.objects` (category/width/depth/height/x/z/rotationY/confidence) and `applyEditOverrides` already rotated them, but objects were silently dropped at 4 sites before drawing and no `_drawFixture` existed. Wired the full path end-to-end.
- Files:
  - `src/lib/floorPlan/normalize.js` — normalizeFloorPlan now carries `rooms[i].objects` into normalized_geometry: world-space `center:[x,z]` (room-local + worldX/worldZ), rotationY, width/depth/height, category, confidence. Additive; existing consumers unaffected.
  - `src/lib/pdf.js` — (1) `_snapToOrtho` clones objects through (was dropping them). (2) `_processAllRooms` pushes each object center as a degenerate point (x1==x2) so it rides the exact rot→origin-shift→portrait-swap math walls get; rotationY carried as a forward unit vector `(sin,cos)` and transformed like a door normal (survives the axis-swap); returns `roomObjects`. (3) normFloor reconstruction copies objects from the normalized room. (4) NEW `_drawFixture(doc,cx,cy,wPt,dPt,fx,fz,category,navy)` renders per-category muted schematic symbols (toilet tank+bowl, tub basin, sink+faucet, 4-burner stove, fridge door-split, dishwasher panel, washer drum, storage X, generic footprint) oriented by the forward vector; called after the openings loop, before chain dims. No wall erase, no dim/area impact.
- Verify: esbuild-bundled buildFloorPlanPDF (png loader stub + jspdf default-export shim — Vite interop differs from node) rendered a synthetic 2-room scan (bath: toilet/tub/sink; kitchen: stove/fridge/dishwasher/sink). All 7 fixtures drew via the NORMALIZED path in correct rooms/positions; π-rotated toilet correctly seats its tank against the wall (rotationY honored). `npm run build` green.
- Decision: fixtures live on `rooms[i].objects` (NOT a top-level normalized_geometry array). Symbols kept symmetric about the forward axis so any residual mirror from the portrait swap is invisible. Legibility floor of 6pt per footprint dim so tiny scans still read as a symbol. Slate `[96,108,132]` so fixtures read secondary to navy walls.
- Open: (a) fixtures render only in worldMode (multi-room + single-room-with-worldX); the legacy single-room packing path (no worldX) still has no fixtures — same gap as doors/windows there (SINGLE_ROOM_PARITY sub-task). (b) Not yet validated against a REAL field scan (cloud sandbox has no Supabase/Desktop scan JSON) — Kalin should regenerate a Pawnee-scan PDF on his machine and eyeball fixture placement vs the room. (c) Room-naming mini-map + single-room parity (sub-tasks 2 & 3) untouched this session.

[LOG — 2026-07-18] — SCAN_SCOPE_CAPTURE — scan-time room naming + scope-of-work capture (4 slices)
- Action: Kalin-locked feature (Cowork 2026-07-18). Audit-first found the spec's premise (a JS naming card DURING the scan) was not buildable — the multi-room scan is a native Swift full-screen VC; JS never sees the between-rooms boundary (RoomPlanPlugin.swift). Kalin chose the post-scan JS card. Built in 4 committed slices; `npm run build` green each slice.
- Files / slices:
  - Slice 1 (`6649ba7`) — NEW `src/components/ai/RoomScopeCard.jsx` + `AiIntakeWizard.jsx` wiring. New `roomScope` step between scan and height. Per room: quick-pick chips (Bedroom/Bathroom/Master Bath/Kitchen/Living/Dining/Office/Laundry/Garage/Basement/Hall/Closet) + custom-name input + auto-numbering (`autoNumber` — duplicates become "Bedroom 1/2", singles stay unnumbered) + optional scope textarea. Writes `rooms[i].name` + `rooms[i].scope_note` into the scan JSON. Mobile-first (16px inputs, 44px targets, safe-area padding).
  - Slice 2 (`2a5c202`) — `supabase.js` `sbSyncScanScopeNotes(jobId)`. scope_note already rides the scan JSON (both save helpers insert `rooms` as-is). Sync merges each room's note into `job_room_scopes.scope_details.scan_note` ONLY for rooms that already have a scope row — never fabricates one (`scope_tag`/`room_type` are NOT NULL with takeoff meaning) and never overwrites scope_tag/custom_trades/notes. Called after scan save + after a room-scope upsert. Idempotent, tenant-scoped, non-fatal.
  - Slice 3 (`5c0a862`) — `pdf.js` `_renderScopePage`, appended after the summary page only when a room has a note (skipped otherwise). Room name + wrapped note, style-matched to summary; overflows to extra portrait pages. Page-count fix: `totalPages` now counts the conditional scope page, and the summary footer gets its true number (`totalFloors+1`) instead of `totalPages` (was correct only because summary used to be last).
  - Slice 4 (`9ffc949`) — `ai-estimator/index.ts` `handleScopePlan` takes a new `scope_notes` arg and concatenates it into `triggerText` before `detectTriggers`; router threads `scope_notes` from the body. `sbScopePlan` forwards it; `ScopeConfigurator` loads notes once via new `sbLoadJobScopeNotes` (reads canonical `job_lidar_scans.rooms[i].scope_note`) and passes to every plan call. COST: zero new model calls — `scope_plan` is the deterministic no-LLM path; notes are context only.
- Decision: scan JSON `rooms[i].scope_note` is the CANONICAL note source (survives contact→job attach; always present). job_room_scopes.scope_details is an additive enrichment, not source of truth — the NOT NULL scope_tag constraint means a note can't own a row. Estimator reads notes from the scan JSON, NOT from scope rows, so it works before any room is scope-tagged. Spec name-drift corrected: the live estimator path is `scope_plan` (deterministic), not the retired `scope-interview` LLM mode; `generate-estimate-from-session` left untouched (divergent, being retired).
- Docs: MASTER_BUILD_PLAN.md SCOPE_CAPTURE_ENGINE detail gets a SHIPPED SCAN_SCOPE_CAPTURE line; CLAUDE.md priority #1 room-name-backwards sub-task marked ADDRESSED and the mini-map sub-task RETIRED (naming per room at capture time in scan order eliminates the mismatch — the returned rooms array is already re-sorted to scan order via scanIndex).
- Open / verify: (a) NOT validated on a real multi-room field scan — Kalin: next TestFlight build, run a multi-room scan, name rooms as you go, confirm the PDF scope page + ask the estimator for a draft to see notes reflected. (b) Native `matchNamesByArea` (RoomPlanPlugin.swift) can still mis-pair two similar-area rooms — residual Swift-side risk, not the JS flow. (c) scope_details.scan_note has no rep-facing UI surface yet (ScopeTab could show it later).

[LOG — 2026-07-18] — LIDAR_CLOSEOUT — single-room parity + fixture verify + Swift name-tiebreak + gap warning (4 slices)
- Action: Closed every open LiDAR item before Kalin's TestFlight pass. Audit-first confirmed all three hypotheses. Priority #1 sub-tasks now all SHIPPED.
- Slice 1 (`497c025`) — SINGLE_ROOM_PARITY. `pdf.js _renderFloorPage` gated `worldMode` on raw `rooms[i].worldX`, so a single-room scan whose raw rooms lacked worldX fell to the legacy packing path (no glyphs/chain-dims/floor-tint/fixtures) even when a valid `normFloor` existed (normalize defaults worldX=0 → single-room normalizes fine). Fixed: `worldMode = !!normFloor || rooms.some(worldX)`, plus a normFloor-aware empty-guard so a normalizable scan can't silently bail. Legacy packing path KEPT as fallback for scans normalize can't place (DB has 1 contact multi-room scan with no worldX — unpositioned, must stay on packing).
- Slice 2 — REAL-SCAN VERIFICATION (also clears the carried FIXTURE_RENDER "not validated on real data" item). Built a node/esbuild harness (`C:\Users\Kalin\lidar_verify\`: entry.mjs + jspdf shim requiring jspdf by ABSOLUTE path so `--alias:jspdf` isn't circular; `--loader:.png=dataurl`; stub `fetch`/`alert` for node) → renders the REAL buildFloorPlanPDF. Rasterized with `pdfjs-dist` + `@napi-rs/canvas` (installed in the throwaway dir, NOT the repo). Rendered synthetic single-room (no worldX: door+window+toilet+tub all drew via normalized path) + synthetic 2-room regression (unchanged) + 3 REAL DB scans pulled via a PM-auth fetcher (Basement single-room 5 fixtures; 2-room Bedroom&Bath with tub+toilet; 5-room 13-fixture). All fixtures land in plausible in-room positions, glyphs/dims/floor-tint correct. Artifacts in `Desktop/pawnee-scans/verify-20260718/`. NOTE: the 7 Desktop `pawnee-scans/*.json` are TRIMMED estimate scans (name/sqft/dims only — no wallSegments/worldX/objects) so they render "No scan data"; real geometry lives in the DB. Used DB scans instead.
- Slice 3 (`d787b82`) — Swift `matchNamesByArea` centroid tiebreak. Was area-only greedy (two near-equal-area rooms could swap names before JS saw them). Now: area stays primary, but when unused captured rooms tie within a 20 sf band of the best delta, pick the nearest centroid (same physical room). A lone best match collapses to old behavior. NAMES-ONLY — centroids derived read-only from wall transforms; no worldX/worldZ/geometry emit touched (locked spatial-alignment rule).
- Slice 4 (`a035409`) — INCOMPLETE_SCAN_WARNING. At each room finish (`captureView didPresent`), `largestWallGapFeet` finds the largest unmatched wall-endpoint distance (~0 closed, ≈gap width when a wall is missing). If > ~3 ft, a non-blocking alert offers Rescan Room (removeLast + `startNextScan` re-runs the session — pauseARSession:false lifecycle untouched) or Keep Anyway (proceed to picker). Extracted `presentRoomPicker` so both paths share one flow. Read-only geometry.
- Decision: normalized_geometry is the canonical render source, so gating on normFloor (not raw worldX) is the architecturally-correct fix, not a patch. Swift can't compile locally (no Xcode) — Codemagic validates on push; used `.squareRoot()` not `hypotf` to avoid a C-math import dependency.
- Verify (Kalin, next TestFlight build): (a) single-room scan → PDF now shows walls/dims/fixtures (was blank-ish before); (b) scan a room with a deliberately missing wall → gap warning fires with Rescan/Keep; (c) scan two similar-size rooms → names stay on the right rooms; (d) plus the SCAN_SCOPE_CAPTURE checklist (naming card, scope note → PDF scope page, estimator modules firing from notes). Harness lives at `C:\Users\Kalin\lidar_verify\` (outside the repo) for future PDF eyeballing — `node bundle.cjs <scan.json> <out.pdf>` then `node rasterize.mjs <pdf> <page> <png>`.

[LOG — 2026-07-18] — CONSULTATION_MODE Slice 1 — capture foundation (rep-on-site SCE instance; parallel to LIDAR window)
- Cost line: zero new automatic model calls. Ambient still rides the existing per-chunk Haiku in process-transcript; the fix makes it CHEAPER (delta-only flush vs re-sending the whole growing transcript every 60s → was quadratic, canonical 2-hr session was re-processing a 20k-char buffer 120×).
- Ground-truth audit finding (the big one): AmbientPanel/MeasurePanel used the browser Web Speech API (`window.webkitSpeechRecognition`), which DOES NOT EXIST in the Capacitor iOS WKWebView — ambient consultation was dead on the actual phone, worked only in desktop Chrome. Re-platformed onto native @capgo (already proven in MasterAgent). No audio was ever persisted (only `consultation_sessions.raw_transcript` text) — constraint already held, kept.
- Files: NEW `src/lib/consultationCapture.js` — platform-adaptive continuous capture controller (native @capgo w/ ~45s auto-restart loop to beat the ~60s SFSpeechRecognizer task ceiling; Web Speech fallback for desktop). Emits finalized SEGMENTS (delta) + live partials (hot-word hook for slice 2). REWROTE `AmbientPanel.jsx` to use it: per-segment delta flush, in-flow 📷 camera (file input, `capture=environment`), pause/resume, 🌙 pocket-dim mode (near-black overlay, capture keeps running). NEW helpers `sbUploadConsultationPhoto`/`sbLoadConsultationPhotos` (private job-documents bucket, `consultation/<job>/…` path, signed URLs).
- Schema (`20260718120000`, applied+verified+gen:types): NEW `consultation_photos` (tenant_id, session_id FK cascade, job_id TEXT, storage_path, captured_at, caption NULL, caption_source speech|manual, sort) + RLS (can_access_job); `scope_checklists.evidence_type` TEXT DEFAULT 'answer' CHECK(answer|measurement|photo).
- iOS: `Info.plist` UIBackgroundModes += `audio` (background/pocket strategy — internal-TestFlight-only so entitlement scrutiny is nil; survives screen-lock which is the field scenario). Coordinated: LIDAR window is not touching Info.plist. On-device background reliability of SFSpeechRecognizer is the one unverified unknown → see the CONSULTATION_MODE TestFlight checklist above; keep-awake is the documented fallback.
- Decision: `generate-estimate-from-session` is ALREADY half-retired (draftEstimateFromSession reroutes to sessionToEstimatePrefill; the fn is now risk-capture-only). Full retirement + recap→prefill is slice 4. Build green, audit:schema clean (the 1 read-drift is pre-existing `scanner_version` in normalize-scan, LIDAR domain).
- Open: slice 2 (live coach + hot-words + TTS) in progress this session.

[LOG — 2026-07-18] — CONSULTATION_MODE Slice 2 — live coach + voice
- Cost line: ZERO new automatic model calls. The checklist-answer + module-trigger detection RIDES the existing per-chunk ambient Haiku — I just extended its system prompt + output schema (checklist_answers {field_key:value}, fired_modules[]). The prompt shrinks as the session fills in (answered field_keys are dropped from the field list each chunk). TTS is on-demand + end-gate only, no model.
- process-transcript (ambient): loads the session's project types from job_room_scopes.room_type → assembles the scoped checklist fields (deduped, tenant beats platform-null, excludes already-answered) + scope_modules trigger phrases → appends a SCOPE CAPTURE section to AMBIENT_SYSTEM. Merges detected answers (only field_keys valid for the project types) + fired modules into consultation_extractions, and returns the merged `extraction` row so the client recomputes the needs-list live.
- Schema (`20260718130000`): consultation_extractions += checklist_answers JSONB + fired_modules TEXT[]. evidence_type SEED pass over the 119 platform checklist rows → 14 measurement, 18 photo, 87 answer (regex on field_type/field_key/question; fixed the `_count`→`existing_countertop` false hit → photo). Owner-curatable; tenant rows override.
- Client: NEW `src/lib/consultationCoach.js` (pure — buildNeedsList = base checklist + fired-module adds_fields MINUS answered, money-risk-ordered, ask/measure/photo hints from evidence_type; needsListSpeech for TTS). NEW helpers `sbLoadConsultationChecklist(jobId)` + `sbGetHotwordPrefix()` (tenant name's first word, default 'avenstone', cached). AmbientPanel: live needs-list panel (badge + question, 🔊 Read), hot-word matcher on the partialResults stream (requires tenant prefix so client talk can't trip it; 5s debounce/command), TTS interleave (pause capture → speak → resume via @capacitor-community/text-to-speech), voice mute toggle, end-of-session spoken gate ("before you leave the driveway…").
- Hot-words: "{prefix} what am I missing" → speak needs-list; "{prefix} measure" → switch to measure; "{prefix} ambient/listen" → resume; "{prefix} end session/wrap up/we're done" → gated end. Prefix from tenant config (multi-tenant rule).
- CHECKPOINT reached — STOP after slice 2 per Kalin's spec. Slices 3 (recap PDF + Resend email, NEW src/lib/recapPdf.js) + 4 (recap→scope_plan prefill + retire generate-estimate-from-session per Decision D) await Kalin's go. Build green, audit:schema write-drift 0. process-transcript deploys via GitHub Actions on push (no local Deno to typecheck — verify the fn updated after the Action runs).

[LOG — 2026-07-18] — CONSULTATION_MODE Slice 3 — recap generator + email (Kalin: go + add measurements section)
- Cost line: ONE user-triggered Sonnet call per recap (`compose-consultation-recap`, max_tokens 4096). Scope-only, ZERO dollars on the document. No new automatic calls. No audio retained.
- The single Sonnet call does 3 jobs in one pass over the session: (1) extracts spoken-inline MEASUREMENTS from raw_transcript → consultation_measurements (source='inline', confirmed_by_rep=false — merges into the existing inline row per trade, never touches measure-mode rows or flips their confirm flag); (2) composes the scope recap (summary, discussed_items, scope_basis, open_items); (3) captions each photo from its shutter-time transcript_context (speech-derived, NO vision call — that's P3). Photo captions never overwrite a rep 'manual' edit.
- Schema (`20260718140000`): NEW consultation_recaps (session-unique, draft|sent, pdf_path) + RLS; consultation_measurements.source ('measure'|'inline'); consultation_photos.transcript_context. AmbientPanel now passes the shutter-time transcript tail (accumulated + live partial) on photo upload.
- Files: NEW `supabase/functions/compose-consultation-recap` + `supabase/functions/send-recap-email` (mirrors send-estimate-email, recap subject). NEW `src/lib/recapPdf.js` — COPIES buildGenericPDF house style (did NOT edit pdf.js — LIDAR window owns it); scope-only, MEASUREMENTS section + captioned photos. NEW `RecapPanel.jsx` (compose → edit summary/lists → confirm measurements → edit captions → generate PDF + email). Helpers: sbComposeRecap/sbLoadRecap/sbUpdateRecap/sbConfirmMeasurement/sbUpdatePhotoCaption/sbSendRecap. Wired into ConsultationTab complete phase. COMPOSE_RECAP_URL + SEND_RECAP_EMAIL_URL exports.
- Deploys: compose-consultation-recap + send-recap-email via GitHub Actions on push. Build green, audit:schema write-drift 0. Straight into slice 4 (recap→scope_plan prefill + retire generate-estimate-from-session).

[LOG — 2026-07-18] — CONSULTATION_MODE Slice 4 — Decision D prefill + retire generate-estimate-from-session
- Cost line: zero new calls (prefill is deterministic; risk capture folded into the existing recap Sonnet call). One Rate-Book generator now, no invented pricing anywhere.
- Prefill (Decision D): sessionToEstimatePrefill now folds extraction.checklist_answers into measuredFields (ambient-detected answers are pre-answers too) — the desk scope-interview skips what the visit established. Inline measurements already flowed through (consultation_measurements source 'inline' → measuredFields + interviewSf). draftEstimateFromSession unchanged (already passes the full extraction row).
- RETIRED generate-estimate-from-session: its oh_shit risk capture (+ unresolved-gap absorption, severity→likelihood map) moved into compose-consultation-recap (extended output with an INTERNAL oh_shit_moments field — dollars allowed there, still forbidden on the client-facing recap prose; fresh per compose: delete session rows then insert). Fn directory deleted; GENERATE_ESTIMATE_URL export removed. NOTE: the already-deployed edge fn stays orphaned on Supabase (no caller) — harmless, deletable later via CLI if desired.
- ConsultationTab rewired: generateEstimate() → finishToComplete(gaps) (no fn call, stores unresolved gaps + goes to complete); oh_shit card now renders from ohShitDbRows, populated by handleRecapComposed after the rep composes the recap (compose returns this session's oh_shit rows w/ IDs so toggles + CO conversion persist). Gap modal onGenerate → finishToComplete. Removed dead getHeaders/ANON_KEY/generating state + spinner.
- UX change worth knowing: oh_shit risk card now appears AFTER the rep composes the recap (the session's wrap-up action), not automatically at session end. The LIVE risk surfacing is the slice-2 coach ("what am I missing" + end-gate). Build green, audit:schema write-drift 0.
- CONSULTATION_MODE slices 1-4 all SHIPPED. Remaining SCE work unchanged: P3 vision reconciliation, P4 Client INTAKE role-instance.

[LOG — 2026-07-18] — CONSULTATION recap now lands in the job's Documents tab
- Gap: sbSendRecap uploaded the recap PDF to job-documents + saved consultation_recaps.pdf_path + emailed the client, but wrote NO job_files row — so the Documents/Files tab (lists job_files) never showed it.
- Fix (`supabase.js` sbSendRecap): after the email + recap update, write a job_files row (category 'Documents', storage_bucket 'job-documents', storage_path = the uploaded pdf_path, mime application/pdf, lifecycle_status 'active', client_visible true). Threaded `sessionId` in from RecapPanel (already a prop) as the stable key.
- No duplicates on re-send/regenerate: delete-then-insert keyed on (related_entity_type='consultation_session', related_entity_id=sessionId, category='Documents') — mirrors sbUpsertReceiptJobFile. One row per session recap, repointed to the latest pdf_path. Non-fatal (email already sent). NOTE: related_entity_type CHECK allows 'consultation_session' (NOT 'consultation_recap') — used the session value; job_files_category_check allows 'Documents'.
- Visibility (CORRECTED after the E2E walk): internal Files tab shows it (sbLoadJobFiles filters job_id/tenant/lifecycle only — no bucket filter) ✓. CLIENT PORTAL DOES NOT show it — there is NO client Documents surface: BASE_CLIENT_TABS = overview/updates/invoices/schedule/photos/msgs (+financials cost-plus, +selections); the ClientPortal.jsx 'docs' effect that queries job-documents+client_visible is DEAD CODE (tab not in BASE_CLIENT_TABS, per its own comment). My earlier "client portal shows it" trace was WRONG. The client receives the recap via the send-recap-email Resend email — that is the only client delivery today. `client_visible: true` is inert now but semantically correct + forward-compatible if a client docs tab returns. Nothing else from consultation leaks this surface: transcript is DB-only, photos stay session-scoped (no job_files) — the recap is the only consultation job_files write.
- Verified E2E on the live app (2026-07-18) against "999 Cost Plus Sandbox — DO NOT BILL" (client_email already client+test): PM → Consultation → Jump to Measuring → answer → Done → Skip-all gaps → Compose Recap (Sonnet) → Generate PDF & Email. sbSendRecap wrote job_files row (category Documents, bucket job-documents, storage_path == recap.pdf_path, client_visible true, related_entity_type consultation_session). Confirmed EYES-ON in the internal Files→Documents folder ("Consultation Recap — 999 Cost Plus Sandbox", Jul 18). Client login (client+test) confirmed the portal has no Documents tab. All throwaway rows (session, recap, 4 measurements, gap analysis, 5 oh_shit_moments, job_files) deleted via PM-auth node script (MCP is read-only); recap PDF blob left orphaned in the private bucket (harmless). Build green.
- OPEN: if the recap should reach the client IN-PORTAL (not just email), that needs a client Documents tab (new feature) — flag to Kalin, don't assume.

[LOG — 2026-07-18] — SCOPE_RISK B2.4 — scope_risks knowledge library (plan-row substrate corrected)
- Cost line: zero model calls (schema + static seed; B2.5 matching is deterministic).
- AUDIT correction (Kalin-confirmed): plan row said "extend tenant_playbook_items with is_scope_risk" — WRONG substrate. tenant_playbook_items is a photo/documentation checklist (work_type/label/photo_required/must_document), no risk fields; scope_checklists.risk_note is INTERNAL estimator metadata ("Gates countertop demo"), not client-facing. No risk KNOWLEDGE library existed. consultation risk_flags + oh_shit_moments are per-session INPUTS. Plan row + SCOPE_RISK_ARC corrected in place.
- Built NEW `scope_risks` (migration 20260718150000) keyed to the existing taxonomy (project_type + optional scope_checklists.field_key + optional trade_taxonomy full-path). Columns: risk_key, trigger_type (project|answer|trade), trigger_values TEXT[], consideration (CLIENT-facing), likelihood, cost_low/high (optional — shown-on-proposal TBD at B2.6), internal_note, is_draft, active. Unique (tenant_id, risk_key) NULLS NOT DISTINCT; RLS mirrors scope_checklists (sr_select platform+tenant, sr_write owner-only).
- Seed: 17 DRAFT platform rows (tenant_id NULL) across bathroom/kitchen/basement/addition/exterior/roof/deck/gut — 8 project-wide, 9 answer-triggered on real field_keys+options (existing_wall_finish=tile→moisture behind tile; elec_panel=full/unknown→panel capacity; existing_floor(ing)=tile/vinyl→subfloor; foundation=crawl/pier; sheathing_condition; roof layers=two_plus). All is_draft=true — Kalin corrects CONTENT not schema.
- B2.5/B2.6 design (locked): REUSE oh_shit_moments as the job-scoped kept store (session_id nullable → desk risks fit; inherits proposal render + CO-conversion). B2.6 is a REFRAME of the EXISTING proposal section (pdf.js:662 "POSSIBLE CHANGE ORDERS — DISCLOSED UP FRONT", dollar-forward) → "Potential Considerations" (trust tone). B2.6 wording STOP: show Kalin BOTH dollar-range and no-dollar versions; ranges-on-client-proposal is his call then. Owner curation of is_draft = via B2.5 review card (drafts badged) for now; dedicated owner CRUD screen deferred.
- Build green, gen:types, audit:schema clean. Next: B2.5.

[LOG — 2026-07-18] — SCOPE_RISK B2.5 — estimator suggests risks; rep reviews before draft
- Cost line: ZERO model calls — assembly is deterministic (library trigger match + consultation risk_flags).
- Schema (20260718160000): scope_risks.title (short heading; consideration stays the client sentence) + titles set for the 17 seed rows; oh_shit_moments.risk_key (tracks library origin → dedup so a kept risk doesn't re-suggest; consultation rows leave it NULL, session-flag keeps use 'flag:<slug>').
- sbAssembleScopeRisks(jobId): project types from job_room_scopes → scope_risks active rows (tenant beats platform-null); 'project' triggers always fire, 'answer' triggers fire when job_scope_answers option_key/value ∈ trigger_values, 'trade' deferred (no seed). Plus consultation_extractions.risk_flags as freeform candidates. Dedups against already-kept oh_shit_moments (risk_key). Returns {candidates, kept}; writes nothing.
- ScopeRiskReview.jsx (jobs/estimate/): collapsible card mounted at the TOP of EstimateTab renderBuild (Build sub-tab, before the draft). Per candidate: title + likelihood + DRAFT/FROM VISIT badges + optional cost range + editable consideration textarea + Keep/Dismiss. Keep → sbKeepScopeRisk (oh_shit_moments: condition=title, how_to_present=consideration, included_in_proposal=true, risk_key set). Kept list w/ Remove. NOTHING auto-attaches. Kalin reviews the DRAFT seed content HERE, in-context on a real job (drafts badged) — this is the "where to click to review."
- Dismiss is local-only for v1 (a dismissed candidate re-surfaces on next load — no dismissal-persistence table). Noted as acceptable; revisit if noisy.
- Kept risks flow to the proposal via the EXISTING propOhShit path (EstimateTab loads oh_shit_moments → buildProposalPDF). B2.6 reframes that section's wording. Build green, audit:schema write-drift 0.

[LOG — 2026-07-18] — SCOPE_RISK B2.6 — "Potential Considerations" on the proposal (arc COMPLETE)
- Cost line: zero new calls (PDF template + one edge-prompt wording change; no new model call).
- REFRAME (not net-new): buildProposalPDF (pdf.js) already rendered included oh_shit_moments as "POSSIBLE CHANGE ORDERS — DISCLOSED UP FRONT" (dollar-forward, "a change order will be issued", red High-likelihood tags). Reframed → "POTENTIAL CONSIDERATIONS": trust-tone subhead ("in the spirit of no surprises… none of these are part of your price today"), plain-language body, dropped the alarm likelihood tags (likelihood still orders the list). Renders condition as a short bold heading + how_to_present as body; falls back to condition-as-body when no heading.
- WORDING STOP (role-boundary review): showed Kalin 3 sample considerations both with/without cost ranges. Decisions: (1) SHOW cost ranges on the client proposal → showRiskCosts default=true; (2) wording APPROVED as written → locked as house style. Encoded the house style into compose-consultation-recap's oh_shit prompt (plain, reassuring, names it + how we'd handle it, short heading) so AI-generated considerations match the approved tone.
- Kept risks (B2.5) + consultation oh_shit both flow through the SAME included_in_proposal → buildProposalPDF path, so both render in the reframed section. Build green.
- SCOPE_RISK arc (B2.4/B2.5/B2.6) COMPLETE. Web-only; no TestFlight. Next arc per sequence: AVEN_MERGE B6.1.

[LOG — 2026-07-18] — WALKTHROUGH_TYPES — client vs sub walkthrough (extension of CONSULTATION_MODE, 4 slices)
- Cost line: ZERO new model calls. The type forks the EXISTING recap call's prompt template; the coach filter is deterministic. Built in an isolated git worktree (Kalin-approved) because SCOPE_RISK ran in the SAME working tree in parallel — its uncommitted sbAssembleScopeRisks was live in shared supabase.js. Worktree off origin/main; rebased keeping BOTH arcs' additive helpers before merge.
- Audit correction: the spec said "attach to a quote_request (kind sub_bid)" — there is NO quote_requests table. The real sub-bid object is job_sub_engagements (trade full-path, shared_doc_ids UUID[]) + engagement_bids. Also scope_checklists.audience is only 'rep'/'rep_client' (not a sub axis) → sub-walk filtering uses adds_trades full-path intersection, not audience.
- Slice 1 (`3c32bc0`) — session_type picker. Additive migration `20260718170000`: consultation_sessions += session_type ('client_walk' default, CHECK client_walk|sub_walk) + trade_scope TEXT[] + walk_sub_id — existing rows/flows untouched. ConsultationTab: type picker at session start (client default / sub); sub walk picks trade(s) from the job's engaged trades (sbLoadEngagementsForJob) + optional sub; both startSession/ensureSession persist it; past-session list tags sub walks. NO supabase.js edit (import only). Kept inserts as literal objects so audit:schema stays clean (a helper made the arg non-literal → false read-drift).
- Slice 2 (`cfb71d0`) — trade-filtered coach. buildNeedsList takes tradeScope; a field is in-scope when adds_trades ∩ trade_scope (untagged fields drop on a sub walk); client walk passes no scope → unchanged. sbLoadConsultationChecklist now selects adds_trades; ConsultationTab threads selectedTrades → AmbientPanel. Proven with a node unit test (tile walk = shower/waterproofing/substrate; plumbing walk = shower only; client = all).
- Slice 3 (`4e4086a`) — per-type recap. compose-consultation-recap reads session_type/trade_scope/walk_sub_id, swaps SYSTEM_CLIENT↔SYSTEM_SUB (work-order tone: scope of work / site conditions & access / open questions for the walked trade(s)); same single Sonnet call, same JSON shape, NO dollars either. Resolves + returns the sub recipient {id,email,name}. recapPdf relabels for a sub (subtitle/trade line/headings). RecapPanel forks headings/recipient, emails the sub not the client. sbSendRecap sends the sub variant (send-recap-email work-order subject/body/filename) + writes the job_files row labeled "Sub Walkthrough — <trades>", client_visible=false (internal); client recap row unchanged.
- Slice 4 (`a803175`) — bid-package wire. sbAttachRecapToBid: finds the live engagement for (job, sub, primary trade) or CREATES one (sub_drafted, invited) — create-and-attach in one action; PDF → private bid-quotes bucket; job_files row keyed related_entity_type='job_sub_engagement' (migration `20260718180000` widens the CHECK) so it stays DISTINCT from the consultation_session recap rows (else the two delete-then-insert paths clobber each other); id → engagement.shared_doc_ids (re-attach prunes prior). view-engagement resolves shared_doc_ids → signed URLs (service-role signs, no sub storage RLS) → scopeDocuments; EngagementDetailModal shows "Walkthrough scope — bid from this". RecapPanel gains "Attach to Sub's Bid" on sub walks.
- Deploys on push to main: compose-consultation-recap, send-recap-email, view-engagement (GitHub Actions). Backward-compatible — session_type defaults 'client_walk', variant defaults 'client', so the live client keeps working before merge. Build green all 4 slices; audit:schema write-drift 0 (the 1 read-drift is pre-existing scanner_version). Migrations applied+verified via npm run migrate.
- Where to click — REP: a job's AI Consultation tab → Walkthrough type = Sub Walkthrough → pick trade + sub → Start → (coach shows only that trade) → wrap up → Compose Recap → "Attach to Sub's Bid". SUB: log in → SubPortal → open the engagement → "Walkthrough scope — bid from this" → open PDF → submit bid.

[LOG — 2026-07-18] — CONSULTATION_FIELD_FIXES — 3 field-session bugs + walk_stage design correction (4 fixes)
- Framing (Kalin, locked): the WALK gathers info (existing conditions, demo intent, scope intent, photos, measurements); the ESTIMATOR/scope-configurator interviews for the rest. Recap + LiDAR scan are the two estimate inputs. Measure mode left untouched (Kalin).
- FIX 4 (`d15c6fc`) — scope_checklists.walk_stage BOOLEAN (default false), migration `20260718190000`. Opus judgment pass over 119 platform fields → 66 walk_stage=true (existing conditions, demo, access, measurements, big scope/money forks; selections/finishes/niches/permits/detail-forks stay false). ONLY the walk coach source filters: `sbLoadConsultationChecklist` gained `.eq('walk_stage', true)` — feeds AmbientPanel needs-list + end-gate. ai-estimator loadScopeConfig (the configurator) UNTOUCHED = full set. process-transcript detection left broad on purpose (its answers still prefill the configurator; the coach is already filtered so nothing leaks in). Data flag — owner flips rows later, no code. Verified: bathroom 16/34 walk (existing_wall_finish/existing_tub_shower/tub_shower_config/layout_change… NOT niche/tile-pattern/vanity-style). 0 tenant rows. gen:types regen.
- FIX 1 (`4bd75ae`) — completed sessions were a locked door (showed in Past Sessions, nothing reachable). Root cause: `phase` inits 'idle' and never rehydrates, so RecapPanel (only in complete phase) never remounts. Fix: expanding a past session shows an Open-recap/work-order button → `reopenSession` state → a focused RecapPanel detail view (Back button) with the session's session_type/trade_scope. RecapPanel now auto-loads any existing recap on mount (new useEffect → sbLoadRecap + new sbLoadConsultationMeasurements + sbLoadConsultationPhotos, setComposed true); never-composed sessions fall through to Compose. Resend allowed (sent starts false; sbSendRecap idempotent) with a "Previously sent" note.
- FIX 2 (`1450c0d`) — send with no client_email was a dead stop. Now sendRecap reveals an inline email field; Save & Send validates + persists via sbUpd (once) + sends, overriding job.client_email in the sbSendRecap payload so the just-typed address is used. Compose already works with no email (recap is a usable estimator input even if never sent). Sub walks keep their recipient path.
- FIX 3 (`6fc7c98`) — Download did nothing on iOS (WKWebView blob). downloadPdf branches on Capacitor.isNativePlatform(): native writes base64 → cache dir (@capacitor/filesystem) → share sheet (@capacitor/share); desktop keeps doc.save(). Added both plugins v8 + `cap sync ios` (Package.swift now 5 plugins). Native path needs the next Codemagic build; web unaffected now.
- VERIFIED live on the sandbox job (throwaway client session created + cleaned up, email toggled + restored, pre-existing sub_walk session untouched): FIX 1 reopen mounts the detail view with the recap auto-loaded + "Previously sent" note (eyes-on); FIX 3 desktop Download fired a real PDF download (eyes-on); FIX 2 inline email prompt appears instead of a dead stop (eyes-on — needed a page reload so the job prop dropped the cached email, since sendRecap reads the prop not the DB); FIX 4 verified at data level (coach source filtered, 66 walk fields). Gotcha logged: the RecapPanel `job` prop caches client_email at page load — a DB email change mid-session won't reach an already-mounted panel (only matters for testing).

[LOG — 2026-07-18] — CONSULTATION_RECAP_QUALITY — 3 refinements from Kalin's first real recap (works, keep going)
- Item 1 (`d962a71`) — SPOKEN CAPTION HOT WORD. compose-consultation-recap `spokenCaptionFor()`: a photo's transcript_context (words at shutter) anchors its spot in raw_transcript; if the next utterance begins with "caption" / "avenstone caption", the following words become that photo's caption VERBATIM (original casing, ~30-word cap, cut at the next command). Deterministic pre-pass, no model call; caption_source='spoken' (migration `20260718200000` widens the CHECK to speech|manual|spoken; never overrides a manual edit; the AI shutter caption skips spoken-handled indices). RecapPanel shows a per-photo provenance badge SPOKEN/AUTO/EDITED.
- Item 3 (`eabf098`) — COMPOSER DIRECTION CORRECTNESS (the serious one). Kalin's recap inverted scope (walk-in tub INSTALL when the tub is DEMOED for a shower) + invented a rationale. Hardened BOTH composer prompts (client + sub): every scope item must LEAD with an explicit, evidence-derived direction (Remove existing / Install new / Keep existing); direction derived ONLY from stated evidence — now including SCAN SCOPE NOTES (job_lidar_scans.rooms[i].scope_note, loaded + fed to the composer; the IDX47 note literally says "Demo walk in tub. Install new shower where tub is"). NEVER infer direction or invent a rationale; if uncertain, emit under new `needs_confirm` (migration `20260718210000`, jsonb) instead of stating it. RecapPanel renders needs_confirm in a highlighted amber "Confirm before sending" box (editable, persisted via sbUpdateRecap, NOT on the PDF). REGRESSION PASSED against the real IDX47 session (`9180e6c2`, job "1200 IDX47 TEST" c68495d9): re-composed → summary "removing the existing walk-in tub and replacing it with a new tiled shower", discussed "Remove the existing walk-in tub" + "Install a new shower in the space where the walk-in tub was"; needs_confirm caught real gaps (flooring material, vanity 32-vs-30). Residual RESOLVED (`eef6ead`): added a GROUNDING rule to both prompts — anything not grounded in the transcript/scan/scope notes never appears on summary/discussed/basis/open, even softly framed ("considered but set aside" / "may include" / hedges); it goes to needs_confirm for the rep instead. Re-ran the IDX47 regression: the soft bench line is gone from the client recap and now reads under needs_confirm ("Niche: noted as the selected option over a bench, ... confirm before bidding tile"); summary/discussed are clean directional statements, INVERTED=false.
- Item 2 (`97c6373`) — SPEECH VOCAB BIASING. @capgo plugin wraps SFSpeechRecognizer but did NOT expose contextualStrings. Added the pass-through in the plugin Swift (RecognitionOptions.contextualStrings + call.getArray("contextualStrings", String.self) + recognitionRequest.contextualStrings) — persisted via patch-package (NEW: patch-package devDep + "postinstall":"patch-package" so it survives Codemagic's npm install; patch file `patches/@capgo+capacitor-speech-recognition+8.1.2.patch`). RoomPlanPlugin untouched. NEW `src/lib/consultationVocab.js` = construction vocab (trade terms + trade_taxonomy parent names) as config; buildContextualStrings(prefix) appends the tenant hot-word prefix. AmbientPanel → createCaptureController → SpeechRecognition.start passes contextualStrings (ignored on web). NEEDS the next Codemagic build to verify on-device. Verified: typed getArray<T> API exists (JSTypes.swift:95); web build green.
- All three build green; edge fn (compose) deploys via GitHub Actions (verified deploy success + ran the live regression). TestFlight items: spoken "caption ..." override on a photo; trade-term transcription accuracy (say "niche / curbless / GFCI / backer board" — should transcribe clean).

[LOG — 2026-07-18] — AVEN_MERGE_ARC B6.1 (merge the two agent edge fns) — Phase A/B + Slice 1
- Design-first, highest-blast-radius arc. Blueprint: AVEN_MERGE_ARC.md (root). Kalin go: 3-slice path approved; R5 (field money read-back) = SLICE 4 after cutover, NOT diluted into the verbatim-card cutover proof.
- PHASE A re-audit (vs archived 2026-06-16): STILL exactly 6 duplicated blocks, ZERO further drift since June (the "mirror must stay identical" guards held). Tools verified live: master 28 (6 read + 22 write, 14 CONFIRM_TOOLS), field 7 (3 CONFIRM). Overlap: 5 shared verbs with DUAL executors (log_payment, log_receipt, add_note, advance_phase, submit_change_order), 23 master-only, 2 field-only (create_lead, update_material_status). Key finding: field's confirm cards use fmtMoney only — NO amountToWords money read-back (standing gap vs the locked rule; field predates it) → R5/Slice 4. Callers: AI_MASTER_URL→MasterAgent.jsx, AI_FIELD_AGENT_URL→AiFieldAgent.jsx (cutover = repoint 2 exports). Both fns already import _shared → pattern proven.
- SLICE 1 (`02608d3`) — shared modules, drift blocks dead. Extracted to _shared: agentPhaseGates.ts (PHASE_ORDER/PHASE_LABELS/MANUAL_ONLY_PHASES/getNextPhase/runGatesForTransition incl the C3 selections gate), agentNotify.ts (notifyTenantStaff), agentFormat.ts (fmtMoney, amountToWords). Both legacy fns repointed to import them; net −96 lines. Behavior-preserving (the shared code IS what both ran; field aliases notify, doesn't yet use amountToWords). Extraction done via an anchor-based node script (newline-aware — the working tree is CRLF) so it's exact-byte, not hand-retyped. Verified: esbuild bundles both fns clean (syntax + every _shared export resolves); GitHub Actions deploy SUCCESS; live smoke both fns HTTP 200 with normal greetings. Legacy fns still live + untouched-in-behavior = rollback intact.
- SLICE 2 (`f738034`) — merged fn on NEW slug ai-agent (unwired). One Deno.serve dispatches by `surface`: 'master'→handleMasterAgent, 'field'→handleFieldAgent. ai-agent/masterCore.ts + fieldCore.ts are VERBATIM copies of the legacy fns with only the Deno.serve wrapper turned into an exported fn (both import the _shared modules from Slice 1). index.ts reads surface from a CLONED body (core gets the untouched req), default-denies unknown surface. Legacy slugs stay live + independent → rollback = revert the cutover URL only. Deploy SUCCESS. FLOW-TEST (merged vs legacy, live sandbox): routing 400/200/200 + default-deny ✓; master read verb both 200 ✓; master CONFIRM write pending_action VERBATIM-IDENTICAL incl the amountToWords money read-back ("...one thousand two hundred fifty dollars and fifty cents...") ✓; field CONFIRM write identical behavior (field uses a verbal "say yes" text-confirm, pa=null turn 1 — same on both; reply-text wording varies only by model non-determinism, present in legacy too) ✓.
- SLICE 3 (`aef8709`, cutover ALONE) — encoded surface in the URL: AI_MASTER_URL → `${FN}/ai-agent?surface=master`, AI_FIELD_AGENT_URL → `?surface=field`; the dispatcher reads ?surface (addendum `3d72fe4`) so NO caller body changed (MasterAgent/AiFieldAgent build many bodies — a query param is the 2-line cutover). Rollback = revert this one commit. Verified: query routing 200/200/400; audit:schema after = write-drift 0, no NEW drift (the 1 read-drift is pre-existing scanner_version; the audit still scans the unchanged legacy ai-master-agent). Legacy fns marked DEPRECATED (`69bed71`, comment only) — stay deployed as the rollback target; features go in ai-agent/{master,field}Core.ts; deletion is later cleanup.
- SLICE 4 (`345a4a5`, R5) — restored the field money read-back on the LIVE field surface (ai-agent/fieldCore.ts): its 3 money verbs (log_payment, log_receipt, submit_change_order) now render digits + shared amountToWords. Verified eyes-on: "Submit $1,250.50 (one thousand two hundred fifty dollars and fifty cents) change order…", "Log $842.75 (eight hundred forty-two dollars and seventy-five cents) expense…". Field now matches the master surface + the locked money-safety rule.
- ARC COMPLETE (v1). ai-master-agent + ai-field-agent merged → one surface-dispatched `ai-agent`; 6 drift blocks dead (shared); confirm-card + money read-back proven verbatim/parity; rollback lever intact (revert aef8709). Tracked follow-ups (NOT blocking, not started): converge the 5 dual-executor tools (log_payment/log_receipt/add_note/advance_phase/submit_change_order) into one executor each; cache the field surface (R6); delete the deprecated legacy slugs once field-stable (R7). Note for the schema audit: it points at ai-master-agent (legacy copy) — repoint it at ai-agent/masterCore.ts when the legacy slug is deleted.

[LOG — 2026-07-18] — AGENT_DOCS (Aven generates documents from chat, saves to the job) — Slice 1
- Kalin-locked 2026-07-18; builds on the merged ai-agent base. Absorbs plan priority #5 (lien waivers → Slice 2). Audit-first.
- AUDIT (the "hits the books" decision): the client receivable ledger is the `invoices` table (invoice_number via RPC next_invoice_number → INV-YYYY-NNNN, status draft, total_amount, pdf_url; line items in invoice_line_items; job_transactions.invoice_id links payments). Canonical creators: sbCreateInvoice (src) + _shared/autoInvoice.ts (edge). A quick invoice EXTENDS this — never a parallel ledger. pdf-lib already runs in edge fns (send-invoice/generatePDF, `https://esm.sh/pdf-lib@1.17.1`; gotcha: embedPng crashes on alpha PNGs → JPEG/text only). Branding on `tenants` (name, logo_url, business_email/phone/address); no license column. Storage = recap job_files pattern (job-documents bucket; category Invoices/Documents both exist; related_entity_type needed a 'document' value). Doc tools go on the merged fn master surface (ai-agent/masterCore.ts).
- SLICE 1 (`08842d8` + fix `bafcdd9`) — doc engine + quick invoice. NEW `_shared/docRender.ts` (reusable block-based pdf-lib letterhead renderer; branding from tenants — multi-tenant, no hardcode). NEW `_shared/agentDocs.ts` createDocument(): invoice = next_invoice_number RPC → invoices row (draft) + invoice_line_items → branded PDF → job-documents upload → job_files row (category Invoices, related_entity_type 'document', dedupe delete-then-insert keyed to invoice.id) → invoices.pdf_url set → 7-day signed URL. masterCore.ts: create_document tool (doc_type enum ['invoice'] so the model can't pick an unbuilt type) + CONFIRM_TOOLS (15 now) + REQUIRED_FIELDS + describeConfirmAction (amountToWords read-back) + executeTool (owner/PM guard → createDocument → returns summary + signed link). Migration `20260718220000`: job_files.related_entity_type += 'document'. CLAUDE.md tool inventory updated (master surface 29 tools).
- BUG caught in verify: supabase-js PostgREST builder is a thenable with NO `.catch()` — `.insert(...).catch(...)` threw TypeError at execute (invoice row created, PDF aborted). Fixed → await + destructured-error. (autoInvoice.ts still uses `.catch` on an insert — latent same bug there; not touched this slice.)
- VERIFIED live on sandbox (throwaway invoices, cleaned up): confirm card "Create invoice for $1,500.00 (one thousand five hundred dollars) · … · records a receivable on the books"; execute → invoices row draft + line item + job_files (Invoices) + pdf_url; PDF rendered + eyeballed (Avenstone Group LLC letterhead from tenant data, INVOICE #, Bill To/Project, line item, Total Due $1,500.00, footer). COST: zero new model calls (template doc filled in code; tool call rides the existing per-message Sonnet call).
- NEXT: Slice 2 lien waivers — ⚠️ STOP for Kalin: statutory language, MO vs KS differ (MO has statutory form requirements), Avenstone works both sides of the line. Draft 4 variants (conditional/unconditional × partial/final) × state param, show full text both states, flag attorney review before first real use. Ship only after his go; templates as data.

[LOG — 2026-07-18] — CONSULTATION_POCKET_FALLBACK — capture survives the pocket + coach carries room context (2 fixes)
- Field-confirmed by Kalin: consultation capture DIES on screen lock (UIBackgroundModes: audio is NOT enough for SFSpeechRecognizer). Also the coach asked "flooring?" with no room context mid-bathroom-talk. Built in the isolated worktree (SCOPE_RISK/AGENT_DOCS live on main); rebased onto origin/main.
- FIX 1 (`b83f613`) — keep-awake + gap safety net. NEW `screenWake.js` holds a web Screen Wake Lock for the whole active session so the phone never auto-locks (re-acquired on visibilitychange→visible since the OS drops it on hide; graceful no-op where unsupported). DELIBERATELY not the native @capacitor-community/keep-awake plugin — that touches shared package.json/node_modules across parallel instances + needs a Capacitor sync; ESCALATION path if the web lock doesn't hold in the WKWebView. Session-start "going hands-free? tap 🌙" hint. If the rep locks anyway: `visibilitychange` catches the return; if backgrounded >2.5s mid-recording, stop cleanly, insert a timestamped `[CAPTURE GAP …]` marker (flushed to raw_transcript), show a "capture paused — resume" banner (Resume re-arms). Both recap SYSTEM prompts (client + sub) now honor gap markers — never invent gap conversation; flag as an open item. VERIFIED live: simulated background→foreground (>2.5s) → banner + marker in the UI AND in raw_transcript (DB-confirmed); Resume returned to "Ambient Recording Active".
- FIX 2 (`1a01d82`) — coach items carry room context. Root cause: sbLoadConsultationChecklist collapsed all scoped rooms into a project_type set and deduped fields globally, so items had no room. Now returns each in-scope ROOM INSTANCE (job_room_scopes: id + room_label; duplicate unnamed labels → "Bedroom 1/2") + fields grouped by project_type. buildNeedsList expands per (room, field), tags room_label, groups by room, floats the room(s) detected in the current transcript to the top (MENTIONED NOW badge); needsListSpeech reads the room name ("For Bedroom 1, ask flooring material?"). Whole-job coverage kept. Answer tracking stays global by field_key (answering a field clears it from every room — accepted approximation, no per-room answer store). Deterministic, no model call. Only AmbientPanel consumed these (safe reshape). VERIFIED live: seeded Master Bath + Bedroom on the sandbox job, coach rendered the "Master Bath" group header over its 16 items (bedroom has no walk_stage fields → no group, correct); unit-tested multi-room grouping/float/global-answer + room-in-speech; seeds cleaned up.
- COST: zero new model calls (both fixes deterministic; the gap marker rides the existing per-chunk ambient Haiku on the rare lock event). Build green both commits. TestFlight checklist items 8 (keep-awake + gap recovery, incl. the native-plugin escalation) + 9 (steam-shower escalation once capture survives) appended.

[LOG — 2026-07-19] — AGENT_DOCS Slices 3 + 2 + 4 shipped (forms/letter, lien waivers, send verb)
- Built on one lean `create_document` verb (doc_type dispatch) + one `send_document` verb — not seven verbs. `_shared/agentDocs.ts` refactored a shared `saveDocPdf` (upload → job_files → 7-day signed URL); createInvoice/createLetter/createForm/createWaiver all route through it. All doc paths reuse `_shared/docRender.ts` block renderer (meta/twoCol/table/totals/heading/paragraph/signature) — paragraph+signature blocks first exercised here (Slice 1 invoice used only meta/twoCol/table/totals).
- SLICE 3 — forms + generic letter. Forms delivery_acceptance / damage_waiver / material_receipt = template fill in code, ZERO model calls (params: description=what it covers, party=other party, optional amount). Letter = the agent composes the full body into the `body` tool arg during its normal turn (no extra model call); describeConfirmAction previews the WHOLE body on the confirm card so the rep vets the words before save. Non-invoice docs save to category Documents with a fresh-uuid related_entity_id (each distinct; no dedupe).
- SLICE 2 — lien waivers. ⚖️ **Attorney reviewed MO + KS text and signed off 2026-07-18, NO redlines** (provenance on record per Kalin). Migration `20260718230000_waiver_templates.sql`: `waiver_templates` table (RLS: owner-editable, tenant-select) seeded 8 rows = Avenstone × {MO,KS} × {conditional,unconditional} × {progress,final}; text lives as DATA, editable without code. `createWaiver` loads the row, fills {{state}}/{{claimant}}/{{customer=branding}}/{{owner=client}}/{{job_address}}/{{amount}}/{{through_date}}/{{exceptions}}, renders (NOTICE recital + body + signature). UNCONDITIONAL variants carry `requires_payment_gate=true` → HARD GATE: executor refuses to render unless `payment_received===true`; confirm card forces amountToWords read-back + explicit "payment received?" prompt. Auto-fills amount/claimant from a linked sub_invoice; links the saved file back via `sub_invoices.lien_waiver_file_id` / `job_transactions.lien_waiver_url` (+lien_waiver_signed_date) when a sub_invoice_id/transaction_id is passed. (contacts.company doesn't exist — claimant lookup selects `name` only.)
- SLICE 4 — send verb. NEW `_shared/agentSend.ts` sendDocument(): emails a job document/estimate as a secure 7-day link via Resend, on the agent's SERVICE-ROLE client (no user JWT — unlike JWT-gated send-files-bundle; and subject reflects the real doc, unlike send-estimate-email's hardcoded "Estimate"). Resolves by document_id or by document_kind (latest active match by category/name). `send_document` in CONFIRM_TOOLS (16 now) + REQUIRED_FIELDS; confirm card renders recipient address VERBATIM; owner/PM only.
- masterCore.ts: doc_type enum → {invoice, delivery_acceptance, damage_waiver, material_receipt, lien_waiver, letter}; REQUIRED_FIELDS.create_document trimmed to [job_id, doc_type] (per-type fields validated in executor so a letter isn't asked for an amount). CLAUDE.md inventory → 30 tools / 16 CONFIRM_TOOLS; priority #5 marked SHIPPED.
- VERIFIED live on sandbox (all 4 doc types + send), PDFs eyeballed (rasterized with standardFontDataUrl): waiver = Avenstone letterhead, LIEN WAIVER, $5,000.00, NOTICE recital, all placeholders filled, signature; letter = model-composed multi-paragraph body matching the card preview. Payment gate proven at BOTH layers: model refuses to build a card until payment confirmed (conversational), AND executor rejects a replayed card with payment_received=false (exactly ONE unconditional waiver written, not two). send_document fired a real email to client+test with the verbatim address. Test docs cleaned up (0 remain; storage + job_files). audit:schema = 0 new drift (the 1 read-drift `normalize-scan.scanner_version` + 2 write-skips in supabase.js are pre-existing, unrelated). COST: forms/waivers/send = zero new model calls (template fill / one send). Letter = zero EXTRA calls (body rides the existing per-message Sonnet turn). Verification burned ~10 sandbox agent turns.
- Slice 2 follow-through DONE as designed: blessed text wired as data + unconditional payment-gate + ledger links. Nothing left parked.

[LOG — 2026-07-19] — CONSULTATION_POLISH — 3 fixes + copy tweak (vocab biasing already confirmed on-device)
- Audited IDX47 (`c68495d9`) live before coding: session had rich extraction (scope_hints/concerns/risks) + ONE scoped room (Bathroom `_2`, raw sqft 111, full_remodel) but ZERO consultation_measurements + empty checklist_answers; scan normalized total = 1235.9 (~1236, the 5-room whole-floor sum). Root cause of the "1236 + blanks" bug = two mount-effect RACES where the job/scan path beat the session path.
- FIX 1 (session-first prefill). Three parts. (a) `sessionToEstimatePrefill.js`: SF now derives from width×length pairs and spoken "AxB / A by B" phrases (in fields OR scope_notes), not just explicit total keys — so spoken "10x5 bathroom" → 50 SF; still MAX-not-sum across rows (shower-inside-bathroom won't double-count); measurement `scope_notes` now fold into the notes/`special` block. (b) `EstimateTab.jsx`: killed the races. Prefill read ONCE synchronously into `prefillRef` (keyed on job.id) before any async effect, one-shot LS clear moved to its own effect — all consumers read the ref. Estimate-history effect now RETURNS without touching estForm when a prefill exists (was clobbering scope/rooms/notes → the blanks). SF is now a single authority effect with strict precedence: floor_sf answer > **session measured SF** > scoped-room derivation (deriveProjectSf) > **whole-scan total (LAST fallback only)**; removed the crude scan-SF set from the scanMeas effect (that was the 1236 override). (c) Provenance labels added for `scan` ("from the full floor-plan scan (not this session)") and `answer`; `session` label clarified "in this session". NET for IDX47: prefill now shows Bathroom scope + 111 SF (source 'scope', 1 scoped room), NOT 1236 + blanks.
- FIX 2 (live transcript). `AmbientPanel.jsx`: interim hypothesis now painted live — new `livePartial` state set in `handlePartial` (was ref-only → no repaint), rendered appended to the finalized transcript in muted italic (#9CA3AF), cleared on `onSegment` when it solidifies. Display only, zero capture-pipeline change.
- FIX 3 (retroactive caption). `compose-consultation-recap/index.ts` `spokenCaptionFor`: added the RETRO form — "caption that" / "caption that to the picture/photo" (command w/ NO content, signal = the demonstrative "that" right after "caption") uses the utterance IMMEDIATELY BEFORE the command as the caption; FORWARD "caption <text>" unchanged. Handles both orderings (describe-then-shot AND shot-then-describe), strips trailing punctuation before the sentence-boundary cut (the bug that ate the whole utterance) + a leading discourse filler (then/and/so/ok…). Deterministic, zero model calls; flows through the same caption_source='spoken' path. Unit-tested 7/7 (fwd + retro + prefixes + no-command).
- Copy: wrap-up button now "Draft estimate from this session" (was "Draft Estimate from Session →") — reads as the bridge, not a demand.
- Build green. COST: zero new model calls (all three deterministic; FIX 3 rides the existing per-recap Sonnet call). Fixes 1+2 verifiable on web/sandbox; FIX 3 + live-render feel confirmed on the next device session.

**[LOG — 2026-07-24] TAKEOFF_QA — bathroom takeoff produced garbage lines; 8 symptoms traced to engine/seed/UI.** Commits `9d595ff` (engine) · `55f9b5b` (seed) · `26cb9cd` (UI), pushed to main. Real run audited: room `fc8cd059..._2` (full_remodel, shower 48×36×96, scan_note "Glass door…tall elongated toilet" but door_type persisted as the `slider` default).
- Sym 3 (LVP checked+optional / double floor): `optional`+`conditional` were captured onto lines but NEVER gated. `pricingCore` now skips optional/alternate trades unless EXPLICITLY named by a subset/custom list; fired lines never show "optional". LVP no longer auto-fires under `__all__`.
- Sym 4 (vanity/top/toilet/sink ×2): schema `fixture_select` + `sink_count` collided with the SAME fixtures hardcoded in templates' `materials_formula`, unreconciled. Added fixture-identity de-dup (`schemaOwnedFixtureSet`/`fixtureIdForMaterial`): when a room's schema owns a fixture, the template generic is suppressed. `tile_only` (no toilet selector) keeps its template Toilet — cannot regress.
- Sym 5 (tub spout on shower-only): material formula entries now support a structured `when` (`matchesWhen`); seed tags Tub spout → tub present, Shower valve trim → shower present.
- Sym 6 (door prices slider despite swing): capture was fine; the schema `default:"slider"` silently priced an $850 door when the rep never picked one. Dropped the default (full_remodel+tile_only). NOTE: existing saved rows still hold `slider` — rep must re-pick on those. Door intent living in free-text scan_note is a known gap (recommend surfacing scan_note by the door picker — NOT built).
- Sym 7 (casing 7 LF = one leg): seed multiplier 7→17 (2 legs + head, one side). Only casing formula in catalog. One-vs-both-sides on rate list.
- Sym 8 (tile wall $2,300): `shower_wall_sf_from_dims` tiled 4 walls for `shower_only`; alcove has open front = 3 walls (`w + 2*l`). 48×36×96: 112→80 sf (~30% of the inflated $2,300 was the SF bug). `tub_only` unchanged; `tub_plus_shower` kept 4-wall.
- Sym 1/2 (UI): custom trade picker now room-relevant-first + "show all" expander (`sbLoadTemplateTradesByRoomType`); `feet_inches` field → two 16px numeric ft/in boxes (iOS-zoom safe), stored contract still total inches.
- Both `pricingCore` + `computeFns` edited in lockstep across src/lib and `_shared` (edge). New `pricingCore.takeoffqa.test.mjs` (9/9) + existing `takeoff.test.mjs` (10/10) green; web build green. Seed migration applied+verified live. COST: zero model calls.
- **Rate-review list handed to Kalin (NOT changed silently):** Demo $5.50/sf on floor-sf-only basis (a gut ≈ $286 → too cheap; basis excludes walls/fixtures); Tile-Wall labor $20.50/sf (~$1,640 at corrected 80 sf — his call); casing one-vs-both-sides; "Bath faucet" left unconditional (tub-vs-lav ambiguity); shower_wall orientation assumption (width=back wall).

**[LOG — 2026-07-26] TIER2 #4 RATE-RECONCILIATION Slice 1 — close the gap-fill write-path leak.** Commits `9cbcbdc` (edge fn) · `2880758` (EstimateTab) · docs, pushed to main.
- **Confirmed root cause: write target ≠ read target.** The deterministic `price_plan` engine reads `takeoff_unit_costs` (`handlePricePlan` loader: `.eq("room_type", projectType).or(tenant_id.is.null,eq.<tenant>)`; `pricingCore.buildCostMaps` keys base labor by `trade` alone). But rep-filled gap rates were written by `EstimateTab.saveLearnedRates` → `sbInsertRateBookLabor` → `rate_book_labor`, a table the engine never reads. So filled gaps re-asked on every regen. Confirmed all four audit points against live code before touching anything.
- **FIX.** (1) `ai-estimator/index.ts`: `draftLineToPricedLine(line, geoSource, projectType)` now attaches `unit_cost_key {room_type, trade, category, material_name, unit}` on the MISSING-RATE branch only — `room_type` = the exact `projectType` the loader filtered on (byte-identical, else the override lands where the loader never looks). Added `unit_cost_key?` to `PricedLine`. Formula lines never get it — its presence is the routing signal. (2) `EstimateTab.jsx`: `applyGapRates` carries `line.unit_cost_key` onto the candidate; `saveLearnedRates` routes per-candidate on `unit_cost_key` presence → `sbSaveTenantUnitCostOverride({tenantId, roomType, trade, materialName, category, unit, baseRate, sourceUnitCostId:null})` for price_plan gaps, `sbInsertRateBookLabor` for legacy LLM gaps (unchanged). Failure-shape difference handled (`{error}` vs `{ok}`) — any failure → panel 'error'. Save-offer copy is now destination-aware ("Takeoff Costs" vs "Rate Book", split on mixed batches) to match the gap badge "⚠ add to Takeoff Costs".
- **Round-trip VERIFIED live** on sandbox `5ebd7c3c` (999 Cost Plus Sandbox, bathroom). Deployed endpoint emits gap line `Cleanup/lump` with `unit_cost_key.room_type="bathroom"`. Inserted a tenant override (base_rate **777.77**, id `9b0f460b…`, tenant_id = Avenstone NOT null) mirroring exactly what `sbSaveTenantUnitCostOverride` writes → re-ran `price_plan`: Cleanup line = **`source_label:"takeoff_formula"` + badge `◈ Formula (tenant_override)`**, unit_price 777.77, gap gone. `rate_book_labor` Cleanup rows = 0 (routing never touched it). Deleted the test row → gap returns (`regional_avg` + `unit_cost_key`), catalog restored to baseline (only the platform null-rate row remains). Web build green. `audit:schema` = 0 new drift (the 1 read-drift `normalize-scan.scanner_version` + 2 supabase.js write-skips are pre-existing). COST: zero model calls (deterministic path throughout).
- **Open (out of scope, per prompt):** (a) MATERIAL gaps stay on the legacy path — `sbSaveTenantUnitCostOverride` inherits `coverage_sf` from the platform default; no platform row → `coverage_sf=null` breaks the ÷coverage formula. Needs its own decision (ask rep for coverage, or block). Only labor (`category==='labor'` filter) routes to overrides. (b) `RateBookScr.jsx` still edits `rate_book_labor`/`rate_book_material` — pointing that surface at `takeoff_unit_costs` is Slice 2. (c) Existing `rate_book_labor` rows NOT migrated — locked decision is discard-and-re-enter, no backfill. (d) Legacy LLM pricing branch unchanged, still reads the rate book.

**[LOG — 2026-07-26] TIER2 #4 RATE-RECONCILIATION Slice 2a — room-agnostic rates + 4-level precedence (SUBSTRATE ONLY, no UI).** Commits `5f00138` (migration+types) · `6fb0914` (pricingCore lockstep) · `28e82d9` (loader) · `20c4127` (helper) · `5354042` (test) · docs. Kalin's locked call: labor rates do NOT vary by room type — one rate per trade, entered once.
- **Audit (live, pre-change):** `takeoff_unit_costs` had `room_type NOT NULL`, no `vetted`/`source` column, 137 rows (135 platform / 2 tenant / 6 room types). Unique indexes `idx_uc_labor_uniq` + `idx_uc_mat_uniq` used `COALESCE(tenant_id::text,'__platform__')` + raw `room_type` + `COALESCE(material_name,'')`. Both `buildCostMaps` copies (src + `_shared`) confirmed logically identical before edit — no drift.
- **Migration `20260726120000`:** `room_type` → NULLABLE (NULL = all room types). Rebuilt both unique indexes with `COALESCE(room_type,'__all__')` (matched the table's coalesce pattern, NOT NULLS NOT DISTINCT). Added `vetted BOOL NOT NULL DEFAULT false`; backfilled `vetted=true WHERE tenant_id IS NULL` (135 platform vetted / 2 tenant unvetted — restores the substrate B3.1/B3.2 needs, stored-not-read). No rows collapsed/migrated. Verified live via information_schema + pg_indexes (not just the apply script); gen:types committed.
- **Precedence (buildCostMaps, both files lockstep):** up to four rows compete per key → `rank=(tenant?2:0)+(room_type!=null?1:0)`; tenant+room(3) > tenant+all(2) > platform+room(1) > platform+all(0). Best PLATFORM row = base (coverage_sf/waste_pct/unit), best TENANT row = rate (base_rate/tenant_id/id), fall back to tenant as base when no platform. Order-independent (unique index → no intra-tier rank ties). Tenant always beats platform on rate (locked #5).
- **Loader:** `.eq("room_type",pt)` → `.or("room_type.eq.<pt>,room_type.is.null")`; combined with the separate `.or(catalogFilter)` tenant group = `(room_type=pt OR NULL) AND (tenant_id IS NULL OR =me)` — two independent AND-ed groups, the room widening can't surface another tenant's rows. Templates stay room-scoped. `sbSaveTenantUnitCostOverride` gained `roomType=null` (`.is('room_type',null)`); Slice 1 call site unchanged (gap fills stay room-specific).
- **PRICE STABILITY (the gate): BEFORE $7,443.48 / 47 lines — AFTER $7,443.48 / 47 lines**, spread $0.00, deep-equal both runs (`tools/price_stability_test.cjs` N=10, live deployed fn). Prices provably unmoved. De-risked the one tenant materials override (Drywall, coverage_sf=null): current stable loader order is platform-first, so the live line already used coverage=32 (platform) + rate=20 (tenant) — exactly what the new always-merge produces. Tests: rateAuthority 47/47 (21 orig + 26 new precedence, both input orders), pricingCore.takeoffqa 9/9, takeoff 10/10, scopeTranslation 40/40, pricePlan 12/12; pricingCore.test.mjs 22/24 (F1,F3 PRE-EXISTING fails, unrelated — test-fixture drift from TAKEOFF_QA, unchanged by this slice). build green; audit:schema 0 new drift.
- **Open:** Slice 2b (point RateBookScr at `takeoff_unit_costs`) is the surface. `vetted` written but not yet read (B3.1/B3.2 consume it). Material gap-fill still not routed (coverage question, since Slice 1). No existing rates migrated (Kalin re-enters clean).

**[LOG — 2026-07-26] TIER2 #4 RATE-RECONCILIATION Slice 2b — Rate Book onto takeoff_unit_costs. ITEM #4 CLOSED.** Commits `4fe812b` (helpers) · `0682bfd` (RateBookScr) · `759f25e` (F1/F3) · `e27e0f3` (waste_pct fix) · docs. The editing surface now writes to the table the engine reads; Kalin re-enters rates here and they land where price_plan looks.
- **Audit:** RLS on `takeoff_unit_costs` already has all four tenant-scoped policies (SELECT platform+own; INSERT/UPDATE/DELETE `tenant_id = get_my_tenant_id()`); platform rows (`tenant_id IS NULL`) unreachable for write. **No migration needed.** Catalog 137 rows (135 platform / 2 tenant: Demo bathroom labor 5.50, Drywall-Hang bathroom materials 20.00 — both vetted=false). 4 trades' platform defaults disagree across room types on BOTH rate AND unit (Cabinets, Electrical-Rough-in, Plumbing-Finish, Plumbing-Rough-in) → collapsed view shows a range, not one number.
- **Helpers (supabase.js):** `sbLoadTakeoffCatalog` (platform+tenant, all rooms; carries a DELIBERATE 2nd copy of buildCostMaps rank, cross-ref comments both ways), `sbSetUnitCostVetted` + `sbDeleteTenantUnitCostOverride` (`.not('tenant_id','is',null)` guard → platform rows refused). **`sbSaveTenantUnitCostOverride` had a latent NOT-NULL bug** (`waste_pct: platform?.waste_pct ?? null`) that threw on EVERY all-rooms write (no all-rooms platform row to inherit from) — the write path 2a added but never exercised. Fixed to `?? 0` (`e27e0f3`); strictly safe.
- **Screen (RateBookScr):** repointed off `rate_book_*`. One row per (trade, category, material_name) collapsed across rooms; provenance badge (Your rate all-rooms / room-only / Platform default / range). Edit always writes an all-rooms tenant override (`room_type=null`). Footgun surfaced: room-specific override (rank 3) shadows all-rooms (rank 2) → warning names the rooms + one-tap "use my all-rooms rate everywhere" deletes the room-specific row(s).
- **VERIFIED live (real values):** (1) Multi-room round-trip — all-rooms Plumbing-Rough-in override $88.88 → `price_plan` **bathroom = 88.88** AND **kitchen = 88.88**, both `unitCostSource=tenant_override` (platform defaults were bathroom each $525 / kitchen lump $1150 — one entry, every room). (2) Footgun — bathroom-specific override $55.55 (`8780c014`) alongside all-rooms $88.88 (`eee92d8f`): bathroom priced **55.55** (shadow proven); "use everywhere" deleted `8780c014`; bathroom reverted to **88.88**. (3) Platform immutability — UPDATE and DELETE of a platform row via the helper guard both returned `[]` (refused). All test rows cleaned; catalog back to 137/2-tenant baseline.
- **Tests:** pricingCore.test.mjs **24/24** (F1/F3 corrected — see below), rateAuthority 47/47, takeoffqa 9/9, takeoff 10/10, pricePlan 12/12. Price stability N=10 = **$7,443.48** spread $0.00 deep-equal (identical to 2a — no rate moved). build green; audit:schema 0 new drift.
- **F1/F3 were FIXTURE DRIFT, not regressions** (engine confirmed correct): F1 expected the old 4-wall shower (128); Sym 8 made shower_only a 3-wall alcove → `(60+2·36)/12·96/12 = 88`. F3's fixture declared the trade `optional:true`; Sym 3 stopped optional trades auto-firing so no line appeared → changed fixture to `optional:false` so it exercises the pending_rate path it claims to.
- **Open / limitations:** `rate_book_labor`/`rate_book_material` now have NO editing surface (feed only the deprecated LLM/flip branch; legacy gap-fills still write there unreviewed — acceptable, not building a 2nd screen). `vetted` editable but not yet enforced (approval gate = B3.1/B3.2). Material gap-fill still not routed (coverage_sf question). No existing rates migrated.

**[LOG — 2026-07-26] RATE_BOOK_POLISH — make Rate Book fit for entering ~50 rates in one sitting.** Commits `4127c89` (RateBookScr) · `0ddf739` (walk measurements) · docs. From the RATE_BOOK_WALK seat review (mechanics were right; friction was findability + no chaining). Files: `RateBookScr.jsx` only (no helper needed — in-place update built from `sbSaveTenantUnitCostOverride`'s returned id).
- **Shipped:** (1+2) segmented **All/Labor/Materials filter + search** on trade/item name — Labor cuts the list **98→30 rows**. (3) **Enter-to-save-and-advance** — Enter saves the row and opens+focuses the next visible row's rate input (`id=rbrate-<key>` + autoFocus + `flatKeys` order); Esc cancels; Tab unchanged. (4) **In-place row update** — save/vetted/use-everywhere mutate local `rows` state; the per-save `load()` (a `select=*` of all rows) is GONE. (6) **Mobile badge density** — `provenance().labelShort` + ellipsis/nowrap so the range badge stops wrapping to 3 lines at 390px.
- **Counter (fix 5) was TIMING, not a bug** — the walk's desktop 0/3 vs mobile 0/7 was test rows being created between the two captures; denominator is the live tenant-row count (baseline reads 0/2). No change.
- **Before→after ergonomics (tools/rate_book_walk.cjs, owner seat test-rep):** interactions/rate: mouse path unchanged (2 clicks) BUT keyboard path now **type→Enter, 0 mouse clicks between rows** (`keyboardEnterSaves` false→**true**; 3/3 chain steps advanced focus to the next input). full-catalog refetches per save: **~1 select=* (all rows) → 0** (helper's 2 targeted single-row lookups/save unchanged). Findability: **no search → search + Labor filter (98→30)**. Latency measurement (~450 ms) is harness-bounded (my fixed wait + the rate-button selector matching every row), not a real save-time delta — the real win is the removed refetch. Mobile 390px renders clean (no 3-line badge wrap). staleWarning still fires + one-tap-removes; cleanup restored 137/2 baseline.
- **Commit consolidation note:** the six fixes live in one interdependent file (filter → `flatKeys` → Enter-advance), and no interactive partial-staging was available, so fixes 1–4+6 shipped as ONE RateBookScr commit rather than the planned 3 — splitting would have produced non-building intermediate commits. Walk measurements are a separate tools commit.
- **Seat note:** `test-rep@avenstonekc.com` is role `owner` (only owner-capable test account); its password was set to a known value to run the owner-gated walk. Rotate if desired.

**[LOG — 2026-07-26] TIER2 #5 GAP-ENTRY SANITY RAILS — catch the mistyped rate AT entry.** Commits `700972e` (repoint) · `a1cc6e1` (rails helper) · `9df55b4` (GapBatchAsk) · `5bfeb40` (RateBookScr) · `9c977d2` (tests) · docs. Trigger incident (2026-07-17): $250 into a per-SF cleanup slot silently became $250×49=$12,250 (55% of the estimate); the rep meant a lump sum. Higher stakes post-Slice-1 because a bad rate now persists into `takeoff_unit_costs`, the authority table.
- **Defect fixed first:** `buildRateEnvelope` built its outlier band from `rate_book_labor` (retired in Slice 1) — the pre-send guard was checking proposals against rates Kalin is discarding. Repointed to `takeoff_unit_costs`. It has a single `base_rate`, not lo/hi, so the band = the DISTRIBUTION of effective rates across room contexts, each resolved with the engine's precedence rank (reused from buildCostMaps, not a 3rd copy): a single all-rooms tenant rate collapses the band to one point; room-varying platform defaults widen it. Keyed by **(category, trade, unit)** so a trade's labor and material rates on the same unit never mix. Verified live: `labor|plumbingroughin|each`=[525,525] (bathroom+basement each; kitchen's $1150 is unit `lump` → separate band, correct), `labor|paintinterior|sf`=[3.50,3.50] (5 platform rooms).
- **Three rails, ALL SOFT (warn, never block) — `priceSanity.checkGapEntry`:** Rail 1 = live line total, per-unit `$250/sf × 49 = $12,250` vs lump-sum `whole line = $250` (the contrast IS the guard; updates per keystroke). Rail 2 = draft-share ≥25% with the actual % ("55% of the estimate"). Rail 3 = unit-rate >5×/<0.2× the envelope; **NO envelope → NO flag** (locked #3 — a gap has no reference; never synthesize one). Lump-sum entries are never envelope-checked. Wired into `GapBatchAsk` (all 3; EstimateTab passes the shared envelope + priced-base draftSubtotal) and `RateBookScr` (Rails 1+3, rate-only mode: Rail 1 → unit echo, Rail 2 N/A).
- **Incident regression (priceSanity.test.mjs, 20/20):** BEFORE (per-unit) `$250/sf × 49` on ~$22k → Rail 1 breakdown `$250.00/sf × 49 sf = $12,250.00` + Rail 2 `55%` + Rail 3; AFTER (same $250 as lump sum) → `whole line = $250.00`, zero warnings. Plus an explicit "nothing blocks" assertion (checkGapEntry only ever returns warnings). No-envelope-no-flag case covered.
- **Verify:** live Rate Book $250/sf into Tile-Wall (envelope $20.50) → `Entering $250.00 / sf` + amber `over 5× the typical high ($20.50)`, Save still present (non-blocking) — screenshot `tools/rb5_ratebook.png`. The GapBatchAsk gap-panel is a mid-generation surface (uncommitted, freshly-priced scope); reaching it live needs the full ScopeConfigurator interview, not safely scriptable without a destructive sandbox reset — its rails use the identical `checkGapEntry` helper, proven by the 20/20 tests against the exact incident. All existing suites green (pricingCore 24, rateAuthority 47, takeoffqa 9, takeoff 10, pricePlan 12). price_stability $7,443.48 spread $0.00 (no engine change). build green. takeoff catalog restored 137/2.
- **Nothing blocks — confirmed.** Every rail warns and the rep/owner proceeds. The pre-send guard (computeSanityFlags) still gates the client PDF as before (unchanged behavior), now checking against the authority table.

**[LOG — 2026-07-28] TIME_CLOCK_ARC S1 — crew role + time_entries + the punch loop.** New arc, Kalin-locked 2026-07-26. Commits `cd8117a` (migration+types) · `60333b6` (punch helpers) · `f8dcb97` (CrewHomeScr + routing + crew job-read migration) · docs. Hourly crew clock in/out from their phone; hours only (dollars=S4, distance flag=S2, splitter=S3).
- **Audit:** (1) role = TEXT column + CHECK `profiles_role_check` (no enum); routing = App.jsx early-returns for sub/client (added crew). (2) `send-invite` passes `role` straight through → crew invitable UNCHANGED (added a guard so only subs get auto-enrolled in `sub_invited` sequences). (3) `jobs` carries `address` TEXT only — NO lat/lng (S2 geocoding needs a new column; built nothing). (4) No `@capacitor/geolocation` plugin — reused existing `lib/gps.js stampGPS()` (navigator.geolocation, Capacitor-bridged on iOS, one-shot 6s, null-on-fail).
- **Migration `20260728120000`:** added `crew` to the role CHECK; `time_entries` (job_id TEXT FK→jobs since jobs.id is TEXT; user_id UUID FK→profiles; in/out lat/lng; source punch|switch|manual); **partial unique index `one_open_per_user ON (user_id) WHERE clock_out IS NULL`** (the load-bearing invariant); RLS (crew own; crew UPDATE only while open via `clock_out IS NULL` in USING; owner/PM tenant); atomic `time_clock_switch()` RPC (SECURITY DEFINER, identity from auth.uid() so a caller can only switch their own). `20260728120100`: extended `can_access_job` to give crew tenant job READ for the picker (read only). Both applied + verified live (information_schema + pg_indexes + pg_policies).
- **Punch helpers (supabase.js):** sbClockIn (returns the open entry as `already_clocked_in` if one exists — UI offers Switch — index backstops a race), sbSwitchJob (atomic RPC), sbClockOut, sbMyOpenEntry, sbMyEntriesToday, sbCrewJobs. Coords passed in; null never blocks. `CrewHomeScr.jsx` phone-first: clocked-out = one big Clock In → job picker (active jobs, address+client, recent first); clocked-in = job + live ticking elapsed + Switch/Clock Out; Today strip of segments.
- **VERIFIED live** (real crew JWT through RLS, seat `crew+test@avenstonekc.com` / `Avenstone`): full loop — clock in A (coords), switch→B (source `switch`, closes A + opens B), clock out; two closed segments, coords on all four punch points, **A.clock_out === B.clock_in exactly `…04:02:14.073821+00`** (atomic RPC single-transaction now()). Bypass test: raw 2nd open insert **REFUSED code 23505** (unique `one_open_per_user`). RLS: crew SELECT another user's rows = 0; crew UPDATE a CLOSED own entry = 0 rows (blocked); owner SELECT crew rows = all. GPS-denied punch = OK with null coords. build green; audit:schema 0 new drift; test entries cleaned (final count 0).
- **Known gap (locked, not built):** no offline punch queue — a dead-zone punch fails visibly, worker retries. KALIN_QUEUE (o) written GPS-capture notice before S2 flagging; (p) rotate test-rep owner password.

**[LOG — 2026-07-28] TIME_CLOCK_ARC S2 (resequenced) — employee records + pay rates + invite + YTD pay panel.** Kalin-locked 2026-07-28. Commits `7f342b0` (migration+types) · `db06d95` (helpers + earnings math) · `8089aef` (Team employee UI) · `2b821eb` (My Pay panel) · docs. STRAIGHT-TIME ONLY v1 (no OT). Never touches job financials (that's now S5). Arc renumbered: S3=distance flag+timesheet review+vigilance, S4=splitter, S5=job_transactions labor costing.
- **Audit:** (1) Team→Add = `UserMgmt.invite()` → `sbInviteStaff` → send-invite; post-password SetPasswordScr → updateUser → onDone clears the invite hash → `role='crew'` early-return → CrewHomeScr (same proven path as sub/client, NOT staff shell). (2) `profiles` SELECT = `tenant_id = get_my_tenant_id()` → **every tenant user reads ALL profiles** → pay data MUST be in separate owner+self tables (else all crew see each other's rate). (3) `inviteUserByEmail` on an existing email **errors** ("already registered"), does NOT resend → UI needs an explicit resend.
- **Migration `20260728140000`:** `employee_details` (classification w2|1099) + `employee_pay_rates` (**append-only**: UNIQUE(user_id, effective_date), **NO UPDATE policy** — raise = new row, fat-finger = same-day row or owner DELETE). RLS both: owner full, each user SELECT own, **no PM, no cross-crew**. Verified live (information_schema + pg_policies; pay_rates has select/insert/delete only).
- **Math (`lib/earnings.js`, pure, 8/8 tests):** effective rate = greatest `effective_date <= entry's America/Chicago local date`; straight time × rate; YTD (calendar year) + Sun–Sat week grouping (UTC-safe boundaries). No-rate entries: hours count, dollars EXCLUDED (never $0-priced, note shown). Open entries excluded from $. Helpers in supabase.js (load/save details, append-only add/delete rate, owner list, My Pay load, find-by-email + resetPasswordForEmail resend).
- **UI:** `EmployeeModal` (Team → Add Employee form: name/email/phone/address/start/classification/rate+effdate → invite as crew + details + first rate; already-invited → lookup profile, still save, offer resend). Owner detail view: current effective rate, classification toggle, append-only rate history + Change rate (new row) + delete-to-fix. `CrewHomeScr` **My Pay** tab: YTD gross/hours, this week, current rate, W-2/1099, weekly history; honesty label "Straight time, before taxes — overtime premium not included. Not take-home."
- **VERIFIED live (real JWTs through RLS):** crew A SELECT own pay_rates=2, crew A→B pay_rates=0, crew A→B details=0, **PM SELECT any pay_rates=0** (no PM access), owner SELECT all=3. Mid-raise math end-to-end (rates+entries read through crew A's RLS then computed): rate $20 eff 07-27 + $25 eff 07-30; entries Tue 07-28 2h + Fri 07-31 3h → **ytdGross $115.00 exactly** (2×20+3×25). No-rate + open-entry cases covered by unit tests. build green; audit:schema 0 new drift; test data + crew B account cleaned (employee tables back to 0).
- **Judgment calls:** (a) Invite dedupe handled CLIENT-SIDE (send-invite untouched) — on "already registered," look up the profile by email (owner SELECT), save details+rate anyway, resend via `resetPasswordForEmail` (recovery → SetPasswordScr → CrewHomeScr). (b) Add-employee lives in the Team screen (`UserMgmt`) via a new `EmployeeModal`, alongside the generic Invite Member. **OT revisit trigger filed (mandatory before any W-2 regularly >40 h/wk).** KALIN_QUEUE (q): 10 min with accountant on 1099-on-timeclock classification.

**[LOG — 2026-07-29] TIME_CLOCK_ARC S2b — fillable W-4/W-9, owner-sent + e-signed (employees + subs).** Kalin-locked 2026-07-28. Commits `b402145` (migration) · `6e10d2c` (fill pipeline+assets) · `ea77093` (owner send+chips+email fn) · `b1f20dd` (recipient card+evidence fn) · docs. **SECURITY NON-NEGOTIABLE HELD: the TIN never becomes DB data — DB TIN sweep of information_schema = 0 real columns; SSN lives ONLY inside the signed PDF in private storage.**
- **AUDIT #1 (the big one): `job-documents` was `public=true`** with an authenticated-any-read policy — existing sub W-9 uploads (`w9/...`, containing SSNs) were reachable via UNAUTHENTICATED public URLs. **FIXED → private** (verified: the public URL now returns HTTP 400; app already used signed URLs so nothing broke). New TINs go to a NEW private `employee-docs` bucket (owner + self by `{user_id}/` folder). #2: reused SignaturePad PNG + `clientIp()` server-IP capture + consent language from the contract flow. #3: IRS field names are machine garbage (`topmostSubform[0].Page1[0].f1_05[0]`) — mapped by CALIBRATION rasterization (fill each field w/ its id, render, eyeball). **SSN lands: W-4 `f1_05`; W-9 `f1_11/12/13` (3-2-4 comb) / EIN `f1_14/15`.** #4: emails via the Resend pattern (`api.resend.com/emails`).
- **Fill pipeline (`lib/irsForms.js`):** DRAWS values at each AcroForm field's widget rectangle + embeds the SignaturePad PNG at a calibrated signature line + flattens (pdf-lib setText+flatten wouldn't rasterize; draw-at-rect + embedded TTF renders everywhere). Runs CLIENT-SIDE → the TIN stays in-browser, only in the PDF bytes. Migration `20260729120000`: `paperwork_requests` (owner+self RLS, self-completes-own-'sent'), employee doc pointers, the two bucket fixes.
- **VERIFY #1 (make-or-break) — rasterized distinctive-value fills of BOTH forms: every value landed in the correct box, SSN in the SSN box (W-9 `123-45-6789` in the 3-2-4 comb, W-4 `987-65-4321` top-right), NOT the EIN box.** Full loop (live JWTs): owner sends W-4 → crew uploads to own folder → `record-paperwork-evidence` completes server-side (status=completed, path, **IP+UA captured**, `employee_details.w4_path` stamped — which self can't write). Sub W-9 → `profiles.w9_url` stamped (SubComplianceModal viewer routes to employee-docs). **Security:** crew→sub paperwork SELECT=0; crew signed-URL for sub's doc = DENIED "Object not found"; crew list sub folder=0; PM→any paperwork=0; owner→all=2. build green; audit:schema 0 new drift; test rows + objects cleaned.
- **Judgment calls:** (a) completion is SERVER-SIDE (evidence fn) not self-UPDATE — `employee_details`/`profiles` are owner-write-only, so self can't stamp the pointer; the fn (service role) does it + captures the trustworthy IP. (b) SubComplianceModal got a MINIMAL viewer change (route by path prefix job-documents vs employee-docs + 10-min expiry) — necessary because the new TINs go to the secure bucket, not "zero modal changes." (c) `job-documents` broad authenticated-read policy left as-is (shared bucket; the public=false fix closes the unauthenticated leak; NEW TINs are properly owner+self in employee-docs) — legacy uploaded W-9s in job-documents remain authenticated-tenant-readable, a lesser residual. KALIN_QUEUE (r): swap the W-4 template each January + re-verify field names.

**[LOG — 2026-07-29] PAPERWORK_LEGACY_SWEEP — migrate legacy sub W-9s to private employee-docs.** Commit `security: migrate legacy sub W-9s to private employee-docs bucket`. Hygiene follow-up to S2b (which closed the public-bucket hole for NEW forms; legacy `w9/*` uploads still sat in shared `job-documents`).
- **Audit finding: NOTHING to migrate.** `profiles.w9_url` is set on **0** rows (no sub ever completed the legacy upload flow) and there are **0 `w9/*` objects** in job-documents. The 8 `%w9%` matches in job-documents are coincidental random-filename collisions (e.g. `twk6w9ns4.pdf`, `yw9cr6cwr4.pdf`) — job-doc uploads in per-entity folders, referenced by no profile, out of scope.
- **Move table (old → new): EMPTY.** `tools/migrate_legacy_w9.cjs` (idempotent, service-role) ran: `{moved: 0, orphansDeleted: 0, errors: 0}`. Script committed for idempotency/future re-runs (it copies w9/*→employee-docs/{uid}/, verifies byte-size, updates w9_url, deletes original; orphans deleted).
- **Denials pasted:** `w9/*` objects remaining = **0**. employee-docs owner+self: crew uploads own folder = ok; **NON-OWNER (sub) createSignedUrl on crew's W-9 path = DENIED "Object not found"**; sub list crew folder = 0. SubComplianceModal prefix routing verified unedited (`bucketFor`: `w9/`|`insurance/`→job-documents, else→employee-docs; `pathFromUrl` handles both URL forms + plain paths).
- Re-rendered the S2b filled-form verifications to `tools/paperwork_w4.png` + `tools/paperwork_w9.png` (SSN in the correct box on both) for Kalin's Opus eyeball.
- **Filed (do NOT fix mid-arc):** `job-documents` bucket SELECT is `authenticated`-wide (not tenant/role-scoped) — violates locked #1; **must be fixed before any second tenant onboards.** Added to MASTER_BUILD_PLAN §7 Parked.

**[LOG — 2026-07-29] PAPERWORK_W9_DATE_FIX — W-9 signature-block draw coords.** Commit `paperwork: fix W-9 signature-block draw coordinates + re-verify with signature`. The W-9 date drew at y=240 — inside the Certification instructions paragraph (y≈219–248) — and the signature at y=236, both above the Sign Here block. Root cause: the W-9 `SIG` rects in `lib/irsForms.js` were bad estimates (NOT a shared-constant bug — W-4 and W-9 have separate entries; W-4 was already correct). Fixed to the block anchors extracted from the template text (W-9 "Signature of U.S. person"/"Date" labels at y≈196–204, cert text at y≈219 → date/sig now y=205; W-4 nudged to y=88/92). Added `export SIG` + `SIG_YBAND` + `irsForms.test.mjs` (6/6: every sig/date rect within its form's Sign Here y-band). Re-rendered `tools/paperwork_w4.png` + `paperwork_w9.png` **with a signature marker at the exported SIG coords** (napi rasterizer segfaults on embedded PNGs, so the verification draws a vector marker at the identical product coordinates) — both now show signature on the Signature line + date on the Date line. **Real-document check: employee-docs empty, 0 completed paperwork_requests — no signed PDF ever created, nothing to regenerate.**

**[LOG — 2026-07-29] EMPLOYEE_PAY_WALK — live both-classifications pay verification (measurement only, no app changes).** Committed the walk harness `tools/employee_pay_walk.cjs` (Playwright + service-role). Two test employees driven through the REAL flows: added via the Team UI (owner `test-rep@`), passwords set via admin (email links can't be auto-clicked), 4 backdated LAST-week sessions each via service-role `source='manual'` inserts, 1 LIVE session each today through the punch UI on a 390px viewport (Walt with a mid-session Switch Job → 2 segments; Nina plain in/out). Nina raised $35→$40 eff 2026-07-26 through the owner UI (append-only → 2 rate rows). All test data + auth users deleted at the end (both profiles gone; only pre-existing `crew+test` employee_details row remains). Screenshots local-only (not committed): `tools/pay_{w2_mypay,1099_mypay,w2_week44,owner_walt,owner_nina,owner_team}.png`.
- **Expected == displayed, to the cent (scraped from each seat's My Pay).** Walt (W-2 $28): week 7/19 = 44.0h × $28 = **$1,232.00**; YTD **$1,232.06** (+ the ~8s live switch-session @ $28); rate $28.00/hr; class W-2. Nina (1099, $35→$40): week 7/19 = 28.0h × **$35** = **$980.00**; week 7/26 (live) priced at **$40**; YTD $980.03; rate now $40.00/hr; class 1099. **Effective-dating proven**: `effectiveRate(nina, 7/20)=$35`, `effectiveRate(nina, 7/29)=$40` — week 1 old rate, week 2 new rate.
- **OT gap (Kalin's flat-rate-v1 call, made visible — NOT a defect).** Walt's 44h week displays **$1,232 straight** (44×$28). FLSA nonexempt would owe 40×$28 + 4×$42 = **$1,288** → **$56 understated**. The honesty label **"Straight time, before taxes — overtime premium not included"** renders directly between the stat cards and weekly history, fully visible WITHOUT scrolling on 390px (whole My Pay panel fits the viewport). Reconfirms the locked mandate: **OT revisit required before any W-2 crosses 40h/week regularly.**
- **RLS live (crew seat, anon key):** Walt reading Nina's `employee_pay_rates` → 0 rows, `employee_details` → 0 rows (blocked); Walt reading own rates → 1 row (allowed). Owner+self isolation holds.
- **No defects on 390px.** Add-employee modal, job picker, punch loop, switch-job, My Pay, and owner Team/detail views all drove cleanly (walk `notes: []`). One harness note for future walks: modal-internal buttons must use exact-text/scoped selectors — a bare `has-text("Add")` matched the "Add Employee" button *behind* the modal (pointer-intercepted); fixed with `getByRole('button',{name:'Add',exact:true})` + an `openDetail()` innermost-card locator.

**[LOG — 2026-07-29] TEST_JOB_CLEANUP — deleted 25 test jobs (DB + storage), keep-list verified.** Migration `20260729180000_delete_test_jobs.sql` (applied+verified) + `tools/cleanup_test_jobs_storage.cjs`. Inventory was 33 jobs → Kalin locked KEEP (6: 8617 Houston, 107 Brentwood, 1206 Lucy Webb, 12101 Pawnee, 999 Cost Plus Sandbox, 456 Test Flow Ave) + spared 2 ambiguous (810 crane st — real-looking address w/ estimate work; 11291 Hemlock — the `client+test` no-draws seat fixture). DELETED 25 = 23 test-named 2026-07-17 rows + `999 Test Lane` (7b44611a, $121k, was KALIN_QUEUE c) + `5010 e pony creek rd` (5cb49d86, Kalin's house/test).
- **FK reality:** this schema DOES enforce inter-child FK constraints (via pg_constraint — the `information_schema.constraint_column_usage` join returned empty and misled the first attempt, which failed atomically on `oh_shit_moments.converted_to_co_id→change_orders`). One cycle `oh_shit_moments↔change_orders` broken by nulling `converted_to_co_id` first, then topological child→parent deletes across ~60 tables + 8 grandchild tables (draw_line_items, sub_invoice_payments, invoice_line_items, floor_plan_versions, schedule_item_invitees, site_visit_checklist_items, engagement_bids, sales_turns). No delete-triggers exist on any child or on `jobs` — zero cascade risk to kept jobs.
- **Verify:** jobs 33→8 (6 keep + 2 spared). Orphan sweep across 24 job_id tables (incl. all money/artifact) = **0**. **Houston ledger identical before/after: $178,696.69 / 98 txns / 115 line items.** Lucy Webb $329,099.68 unchanged. Storage: 11 real objects removed by explicit path (guard: skip any path also on a kept job; 4 DB paths were phantom/never-uploaded). Removed by EXACT path only — a tenant-prefixed floor-plan + a `test/` photo were in the set; a prefix delete would have nuked shared objects.
- **KALIN_QUEUE:** item (c) `999 Test Lane` status-fix job is now DELETED → item (c) is Lucy-Webb-only. **Open manual step:** pony creek's $542.50 Home Depot/lumber txn was already QB-synced (2026-07-21) — the DB delete cannot reach QuickBooks; that QB entry must be voided by hand.

**[LOG — 2026-07-29] DRAW_MULTIFILE_FIX — build-draw-package dropped mislabeled-mime receipts.** Commit `build-draw-package: route attachments by real type, not mislabeled mime`. Symptom (Kalin, live Lucy Webb): check N boxes, only ONE file lands in the built PDF — "the most recently selected." **The selection was NOT collapsing** — `included_file_ids === file_refs` (edge line 619) showed the composer saved all 3 (QXO d75c6237 $1621.25, Haven 8b47c1d9 $10500, uploaded gutter 1f8cfa9e). **Root cause: `addDocumentPages` routed on `mime_type`.** Receipt `job_files` frequently store `mime_type='image/jpeg'` on an actual PDF (storage_path ends `.pdf`) — QXO + Haven are such PDFs. mime-based routing sent them to the image branch → `embedJpg` on PDF bytes fails → page silently dropped (only the uploaded JPEG, whose mime matched its bytes, survived). Fix: route by `storage_path` extension + `%PDF` magic bytes (final arbiter), fetch PDF candidates RAW (no imgproxy transform — corrupts them, and copyPages needs them intact), images downsized as before. Verified on the real draw via `tools/verify_draw_embed.cjs` (owner-auth invoke): A=[gutter]→2 pages, B=[QXO,Haven,gutter]→4 pages (Δ+2). The successful B build left Kalin's package correctly regenerated (included_file_ids=3, PDF 4 pages). **Data note (benign, not fixed): the receipt-upload path records image/jpeg mime on PDF uploads; the build fn is now tolerant so no backfill needed.** Related: [[the manual Add-attachment feature]] shipped same day (`7ce2a3a`).

**[LOG — 2026-07-29] REIMBURSEMENT_STATUS_CLASS — draw composer's reimbursable predicate was wrong in two directions.** Commit `fbafdca` + migration `20260729200000_fix_reimbursement_status_class.sql`. Symptom (Kalin, live Brentwood `4460936c`): building a draw, NO pending expenses appear under next draw. **Root cause: composer identified reimbursables by `reimbursement_status = 'unreimbursed'` EXACTLY.** Two failure modes, same defect: (1) legacy NULL rows (created before the column; the `set_cost_plus_defaults_on_jt` trigger only defaults NEW cost-plus outbound rows) silently HID — Brentwood had 18 outbound NULLs ($9,275.86) invisible; (2) `status='void'` rows still `unreimbursed` silently SHOWED as reimbursable ($3,852.12 across Lucy Webb + Houston) → risk of billing a client for voided money. Across all jobs: 21 stranded outbound NULLs (3 jobs), 4 void in_draw/null-draw_id orphans (Lucy Webb, $19,307.54), 3 void-visible rows. **Canonical predicate now everywhere:** `direction='out' AND status<>'void' AND billing_treatment<>'client_paid' AND COALESCE(reimbursement_status,'unreimbursed')='unreimbursed' AND draw_id IS NULL`. Fix: migration backfilled outbound NULLs→unreimbursed (INBOUND left NULL on purpose — reimbursement_status is meaningless for client payments; backfilling would have stamped ~$98.5k of inbound) + reset orphans; hardened `compose_draw` flip guard to COALESCE(null)+`status<>'void'` (else a NULL row shown by the new loader gets a line item but never flips → double-draw). Code (supabase.js): `sbLoadUnreimbursedExpenses` / `sbGetBucketBalance` / `sbLoadUncollectedClientPaidMarkup` all use the canonical predicate. Verified: Brentwood 0→18 rows/$9,275.86; Lucy Webb $227,402.43→$227,250.31 (−2 void); Houston $88,463.85→$84,996.69 (−$3,700 void, +$232.84 NULL); 7 inbound NULLs untouched; 0 orphans/stranded remain. **Follow-up (commit `5b620d3` + migration `20260729210000`): closed the source + covered readers the first pass missed.** (1) `reimbursement_status` had NO column default → every insert path (all rely on the `set_cost_plus_defaults_on_jt` trigger, which only stamps cost_plus+out) landed NULL for inbound / non-cost-plus / pre-trigger rows. Added `ALTER COLUMN ... SET DEFAULT 'unreimbursed'` — correct at birth, inert for inbound (always read behind `direction='out'`). No insert path sets the field explicitly (verified: supabase.js 504/970/1689, fieldCore 183/212). (2) `sbLoadJobFinancialSummary` `float_unreimbursed` (1563) + `next_draw` (1631) were NULL-blind (void already excluded by the fn's base `.neq('status','void')`) → now COALESCE NULL→unreimbursed. **`next_draw` is the summary figure behind the "nothing under next draw" symptom.** (3) **LIVE ai-agent `masterCore.ts` compose_draw candidate reads (2261, 2927) had NO void filter** — the "compose a draw" agent verb would pull void-unreimbursed rows into a draw as line items ($3,852 live). Now canonical predicate (neq void + is draw_id null + null-as-unreimbursed). **Dead mirror `ai-master-agent/index.ts` (2136, 2740) left unedited — `AI_MASTER_URL`→`ai-agent?surface=master`, slug unwired; fix there if ever revived.** Every money reader confirmed to filter `direction='out'`, so the inbound default is safe.

**[LOG — 2026-07-29] DRAW_MULTIFILE re-report — instrumented end-to-end.** Commit `1435e9f`. Kalin re-reports "5 checked, 1 lands" AFTER the DRAW_MULTIFILE_FIX above. Last package (`4df71be0`, 17:47) received 3 refs (all job_file: QXO+Haven receipts as `.pdf`/image-jpeg-mime, IMG_9524 real jpg) — all route to `addDocumentPages`, ext-router handles them → should render all 3. So current drop is either a NEW file-type combo the router still mishandles OR client-side selection. Added counting at every stage (no logic change): client logs `selected.size` vs `file_refs.length` + build result; server `addPhotoPages`/`addDocumentPages` return embed counts, handler returns `embed_stats {refs_received, files_resolved, photos_found/embedded, documents_found/embedded}`.

**[LOG — 2026-07-29] DRAW_MULTIFILE_ROUTER — resolved by self-testing the type matrix (commits `816df26`→`e992c2a`).** Instead of asking Kalin to QA file types he can't see, tested build-draw-package directly: `tools/draw_router_matrix.cjs` (rep-auth invoke against a CANCELLED sandbox draw so no real package is touched) builds singletons + combos and reads `embed_stats`. **Matrix result — exactly ONE type dropped: an alpha PNG mislabeled `mime=image/jpeg` + ext `.png` (id `2af619c6`, "Receipt - Permit for Belton").** Mechanism: `.png` ext → `preferPng`, but the alpha-PNG `embedPng` OOM-guard skips it and `embedJpg` rejects PNG bytes → null → the documents branch added NO page and NO placeholder = silent disappearance. Diagnostic build proved magic bytes `89504e47` (‰PNG) at all three byte sources incl. the imgproxy transcode — **Supabase image transform returns origin PNG unchanged on this project (no server-side format transcode), even with `format:'jpeg'` + `Accept: image/jpeg`.** So alpha PNG can't be rendered server-side without a WASM decoder (rejected — the worker has OOM history and this is 1 file tenant-wide). **Fix (rewrote `addDocumentPages`, never-silent):** (1) magic bytes `%PDF` authoritative over mime/ext — catches PDFs mislabeled either way, incl. image-mime+non-.pdf-ext the old router missed; (2) unembeddable images → imgproxy JPEG-transcode retry (`fetchImageJpeg`, `Accept: image/jpeg`) — rescues HEIC/webp/normal cases where transform DOES work; (3) **anything still unembeddable → labeled placeholder page** ("Preview unavailable — Type: <mime>") + `embed_stats.unrenderable[] {id,name,mime_type,reason}`; (4) sequential one-file-at-a-time (memory-safe, dropped batch prefetch); photos branch got the same transcode rescue. **Verified live:** all real jpeg/png/pdf + mislabeled-.pdf + photos render; only the alpha PNG placeholders (reported). **Zero silent drops.** Regression harness: `tools/draw_router_matrix.cjs`. **Open (optional):** to actually RENDER alpha-PNG/HEIC would need an in-worker decoder (ImageScript) — deferred; placeholder is the safe floor and satisfies "render OR report." Also the client-side `file_refs` log from the prior commit is still in `DrawPackagePickerModal.handleSave` — harmless, can stay for future diagnosis.
