...
**STANDING DATA QUESTION (Phase D, business fact — not a code decision):** How does Avenstone sub countertops (cabinet installer / dedicated stone shop / tile sub) and polished-concrete floors? Determines scope_option_trades rows for `countertop`, `counter_edge`, `basement.concrete_polished` — currently Unassigned (trade NULL, rendered in the packet's "Unassigned — confirm trade" section). Do NOT invent a Countertops trade in code; this is Kalin's field call. Once decided: add the scope_option_trades rows (and, if a new trade is chosen, a trade_taxonomy row first).
