      ...
      <div className="app">
        ...
        <div className="main">
          ...
          <div className="pg-wrap">
            {pg === 'home' && profile?.role === 'owner' && <OwnerHomeScr profile={profile} onOpenJob={id => { setPendingJobId(id); setPg('jobs'); }} onNavigate={setPg} onOpenWalkthrough={(jobId, workType, todoId) => setWalkthroughProps({ jobId, workType, todoId })} setPendingAction={action => { setPendingAction(action); if (action?.kind === 'job_create') setPg('jobs'); else if (action?.jobId) { setPendingJobId(action.jobId); setPg('jobs'); } }} />}
            {pg === 'projects' && profile?.role === 'owner' && <ProjectsListScr profile={profile} jobs={jobs} setJobs={setJobs} onNewProject={() => { setPg('jobs'); setPendingNew(true); }} pendingAction={pendingAction} clearPendingAction={() => setPendingAction(null)} pendingTab={pendingTab} clearPendingTab={() => setPendingTab(null)} onTabChange={t => setViewportTab(t)} onAgentDrawPoke={data => setPendingAction({ kind: 'agent_draw_poke', ...data })} onOpenWalkthrough={(jobId, workType, todoId) => setWalkthroughProps({ jobId, workType, todoId })} onJobOpen={id => { if (jobs.some(j => j.id === id)) setViewportJobId(id); }} onJobClose={() => setViewportJobId(null)} />}
            ...
            {pg === 'leads' && isOwnerOrRep && <LeadsScr profile={profile} onConvertToJob={c => { setPg('jobs'); setPendingNew(true); }} onOpenLead={id => { setPendingJobId(id); setPg('jobs'); }} onNewLead={() => { setPg('jobs'); setPendingNew(true); }} />}
