...
function scoreBg(score) {
  ...
  if (score >= 60) return 'var(--amber-bg)';
  return 'var(--red-bg)';
}
...
function EmptyState({ msg }) {
  return <div style={{ textAlign: 'center', padding: 40, color: 'var(--text-subtle)', fontSize: 14 }}>{msg}</div>;
}
...
function IntelligenceTab({ profile }) {
  ...
  useEffect(() => {
    ...
    Promise.all([sbLoadSubPerformance(tid), sbLoadJobOutcomes(tid), sbLoadBidAnalytics(tid)])
      .then(([sp, jo, ba]) => { setSubPerf(sp); setOutcomes(jo); setBids(ba); setLoading(false); })
      .catch(e => { setError(e.message || 'Failed to load'); setLoading(false); });
  }, []);

  if (loading) return <Spinner />;
  if (error) return <div style={{ background: 'var(--red-bg)', color: 'var(--red-text-strong)', padding: 12, borderRadius: 8, margin: 16 }}>{error}</div>;
    ...
    <div style={{ padding: '16px 0', display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12 }}>
        {[
          ...
          <div key={k.lb} className="card" style={{ textAlign: 'center', padding: '14px 12px' }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-subtle)', marginTop: 4, textTransform: 'uppercase', letterSpacing: 0.5 }}>{k.lb}</div>
          </div>
        ))}
      </div>
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ padding: '14px 16px', borderBottom: `1px solid ${BORDER}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-subtle)' }}>{subPerf.length} contractors</div>
        </div>
        {subPerf.length === 0 ? (
          ...
          <div style={{ overflowX: 'auto' }}>
            <table className="tbl" style={{ width: '100%' }}>
              <thead>
                <tr>
                  {['Name', 'Trade', 'Score', 'Jobs', 'On-Time %', 'COs Caused'].map(h => (
                    <th key={h} style={{ textAlign: h === 'Name' || h === 'Trade' ? 'left' : 'center', padding: h === 'Name' ? '10px 16px' : '10px 12px', fontSize: 11, color: 'var(--text-subtle)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5, background: CREAM }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {subPerf.map(s => {
                  ...
                  return (
                    <tr key={s.id} style={{ borderBottom: `1px solid ${BORDER}` }}>
                      <td style={{ padding: '10px 16px', fontSize: 13, color: NAVY, fontWeight: 500 }}>{s.profiles?.full_name || '—'}</td>
                      <td style={{ padding: '10px 12px', fontSize: 13, color: 'var(--text-muted)' }}>{s.trade || '—'}</td>
                      <td style={{ padding: '10px 12px', textAlign: 'center' }}>
                        <span style={{ background: scoreBg(s.score), color: scoreColor(s.score), fontWeight: 700, fontSize: 13, padding: '2px 10px', borderRadius: 12 }}>{s.score}</span>
                      </td>
                      <td style={{ padding: '10px 12px', textAlign: 'center', fontSize: 13, color: 'var(--text-muted)' }}>{s.jobs_completed}</td>
                      <td style={{ padding: '10px 12px', textAlign: 'center', fontSize: 13, color: 'var(--text-muted)' }}>{onTimePct === '—' ? '—' : `${onTimePct}%`}</td>
                      <td style={{ padding: '10px 12px', textAlign: 'center', fontSize: 13, color: s.cos_caused > 0 ? 'var(--red-text)' : 'var(--text-muted)', fontWeight: s.cos_caused > 0 ? 600 : 400 }}>{s.cos_caused}</td>
                    </tr>
                  );
                })}
              </tbody>
...
function EscalationsTab({ profile }) {
  ...
  useEffect(() => {
    const tid = profile.tenant_id || AV_TENANT;
    sbLoadOwnerEscalations(tid).then(d => { setItems(d); setLoading(false); }).catch(e => { setError(e.message || 'Failed to load'); setLoading(false); });
  }, []);
  ...
  const answer = async item => {
    ...
    if (!resp.trim()) return;
    setWorking(item.id);
    await sb.from('owner_escalations').update({ status: 'answered', owner_response: resp, answered_at: new Date().toISOString() }).eq('id', item.id);
  ...
  return (
    <div style={{ padding: '16px 0' }}>
      {toast && <div style={{ background: 'var(--green-bg)', color: 'var(--green-text-strong)', padding: '10px 14px', borderRadius: 8, marginBottom: 16, fontSize: 13 }}>{toast}</div>}
      {items.length === 0 ? (
          ...
          {items.map(item => (
            <div key={item.id} className="card" style={{ padding: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                <div>
                  <div style={{ fontSize: 11, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 }}>{item.source_agent}</div>
                  {item.jobs?.address && <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 4 }}>{item.jobs.address}</div>}
                  <div style={{ fontSize: 11, color: 'var(--text-subtle)' }}>{new Date(item.created_at).toLocaleDateString()}</div>
                </div>
              </div>
              <div style={{ fontSize: 14, color: NAVY, fontWeight: 500, marginBottom: 10, lineHeight: 1.5 }}>{item.question}</div>
              {item.context && (
                <div style={{ marginBottom: 10 }}>
                  <button onClick={() => setExpanded(p => ({ ...p, [item.id]: !p[item.id] }))} style={{ background: 'none', border: 'none', color: GOLD, fontSize: 12, cursor: 'pointer', padding: 0, fontWeight: 600 }}>
                    {expanded[item.id] ? 'Hide context' : 'Show context'}
                  </button>
                  {expanded[item.id] && <div style={{ marginTop: 8, fontSize: 13, color: 'var(--text-muted)', background: CREAM, padding: '8px 12px', borderRadius: 6, lineHeight: 1.5 }}>{item.context}</div>}
                </div>
              )}
              <textarea
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button className="btn btn-navy" disabled={working === item.id || !responses[item.id]?.trim()} onClick={() => answer(item)} style={{ flex: 1 }}>
                  {working === item.id ? 'Saving...' : 'Answer'}
                </button>
                <button className="btn btn-ghost" disabled={working === item.id + '_dismiss'} onClick={() => dismiss(item)}>Dismiss</button>
              </div>
            </div>
          ))}
        </div>
...
function PreviewTab({ profile }) {
  ...
  const launchSub = () => { const p = subs.find(s => s.id === selectedSub); if (!p) return; setPreviewType('sub'); setPreviewProfile(p); };
  const launchClient = () => { const p = clients.find(c => c.id === selectedClient); if (!p) return; setPreviewType('client'); setPreviewProfile(p); };
  ...
  return (
    <div style={{ padding: '16px 0', display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div className="card" style={{ padding: 20 }}>
        ...
        <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 12 }}>See the app exactly as a subcontractor sees it.</div>
        {subs.length === 0 ? (
          <div style={{ fontSize: 13, color: 'var(--text-subtle)' }}>No contractors in this account.</div>
        ) : (
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            <select className="finp" value={selectedSub} onChange={e => setSelectedSub(e.target.value)} style={{ flex: 1, minWidth: 180 }}>
              <option value="">Select contractor...</option>
              {subs.map(s => <option key={s.id} value={s.id}>{s.full_name}</option>)}
            </select>
            <button className="btn btn-navy" onClick={launchSub} disabled={!selectedSub}>Preview as Sub</button>
          </div>
        )}
      </div>
      ...
      <div className="card" style={{ padding: 20 }}>
        ...
        <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 12 }}>See the app exactly as a client sees it.</div>
        {clients.length === 0 ? (
          <div style={{ fontSize: 13, color: 'var(--text-subtle)' }}>No clients in this account.</div>
        ) : (
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            <select className="finp" value={selectedClient} onChange={e => setSelectedClient(e.target.value)} style={{ flex: 1, minWidth: 180 }}>
              <option value="">Select client...</option>
              {clients.map(c => <option key={c.id} value={c.id}>{c.full_name}</option>)}
            </select>
            <button className="btn btn-navy" onClick={launchClient} disabled={!selectedClient}>Preview as Client</button>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div style={{ padding: '16px 20px', maxWidth: 960, margin: '0 auto' }}>
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ fontSize: 13, color: 'var(--text-subtle)', marginTop: 4 }}>Intelligence, approvals, and team visibility</div>
      </div>

      <div style={{ display: 'flex', gap: 4, marginBottom: 20, borderBottom: `2px solid ${BORDER}`, overflowX: 'auto' }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setActiveTab(t.id)} style={{ padding: '8px 16px', background: 'none', border: 'none', borderBottom: activeTab === t.id ? `2px solid ${NAVY}` : '2px solid transparent', marginBottom: -2, color: activeTab === t.id ? NAVY : 'var(--text-subtle)', fontWeight: activeTab === t.id ? 700 : 400, fontSize: 13, cursor: 'pointer', whiteSpace: 'nowrap', transition: 'all 0.15s' }}>
            {t.lb}
          </button>
        ))}
      </div>
