      ...
      <div className="modal" style={{ maxWidth: 560, maxHeight: '90vh', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
        {step === 'review' && <>
          ...
          <div style={{ overflowY: 'auto', flex: 1, marginBottom: 12 }}>
            <pre style={{ fontSize: 11, lineHeight: 1.6, whiteSpace: 'pre-wrap', color: 'var(--text-secondary)', fontFamily: 'sans-serif' }}>{completionText}</pre>
          </div>
        ...
        {step === 'sign' && <>
          ...
          <div style={{ marginBottom: 12, fontSize: 13, color: 'var(--text-muted)' }}>By signing, you confirm the work at <strong>{job.address}</strong> is complete and satisfactory.</div>
          <SignaturePad onSave={submit} onCancel={() => setStep('review')} label="Draw your signature" />
          {saving && <div style={{ textAlign: 'center', fontSize: 13, color: 'var(--text-subtle)', marginTop: 12 }}>Saving...</div>}
        </>}
        {step === 'done' && <>
          <div style={{ padding: '24px 0', textAlign: 'center' }}>
            ...
            <div style={{ fontWeight: 600, color: 'var(--navy-900)', marginBottom: 4 }}>Sign-off Complete</div>
            <div style={{ fontSize: 13, color: 'var(--text-subtle)', marginBottom: 16 }}>PDF saved to documents.</div>
