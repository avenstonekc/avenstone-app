...
export default function JobsScr({ jobs, setJobs, onBack, pendingJobId, clearPendingJobId, profile, openNew, clearOpenNew, clearSel, pendingAction, clearPendingAction, onJobOpen, onJobClose, pendingTab, clearPendingTab, onTabChange }) {
  ...
  if (selJ) return (
    <ErrorBoundary back={() => setSel(null)}>
      <JobDet job={selJ} upd={ch => upd(selJ.id, ch)} del={() => del(selJ.id)} back={() => setSel(null)} profile={profile} pendingAction={pendingAction} clearPendingAction={clearPendingAction} pendingTab={pendingTab} clearPendingTab={clearPendingTab} onTabChange={onTabChange} />
    </ErrorBoundary>
  );
