...
function SubSection({ label, files, onSelectFile, bulkTagMode, selectedFileIds, onToggleSelect }) {
    ...
    <div style={{ marginBottom: 2 }}>
      <button
        ...
        style={{
          ...
          display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px 6px 20px',
...
function CatSection({ category, subs, onSelectFile, bulkTagMode, selectedFileIds, onToggleSelect }) {
  const [open, setOpen] = useState(true);
  ...
  return (
    <div style={{ border: '1px solid #E8E4DC', borderRadius: 8, marginBottom: 8, overflow: 'hidden', background: '#fff' }}>
      <button
        onClick={() => setOpen(v => !v)}
        style={{
          width: '100%', background: '#F7F5F0', border: 'none', cursor: 'pointer',
...
export default function FilesTreeView({ files, onSelectFile, bulkTagMode, selectedFileIds, onToggleSelect }) {
