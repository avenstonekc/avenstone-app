...
import { AV_USER_ID, sbCO, sbUpdCO, sbNotify, sbNotifyUser, sbSendContractEmail, sbPhoto, sbCountPhotosForEntity } from '../../../lib/supabase';
...
export default function COTab({ job, upd, profile }) {
  ...
  const addCO = async () => {
    ...
    if (s.ok) {
      // Upload condition photos linked to the new CO id
  ...
  const apCO = async id => {
    // Check fix photo count before approving
    const fixCount = await sbCountPhotosForEntity('change_order', id, 'co_fix');
    const needsFix = fixCount < fixMin;
    if (needsFix && !canBypass) {
      setCoErr(`At least ${fixMin} fix photo required before approval. Ask your PM or owner.`);
      return;
    }
    if (needsFix && canBypass) {
  ...
  const confirmFixGate = async () => {
    ...
    await _doApCO(fixGate.coId, needsBypass ? fixBypassReason.trim() : null);
    closeFixGate();
    setSavFix(false);
  };
  ...
  return (
    <div>
      ...
      {/* ── Fix gate modal (approve path, owner/PM only) ── */}
      {fixGate && (
        <div className="overlay" onClick={closeFixGate}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-title">Approve CO — Fix Photo Required</div>
            {fixGate.co && (
            ...
            <p style={{ fontSize: 12, color: '#6B7280', marginBottom: 12, margin: '0 0 12px' }}>
              Attach at least 1 photo showing the completed fix, or add a bypass reason.
            </p>
            ...
            <div className="fg">
              ...
              <button
                ...
                {showFixBypass ? 'Hide bypass' : 'Approve without fix photo'}
              </button>
              {showFixBypass && (
                <textarea
                  className="finp"
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
              ...
              <button className="btn btn-gold" style={{ flex: 1 }} onClick={confirmFixGate} disabled={savFix}>
                {savFix ? 'Approving...' : 'Confirm Approve'}
              </button>
            </div>
          </div>
        </div>
