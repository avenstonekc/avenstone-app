      ...
      {job.cost_plus && (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--navy-900)' }}>Draw Schedule</div>
              <div style={{ fontSize: 11, color: 'var(--text-subtle)', marginTop: 2 }}>Planned milestone payments for this job</div>
            </div>
          ...
          {loading ? (
            <div style={{ textAlign: 'center', padding: 32, color: 'var(--text-subtle)', fontSize: 13 }}>Loading...</div>
          ) : draws.length === 0 ? (
            <div style={{ background: '#fff', border: '1px solid #E8E4DC', borderRadius: 8, padding: 32, textAlign: 'center' }}>
              <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 12 }}>No draws planned yet. Add a draw to start the billing schedule for this job.</div>
            ...
            <>
                ...
                {draws.map(draw => {
                  ...
                  return (
                    <div key={draw.id} style={{ background: '#fff', border: '1px solid #E8E4DC', borderRadius: 8, padding: '12px 14px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                        <div style={{ flex: 1 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                            <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-subtle)' }}>#{draw.draw_number}</span>
                          ...
                          {draw.phase && <div style={{ fontSize: 11, color: 'var(--text-subtle)', marginTop: 2 }}>{draw.phase}</div>}
                          {draw.description && <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>{draw.description}</div>}
                        </div>
                        {staff && (
                          <div style={{ display: 'flex', gap: 6, flexShrink: 0, marginLeft: 8, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                            ...
                            <button onClick={() => handleDeleteDraw(draw)} style={{ fontSize: 11, padding: '4px 10px', background: 'none', border: '1px solid #fca5a5', color: 'var(--red-text)', borderRadius: 6, cursor: 'pointer', fontWeight: 600 }}>Delete</button>
                          </div>
                        )}
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
                        {[
                          ['Target', f$(draw.target_amount), 'var(--navy-900)'],
                          ['Paid', draw.paid_amount > 0 ? f$(draw.paid_amount) : '—', draw.paid_amount > 0 ? 'var(--green-dot)' : 'var(--text-muted)'],
                        ].map(([lb, val, c]) => (
                      ...
                      {draw.target_date && (
                        <div style={{ fontSize: 11, color: 'var(--text-subtle)', marginTop: 8 }}>Due: {fD(draw.target_date)}</div>
                      )}
                      {/* Inline send panel */}
                      {sendDrawId === draw.id && pkg?.generated_pdf_path && (
                        <div style={{ marginTop: 10, padding: '10px 12px', background: '#F0F9FF', borderRadius: 6, border: '1px solid #BAE6FD' }}>
                          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--navy-900)', marginBottom: 8 }}>Send Draw #{draw.draw_number} Package</div>
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
                            <div>
                              <div style={{ fontSize: 10, color: 'var(--text-muted)', marginBottom: 3 }}>Recipient email *</div>
                            ...
                            <div>
                              <div style={{ fontSize: 10, color: 'var(--text-muted)', marginBottom: 3 }}>Recipient label</div>
              ...
              <div style={{ background: '#F7F5F0', border: '1px solid #E8E4DC', borderRadius: 6, padding: '10px 14px', fontSize: 12, color: 'var(--text-muted)', display: 'flex', gap: 16, flexWrap: 'wrap', marginBottom: 24 }}>
                <span>Total scheduled: <strong style={{ color: 'var(--navy-900)' }}>{f$(totalScheduled)}</strong></span>
                <span>Total paid: <strong style={{ color: totalPaid > 0 ? 'var(--green-dot)' : 'var(--text-muted)' }}>{f$(totalPaid)}</strong></span>
              </div>
            </>
          )}
        </>
      ...
      {!job.cost_plus && (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--navy-900)' }}>Invoices</div>
              <div style={{ fontSize: 11, color: 'var(--text-subtle)', marginTop: 2 }}>All invoices for this job</div>
            </div>
            {staff && (
          ...
          {!loading && invoices.length === 0 ? (
            <div style={{ background: '#fff', border: '1px solid #E8E4DC', borderRadius: 8, padding: 32, textAlign: 'center' }}>
              <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 12 }}>No invoices yet. Create one to bill the client.</div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 4 }}>
                          <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--navy-900)' }}>{inv.invoice_number}</span>
                          <span style={{ fontSize: 10, background: st.bg, color: st.color, padding: '2px 8px', borderRadius: 10, fontWeight: 700 }}>{st.label}</span>
                        </div>
                        <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                          <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>Date: <strong style={{ color: 'var(--navy-900)' }}>{fD(inv.invoice_date)}</strong></span>
                          {inv.due_date && <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>Due: <strong style={{ color: 'var(--navy-900)' }}>{fD(inv.due_date)}</strong></span>}
                          <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>Total: <strong style={{ color: 'var(--navy-900)' }}>{f$(inv.total_amount)}</strong></span>
                          {inv.amount_paid > 0 && (
                            <span style={{ fontSize: 12, color: 'var(--green-dot)' }}>Paid: <strong>{f$(inv.amount_paid)}</strong></span>
                          )}
                        </div>
                      </div>
                        ...
                        <div style={{ display: 'flex', gap: 6, flexShrink: 0, marginLeft: 8, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                          {inv.status === 'draft' && (
                            <>
                              ...
                              <button onClick={() => handleDeleteInvoice(inv)} style={{ fontSize: 11, padding: '4px 10px', background: 'none', border: '1px solid #fca5a5', color: 'var(--red-text)', borderRadius: 6, cursor: 'pointer', fontWeight: 600 }}>Delete</button>
                            </>
                          )}
                          ...
                          {['sent', 'viewed', 'overdue'].includes(inv.status) && (
                            <button onClick={() => handleVoidInvoice(inv)} style={{ fontSize: 11, padding: '4px 10px', background: 'none', border: '1px solid #fca5a5', color: 'var(--red-text)', borderRadius: 6, cursor: 'pointer', fontWeight: 600 }}>Void</button>
                          )}
                        </div>
                      )}
                    </div>
