...
const STATUS_CFG = {
  pending_review:  { bg: 'var(--neutral-bg)',  color: 'var(--text-secondary)',    label: 'Pending Review' },
  approved:        { bg: 'var(--blue-bg)',      color: 'var(--blue-text)',         label: 'Approved' },
  partially_paid:  { bg: 'var(--amber-bg)',     color: 'var(--amber-text-strong)', label: 'Partial' },
  paid:            { bg: 'var(--green-bg)',      color: 'var(--green-text-strong)', label: 'Paid' },
  disputed:        { bg: 'var(--red-bg)',        color: 'var(--red-text)',          label: '🚩 Disputed' },
  voided:          { bg: 'var(--neutral-bg)',   color: 'var(--text-subtle)',        label: 'Voided' },
};
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start',
        ...
        <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--navy-900)' }}>Sub Invoices</div>
        <button
          onClick={() => setShowAddInvoice(true)}
          style={{ fontSize: 12, padding: '6px 14px', background: 'var(--navy-900)', color: 'var(--gold-500)',
      ...
      {invoices.length > 0 && (
        <div style={{ display: 'flex', gap: 24, padding: '12px 0',
          ...
          <div>
            ...
            <div style={{ fontSize: 20, fontWeight: 600, color: 'var(--navy-900)' }}>{f$(rollup.pendingTotal)}</div>
          ...
          <div>
            ...
            <div style={{ fontSize: 20, fontWeight: 600,
              color: rollup.outstandingTotal > 0 ? '#b45309' : 'var(--navy-900)' }}>{f$(rollup.outstandingTotal)}</div>
            <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 1 }}>
      ...
      <div style={{ display: 'flex', gap: 0, marginBottom: 14, borderBottom: '2px solid #E8E4DC',
        ...
        {viewList.map(v => (
          <button key={v.id} onClick={() => setView(v.id)} style={{
            ...
            background: 'none', borderBottom: `2px solid ${view === v.id ? 'var(--gold-500)' : 'transparent'}`,
            marginBottom: -2, color: view === v.id ? 'var(--navy-900)' : 'var(--text-subtle)', whiteSpace: 'nowrap',
            flexShrink: 0,
          }}>{v.label}</button>
        ))}
      </div>
  ...
  return (
    ...
    >
      <div style={{ flex: 1, minWidth: 120 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--navy-900)', display: 'flex', alignItems: 'center', gap: 6 }}>
          #{inv.invoiceNumber}
          {inv.autoGeneratedNumber && (
            <span title="Auto-generated number" style={{ fontSize: 10, color: 'var(--text-subtle)' }}>🤖</span>
          )}
        </div>
        <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>{inv.subName}</div>
      </div>
      <div style={{ fontSize: 11, color: 'var(--text-subtle)', flexShrink: 0 }}>{inv.invoiceDate}</div>
      <div style={{ textAlign: 'right', flexShrink: 0 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--navy-900)' }}>{f$(inv.amount)}</div>
        {inv.balance > 0 && inv.status !== 'pending_review' && (
          <div style={{ fontSize: 10, color: 'var(--gold-500)' }}>Balance {f$(inv.balance)}</div>
        )}
      </div>
      ...
      <div className="modal" style={{ maxWidth: 520, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
          <div>
            <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--navy-900)' }}>
              ...
              {inv.autoGeneratedNumber && <span style={{ fontSize: 11, color: 'var(--text-subtle)', marginLeft: 6 }}>auto-generated</span>}
            </div>
            <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 2 }}>{inv.subName}</div>
          </div>
        ...
        <div style={{ background: 'var(--bg)', borderRadius: 6, padding: '10px 12px', marginBottom: 14, fontSize: 12 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
            {[
              ...
              <div key={lb}>
                <div style={{ fontSize: 10, color: 'var(--text-subtle)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5 }}>{lb}</div>
                <div style={{ color: 'var(--navy-900)', fontWeight: 600, marginTop: 1 }}>{val}</div>
              </div>
            ))}
          </div>
          {inv.description && (
            <div style={{ marginTop: 8, color: 'var(--text-secondary)', borderTop: '1px solid #E8E4DC', paddingTop: 6 }}>
              {inv.description}
            </div>
          )}
          {inv.invoiceFileId && (
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ border: '1px solid #E8E4DC', borderRadius: 4, overflow: 'hidden', fontSize: 12 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '3fr 1fr 1fr 1fr',
                background: 'var(--bg)', padding: '6px 10px', fontWeight: 700, color: 'var(--text-secondary)', fontSize: 10 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                padding: '6px 10px', background: 'var(--bg)', borderTop: '1px solid #E8E4DC' }}>
                <button onClick={() => setEditRows(r => [...r, BLANK_ROW()])}
                  style={{ fontSize: 11, padding: '3px 10px', background: 'var(--navy-900)', color: 'var(--gold-500)',
                    border: 'none', borderRadius: 4, cursor: 'pointer', fontWeight: 600 }}>+ Row</button>
                <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--navy-900)' }}>
                  Total: {f$(editTotal)}
                </span>
              </div>
            </div>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ border: '1px solid #E8E4DC', borderRadius: 4, overflow: 'hidden', fontSize: 12 }}>
                ...
                {inv.lineItems.map((li, i) => (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '3fr 1fr 1fr 1fr',
                    padding: '6px 10px', borderTop: '1px solid #F3F0EB', color: 'var(--text-secondary)' }}>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          {inv.payments.length === 0 ? (
            <div style={{ color: 'var(--text-subtle)', fontSize: 12, padding: '8px 0' }}>No payments recorded.</div>
          ) : (
            <div style={{ border: '1px solid #E8E4DC', borderRadius: 4, overflow: 'hidden' }}>
              {inv.payments.map(p => (
                <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px',
                  ...
                  <div style={{ flex: 1, textDecoration: p.voidedAt ? 'line-through' : 'none', color: 'var(--text-secondary)' }}>
                    <span style={{ fontWeight: 600 }}>{f$(p.amount)}</span>
                    <span style={{ color: 'var(--text-subtle)', marginLeft: 6 }}>
                      {METHOD_LABELS[p.method] || p.method}
                      {p.reference ? ` · ${p.reference}` : ''}
                    </span>
                    <div style={{ fontSize: 10, color: 'var(--text-subtle)', marginTop: 1 }}>{p.paidDate}</div>
                  </div>
                  {p.voidedAt ? (
                    <span style={{ fontSize: 10, background: '#F3F4F6', color: 'var(--text-subtle)',
                    ...
                    canManage && (
                      <button onClick={() => onVoidPayment(p)} style={{
                        ...
                        borderRadius: 4, cursor: 'pointer', color: 'var(--text-subtle)' }}>Void</button>
                    )
                  )}
                </div>
              ))}
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 24, padding: '8px 10px',
                ...
                <span style={{ color: 'var(--text-secondary)' }}>Paid: {f$(inv.paidSum)}</span>
                <span style={{ color: inv.balance > 0 ? 'var(--gold-500)' : 'var(--green-dot)' }}>
          ...
          {/* Overpayment warning */}
          {overPaid && (
            <div style={{ background: '#FEF3C7', border: '1px solid #FCD34D', color: 'var(--amber-text-strong)',
      ...
      <div className="overlay">
        <div className="modal" style={{ maxWidth: 560, maxHeight: '92vh', overflowY: 'auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            ...
            <button onClick={onClose} style={{ background: 'none', border: 'none', fontSize: 20,
              color: 'var(--text-subtle)', cursor: 'pointer' }}>×</button>
          </div>
          ...
          {/* ── File upload + AI extract ── */}
          <div style={{ background: 'var(--bg)', border: '1px solid #E8E4DC', borderRadius: 6,
            padding: '10px 14px', marginBottom: 14 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', marginBottom: 6 }}>
              Invoice File <span style={{ fontWeight: 400, color: 'var(--text-subtle)' }}>(optional — PDF or image; fields auto-extracted by AI)</span>
            </div>
            {uploadedFileName ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>📎 {uploadedFileName}</span>
                {extracting ? (
                  <span style={{ fontSize: 11, color: 'var(--gold-500)' }}>✨ Extracting fields…</span>
                ) : (
                  <span style={{ fontSize: 11, color: 'var(--green-dot)' }}>✓ Fields pre-filled — review below</span>
                )}
              ...
              <div style={{ fontSize: 12, color: 'var(--text-subtle)' }}>Uploading…</div>
            ) : (
          ...
          <div className="fg">
            ...
            {subInput && !subContactId && (
              <div style={{ fontSize: 10, color: 'var(--gold-500)', marginTop: 3 }}>
          ...
          {/* Invoice # */}
          <div className="fg">
            <label className="flbl">Invoice # <span style={{ color: 'var(--text-subtle)' }}>(leave blank to auto-generate)</span></label>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            ...
            <div className="fg">
              <label className="flbl">Due Date <span style={{ color: 'var(--text-subtle)' }}>(opt.)</span></label>
          ...
          {/* Description */}
          <div className="fg">
            <label className="flbl">Description <span style={{ color: 'var(--text-subtle)' }}>(optional)</span></label>
          ...
          <div style={{ marginBottom: 14 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)',
                ...
                Line Items <span style={{ fontWeight: 400, color: 'var(--text-subtle)', textTransform: 'none', letterSpacing: 0 }}>(optional)</span>
              </div>
            ...
            {lineItems.length > 0 && (
              <div style={{ border: '1px solid #E8E4DC', borderRadius: 4, overflow: 'hidden', fontSize: 12 }}>
                <div style={{ display: 'grid', gridTemplateColumns: '3fr 1fr 1fr 1fr 28px',
                  background: 'var(--bg)', padding: '5px 8px', fontWeight: 700, color: 'var(--text-secondary)', fontSize: 10 }}>
                ...
                {lineItemTotal > 0 && (
                  <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '5px 8px',
                    background: 'var(--bg)', fontSize: 12, fontWeight: 700,
                    borderTop: '1px solid #E8E4DC', color: 'var(--navy-900)' }}>
          ...
          {schedItems.length > 0 && (
            <div className="fg">
              <label className="flbl">Related Schedule Item <span style={{ color: 'var(--text-subtle)' }}>(optional)</span></label>
          ...
          <div className="modal" style={{ maxWidth: 360 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
              ...
              <button onClick={() => setShowNewSub(false)} style={{ background: 'none', border: 'none',
                fontSize: 20, color: 'var(--text-subtle)', cursor: 'pointer' }}>×</button>
            </div>
            {newSubErr && (
              <div style={{ background: 'var(--red-bg)', border: '1px solid #FECACA', color: '#DC2626',
            ...
            <div className="fg">
              <label className="flbl">Phone <span style={{ color: 'var(--text-subtle)' }}>(optional)</span></label>
            ...
            <div className="fg">
              <label className="flbl">Email <span style={{ color: 'var(--text-subtle)' }}>(optional)</span></label>
      ...
      <div className="modal" style={{ maxWidth: 400 }}>
        ...
        {warn === null && (
          <>
            ...
            <div className="fg">
              <label className="flbl">Reference <span style={{ color: 'var(--text-subtle)' }}>(check #, ACH ID, etc.)</span></label>
            ...
            <div className="fg">
              <label className="flbl">Notes <span style={{ color: 'var(--text-subtle)' }}>(optional)</span></label>
