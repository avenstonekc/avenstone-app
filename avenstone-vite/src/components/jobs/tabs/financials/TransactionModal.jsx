...
const STATUSES = ['draft','pending','paid','overdue','void','refunded'];

export default function TransactionModal({ mode: initialMode, tx, job, onClose, onSaved }) {
  const [mode, setMode] = useState(initialMode);
  const [form, setForm] = useState({
    direction: tx.direction || 'in',
    type: tx.type || 'client_payment',
    amount: tx.amount != null ? String(tx.amount) : '',
    description: tx.description || '',
    date_incurred: tx.date_incurred || new Date().toISOString().slice(0, 10),
  ...
  const setDir = dir => {
    ...
    setForm(p => ({ ...p, direction: dir, type: valid.includes(p.type) ? p.type : valid[0] }));
  };
  ...
  const uploadReceipt = async file => {
    ...
    if (!res.error && res.path) {
      if (mode === 'edit' && tx.id) await sbUpdateTransaction(tx.id, { receipt_url: res.path });
  ...
  const uploadLien = async file => {
    ...
    if (!res.error && res.path) {
      if (mode === 'edit' && tx.id) await sbUpdateTransaction(tx.id, { lien_waiver_url: res.path, lien_waiver_signed_date: new Date().toISOString().slice(0, 10) });
      setLienUrl(res.path);
    }
    setUploading(false);
  };
      ...
      <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 480, width: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
          <div className="modal-title" style={{ margin: 0 }}>
            {isView ? (TYPE_LABELS[tx.type] || tx.type) : mode === 'create' ? 'New Transaction' : 'Edit Transaction'}
          </div>
        ...
        {isView ? (
          ...
          <div>
            ...
            <div style={fg}>
              ...
              <textarea style={{ ...inp, minHeight: 60, resize: 'vertical' }} placeholder="Internal notes" value={form.notes} onChange={e => set('notes', e.target.value)} />
            </div>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
              ...
              <button onClick={save} disabled={saving} className="btn btn-navy" style={{ flex: 2 }}>{saving ? 'Saving...' : mode === 'create' ? 'Add Transaction' : 'Save Changes'}</button>
            </div>
          </div>
        )}
      </div>
