  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: 1 }}>Daily Field Logs</div>
      ...
      {/* Log list */}
      {!logsLoaded && <div style={{ textAlign: 'center', padding: 32, color: 'var(--text-subtle)', fontSize: 13 }}>Loading...</div>}
      ...
      {logs.map(l => (
        <div key={l.id} style={{ background: '#fff', border: `1px solid ${l.status !== 'approved' ? '#FDE68A' : 'var(--border)'}`, padding: 16, marginBottom: 10 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
            <div>
              <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--navy-900)' }}>{fD(l.log_date)}</div>
              <div style={{ fontSize: 12, color: 'var(--text-subtle)', marginTop: 2 }}>{l.author?.full_name || 'Unknown'}</div>
            </div>
            {l.status !== 'approved'
              ? <button className="btn btn-gold" style={{ fontSize: 11, padding: '4px 10px' }} onClick={() => openReview(l)}>Review & Send</button>
              : <div style={{ fontSize: 11, color: 'var(--green-dot)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5 }}>Sent</div>
            }
          </div>
          ...
          {l.client_message && (
            <div style={{ background: '#F7F5F0', border: '1px solid #E8E4DC', borderRadius: 6, padding: '8px 12px' }}>
              <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 4 }}>Client Update</div>
      ...
      {showLogForm && <div className="overlay" onClick={resetForm}><div className="modal" style={{ maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>Photos</div>
      ...
      {reviewLog && <div className="overlay" onClick={closeReview}><div className="modal" style={{ maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6 }}>Field Note</div>
          <div style={{ fontSize: 13, color: '#374151', lineHeight: 1.65, background: '#F7F5F0', border: '1px solid #E8E4DC', borderRadius: 6, padding: '10px 12px' }}>
            {reviewLog.work_completed || <span style={{ color: 'var(--text-subtle)' }}>No capture note</span>}
          </div>
        </div>

        {/* Photo curation */}
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6 }}>Photos — tap to include / exclude from client view</div>
          {reviewPhotosLoading && <div style={{ fontSize: 13, color: 'var(--text-subtle)', padding: '6px 0' }}>Loading photos...</div>}
          {!reviewPhotosLoading && !reviewPhotos.length && <div style={{ fontSize: 13, color: 'var(--text-subtle)' }}>No photos attached to this log.</div>}
              ...
              {reviewPhotos.map(ph => {
                ...
                return (
                  <div key={ph.id} className="pcell" style={{ cursor: 'pointer', opacity: included ? 1 : 0.35 }} onClick={() => togglePhotoVisible(ph.id, included)}>
                    ...
                    <div style={{ position: 'absolute', top: 5, right: 5, background: included ? 'var(--green-dot)' : 'rgba(0,0,0,0.65)', borderRadius: '50%', width: 20, height: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: '#fff', fontWeight: 700 }}>
                      {included ? '✓' : '✕'}
                    </div>
                  </div>
                );
        ...
        <div style={{ marginBottom: 12 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: 1 }}>Client Message</div>
