...
const ROLE_COLORS = {
  ...
  sales_rep:       { bg: '#D1FAE5', color: '#065F46' },
...
function VisibilityChips({ roles }) {
  if (!roles || roles.length === 0) {
    return <span style={{ color: '#9CA3AF', fontSize: 11 }}>Owner only</span>;
  }
  return (
...
function ExpirationBadge({ date }) {
  ...
  let color = '#059669', bg = '#D1FAE5';
  if (daysOut < 0)  { color = '#DC2626'; bg = '#FEE2E2'; }
  else if (daysOut <= 30) { color = '#92400E'; bg = '#FEF3C7'; }
  return (
  ...
  return (
    <div style={{ padding: mob ? '16px 14px' : '20px 24px', maxWidth: 860, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start',
        ...
        <div>
          <div style={{ fontFamily: "'DM Serif Display',serif", fontSize: 22, color: '#0A1F44' }}>Company Files</div>
          <div style={{ fontSize: 13, color: '#6B7280', marginTop: 3 }}>
            Compliance documents, licenses, and templates — visible to your team and portals.
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, color: '#6B7280',
  ...
  return (
    ...
    >
      <div style={{ flex: 1, minWidth: 140 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: '#0A1F44', display: 'flex',
          alignItems: 'center', gap: 6 }}>
          📄 {file.type}
          {isArchived && (
            <span style={{ background: '#F3F4F6', color: '#9CA3AF', fontSize: 10, fontWeight: 700,
              padding: '1px 6px', borderRadius: 10 }}>ARCHIVED</span>
          )}
        </div>
        <div style={{ fontSize: 11, color: '#6B7280', marginTop: 1 }}>{file.name}</div>
      </div>
    ...
    <div className="overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal" style={{ maxWidth: 500, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start',
          marginBottom: 16 }}>
          <div>
            <div style={{ fontSize: 15, fontWeight: 700, color: '#0A1F44' }}>{file.type}</div>
            <div style={{ fontSize: 12, color: '#6B7280', marginTop: 2 }}>{file.category}</div>
          </div>
        ...
        {file.lifecycleStatus === 'archived' && (
          <div style={{ background: '#F3F4F6', border: '1px solid #E5E7EB', color: '#6B7280',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
            {[
              ...
              <div key={lb}>
                ...
                <div style={{ color: '#0A1F44', fontWeight: 500, marginTop: 1, wordBreak: 'break-word' }}>{val}</div>
              </div>
            ))}
          </div>
          {file.expirationDate && (
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center',
            ...
            {canManage && !editingRoles && file.lifecycleStatus === 'active' && (
              <button onClick={() => setEditingRoles(true)} style={{ fontSize: 11, background: 'none',
                ...
                cursor: 'pointer', color: '#6B7280' }}>Edit</button>
            )}
          </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                ...
                {['project_manager', 'sales_rep', 'sub', 'client'].map(r => (
                  <label key={r} style={{ display: 'flex', alignItems: 'center', gap: 8,
                    ...
                    {r === 'client' && <span style={{ fontSize: 11, color: '#9CA3AF' }}>(client portal via job)</span>}
                    {r === 'sub' && <span style={{ fontSize: 11, color: '#9CA3AF' }}>(sub portal Company Docs)</span>}
                  </label>
                ))}
              </div>
        ...
        {/* Error */}
        {(viewErr || actionErr) && (
          <div style={{ background: '#FEE2E2', color: '#DC2626', padding: '6px 10px',
  ...
  return (
    <div className="overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal" style={{ maxWidth: 480, maxHeight: '92vh', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center',
          marginBottom: 16 }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: '#0A1F44' }}>Upload Company File</div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', fontSize: 20,
            color: '#9CA3AF', cursor: 'pointer', lineHeight: 1 }}>×</button>
        </div>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ border: '2px dashed #E8E4DC', borderRadius: 6, padding: '16px',
            ...
            {file
              ...
              : <span style={{ fontSize: 12, color: '#9CA3AF' }}>Click to choose a file (PDF, image, DOC)</span>
            }
          </div>
        </div>
        ...
        {/* Type */}
        <div style={{ marginBottom: 12 }}>
          <label className="flbl">Document Type * <span style={{ fontWeight: 400, color: '#9CA3AF' }}>(e.g. "GL Insurance", "W-9")</span></label>
