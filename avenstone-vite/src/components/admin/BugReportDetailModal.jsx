...
Do not touch unrelated files. Stay in scope. Do not opportunistically refactor.`;
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', overflowY: 'auto', padding: '40px 16px' }}
      ...
      <div style={{ background: 'var(--card-bg)', borderRadius: 12, width: '100%', maxWidth: 700, padding: 28, fontFamily: 'DM Sans, sans-serif', position: 'relative' }}>
        <button onClick={onClose} style={{ position: 'absolute', top: 16, right: 16, background: 'none', border: 'none', cursor: 'pointer', fontSize: 20, color: 'var(--text-subtle)', lineHeight: 1 }}>✕</button>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
          ...
          <span style={{ fontSize: 12, color: 'var(--text-subtle)' }}>{relativeTime(bug.created_at)} · {bug.route || '—'} · {bug.user_role}</span>
        </div>

        <div style={{ fontSize: 18, fontWeight: 700, color: NAV, marginBottom: 16, lineHeight: 1.4 }}>{bug.description}</div>

        {bug.screenshot_url && (
          <div style={{ marginBottom: 20 }}>
            <img src={bug.screenshot_url} alt="screenshot" loading="lazy"
              style={{ maxWidth: '100%', borderRadius: 8, border: `1px solid ${BORDER}` }} />
          </div>
        )}

        {breadcrumbs.length > 0 && (
          <div style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: NAV, marginBottom: 8 }}>Last actions</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {breadcrumbs.map((b, i) => (
                <div key={i} style={{ display: 'flex', gap: 8, fontSize: 12, color: 'var(--text-secondary)' }}>
                  <span style={{ color: 'var(--text-subtle)', flexShrink: 0 }}>{(b.ts || '').slice(11, 19)}</span>
                  <span style={{ fontWeight: 600, color: GOLD, flexShrink: 0 }}>{b.type}</span>
        ...
        {consoleErrors.length > 0 && (
          <details style={{ marginBottom: 16 }}>
            <summary style={{ fontSize: 13, fontWeight: 600, color: NAV, cursor: 'pointer', marginBottom: 6 }}>
              Console errors ({consoleErrors.length})
            </summary>
            <div style={{ background: '#FEF2F2', borderRadius: 6, padding: 10, fontFamily: 'monospace', fontSize: 11, color: 'var(--red-text-strong)' }}>
        ...
        <div style={{ fontSize: 11, color: 'var(--text-subtle)', marginBottom: 20 }}>
