...
- **Confirm card is the only commit point.** Every write verb in `CONFIRM_TOOLS` (currently 15: log_payment, log_receipt, submit_change_order, add_todo, create_job, notify_team_member, create_schedule_item, log_sub_invoice, log_sub_payment, approve_sub_invoice, upload_company_file, record_deposit, compose_draw, send_client_portal, create_document) returns `pending_action`. The agent never writes silently. New write tools must be added to this set unless deliberately excluded with a written reason. Total tools on the master surface: **29 (6 read + 23 write)**. Read tools (exempt from confirm): get_jobs, get_team, get_job_financials, get_schedule, get_open_todos, get_alerts. Write tools: 15 CONFIRM_TOOLS (listed above) + 8 auto-execute (add_note, advance_phase, update_phase, notify_team, update_job, add_contact, invite_person, add_knowledge). `send_client_portal` moved from auto-execute → CONFIRM_TOOLS 2026-07-02 (client-facing send; must be gated). `create_document` added 2026-07-18 (AGENT_DOCS Slice 1 — generates invoices/docs; invoice hits the invoices ledger). **NOTE (AVEN_MERGE B6.1): the tools now live in `ai-agent/masterCore.ts` (the merged fn's master surface), not the deprecated `ai-master-agent` slug.** Verified live from masterCore.ts TOOLS array 2026-07-18.
