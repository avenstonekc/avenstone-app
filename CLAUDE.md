...
- **Bathroom takeoff inputs**: shower dimensions in feet+inches (shower_width_in, shower_length_in, shower_wall_height_in — all stored as total inches). `resolveShowerSf()` in takeoff.js computes shower_wall_sf and shower_floor_sf from dimensions, with per-field override available (shower_wall_sf_override, shower_floor_sf_override). `ScopeDetailForm` mirrors the same math for live preview. Labor quantities never apply waste factor — materials only (materialRow.waste_pct on unit cost row).
