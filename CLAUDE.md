...
- `takeoff_unit_costs` — both labor and material rate rows, distinguished by `category` column (`labor` | `materials`). Labor rows: `unit`, `base_rate`, `multipliers` JSONB, `waste_pct`. Material rows: same plus `material_name`, `coverage_sf`. Platform defaults `tenant_id=NULL`; tenant override rows beat platform defaults in `buildTakeoffDraft` JS de-dup. **Rep rate edits in the Takeoff wizard write back as tenant override rows (tenant_id set) via `sbSaveTenantUnitCostOverride`. Platform-default rows are immutable. Gap-fill rates from the deterministic `price_plan` path do the same** (T2#4 S1) — a filled labor gap carrying a `unit_cost_key` (emitted by `draftLineToPricedLine`, `room_type` byte-identical to the loader's filter) routes through `sbSaveTenantUnitCostOverride` in `EstimateTab.saveLearnedRates`, so the engine sees the rate on the next run and the gap stops re-asking. `rate_book_labor` serves the legacy LLM pricing branch only — the deterministic engine never reads it. (Material gap-fill still uses the legacy path; it needs a coverage_sf decision before it can route here — see Known Gotchas.)
