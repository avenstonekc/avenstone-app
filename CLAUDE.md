...
- `takeoff_unit_costs` — both labor and material rate rows, distinguished by `category` column (`labor` | `materials`). Labor rows: `unit`, `base_rate`, `multipliers` JSONB, `waste_pct`. Material rows: same plus `material_name`, `coverage_sf`. **`room_type` is NULLABLE (T2#4 S2a): `NULL` = "applies to all room types" (Kalin's 2026-07-26 call — labor rates don't vary by room). Platform defaults `tenant_id=NULL`; tenant rows override.** Up to four rows can compete for one (trade[, material]) key — `buildCostMaps` ranks them `rank = (tenant?2:0)+(room_type!=null?1:0)`: **tenant+room-specific > tenant+all-rooms > platform+room-specific > platform+all-rooms.** Tenant always beats platform on the rate (locked #5); the best platform row stays the base (supplies `coverage_sf`/`waste_pct`/`unit`) while the best tenant row supplies `base_rate` — order-independent. The `price_plan` loader reads room-specific OR all-rooms (`room_type.eq.<pt>,room_type.is.null`), templates stay room-scoped. **`vetted` BOOL** (default false): platform defaults are `true` (curated), tenant rep-entered rows stay `false` until an owner clears them (B3.1/B3.2 gate; stored but not yet read). **Rep rate edits in the Takeoff wizard write back as tenant override rows via `sbSaveTenantUnitCostOverride` (now accepts `roomType=null` for all-rooms). Platform-default rows are immutable. Gap-fill rates from the deterministic `price_plan` path do the same** (T2#4 S1) — a filled labor gap carrying a `unit_cost_key` (emitted by `draftLineToPricedLine`, `room_type` byte-identical to the loader's filter) routes through `sbSaveTenantUnitCostOverride` in `EstimateTab.saveLearnedRates`, so the engine sees the rate on the next run and the gap stops re-asking. `rate_book_labor` serves the legacy LLM pricing branch only — the deterministic engine never reads it. (Material gap-fill still uses the legacy path; it needs a coverage_sf decision before it can route here — see Known Gotchas.) **`RateBookScr.jsx` (owner Rate Book) edits `takeoff_unit_costs`, not `rate_book_*` (T2#4 S2b).** It shows ONE row per (trade, category, material_name), collapsed across room types, and resolves the live rate with the SAME precedence rank as `buildCostMaps` — this is a DELIBERATE second copy of the rank rule, cross-referenced by comment in both `sbLoadTakeoffCatalog` (supabase.js) and `buildCostMaps` (pricingCore.js + `_shared`). Editing always writes an **all-rooms tenant override (`room_type = null`)** via `sbSaveTenantUnitCostOverride`; platform rows are never touched (`sbSetUnitCostVetted` / `sbDeleteTenantUnitCostOverride` refuse `tenant_id IS NULL`). When platform defaults disagree across room types the screen shows a range, not one number. Footgun surfaced: a room-specific tenant override (rank 3) shadows an all-rooms rate (rank 2) — the screen warns, names the room type(s), and offers a one-tap "use my all-rooms rate everywhere" that deletes the room-specific row(s). `rate_book_labor`/`rate_book_material` no longer have an editing surface (legacy LLM/flip branch only). **Gap-entry sanity rails (T2#5):** `priceSanity.checkGapEntry` runs as a rate is typed — SOFT (warn, never block). Rail 1 = live line total (`$250/sf × 49 = $12,250` vs lump-sum `whole line = $250`); Rail 2 = draft-share ≥25%; Rail 3 = unit-rate outlier vs the `takeoff_unit_costs` envelope (>5×/<0.2×), and **NO envelope → NO flag** (never invent a reference, #3). Wired into `GapBatchAsk` (all 3 rails) and `RateBookScr` (Rails 1+3; Rail 2 is estimate-scoped). `buildRateEnvelope` sources from `takeoff_unit_costs` (base_rate distribution across room contexts, precedence rank honored), NOT the retired `rate_book_labor`.
...
**`time_entries` (TIME_CLOCK_ARC S1):** one row per work segment. `user_id` UUID FK→profiles, `job_id` TEXT FK→jobs (jobs.id is TEXT), `clock_in`/`clock_out` TIMESTAMPTZ, `in_lat`/`in_lng`/`out_lat`/`out_lng` NUMERIC (nullable — GPS never blocks a punch), `source` TEXT CHECK (`punch`|`switch`|`manual`). **Invariant: partial unique index `time_entries_one_open_per_user ON (user_id) WHERE clock_out IS NULL` — a person is clocked into AT MOST one job.** RLS: crew SELECT/INSERT own; crew UPDATE only while OPEN (`clock_out IS NULL` in USING — no editing closed entries from the crew seat); owner/PM full tenant. Switching jobs goes through the atomic `time_clock_switch(p_job_id, p_lat, p_lng)` RPC (SECURITY DEFINER, identity from `auth.uid()`) — closes the open entry and opens a new `switch` one in one transaction so a crash can't leave a worker silently clocked out. HOURS ONLY — labor dollars are Slice 5; distance flagging is Slice 3 (coords stored, unjudged).

**Employee / pay tables (TIME_CLOCK_ARC S2):** pay data lives OFF `profiles` because the profiles SELECT policy is tenant-wide (every crew member could otherwise read each other's rate). `employee_details` (user_id PK, `classification` w2|1099, phone, address, start_date). `employee_pay_rates` (**APPEND-ONLY** — a raise inserts a new effective-dated row; UNIQUE(user_id, effective_date); **NO UPDATE policy** — a fat-finger is a same-day new row or owner DELETE). RLS both: **owner full write; each user SELECT their own; NO PM access, NO cross-crew reads.** Effective rate for an entry = the pay-rate row with the greatest `effective_date <= clock_in`'s America/Chicago local date. Earnings math (`lib/earnings.js`, pure) is **STRAIGHT-TIME ONLY (v1, no OT)** — entries with no rate on file COUNT hours but contribute NO dollars (shown "—", never $0-priced). Owner manages via `EmployeeModal` (Team → Add Employee / Pay & details); crew see YTD/week/rate on the CrewHomeScr **My Pay** tab, labeled "Straight time, before taxes — overtime premium not included." **OT revisit is mandatory before any W-2 crosses 40 hrs/week regularly** (the number is legally understated until then).

**Paperwork — fillable W-4/W-9 (TIME_CLOCK_ARC S2b):** owner sends per-person (no auto-nag); recipient fills a mobile HTML form + e-signs; `lib/irsForms.js` fills the OFFICIAL IRS PDF (`public/irs/fw4.pdf` 2026 / `fw9.pdf` Rev 3-2024) client-side and uploads to the **private `employee-docs`** bucket. **SECURITY: the TIN (SSN/EIN) never becomes DB data — no column, no extraction; it lives ONLY inside the signed PDF.** The DB stores only `paperwork_requests` (status/path/timestamps/sign_ip/sign_user_agent — owner + self RLS, no PM, self-completes-own-sent) and doc pointers (`employee_details.w4_path/w9_path` for crew; `profiles.w9_url` for subs — the SAME fillable W-9 replaces upload-only for subs, so SubComplianceModal keeps working). Completion is server-side (`record-paperwork-evidence`, service role) since `employee_details`/`profiles` are owner-write-only; it also captures the signer IP/UA. Signed URLs are short-expiry (≤10 min). `employee-docs` RLS: owner (tenant) + the person themselves (by `{user_id}/` folder). **`job-documents` was public=true (a live sub-W-9 SSN leak via unauthenticated URLs) — fixed to private in `20260729120000`.** **FORM-REVISION ROT: the IRS revs the W-4 every January — swap `public/irs/fw4.pdf` and re-run the field-name calibration (KALIN_QUEUE r).** I-9 is paper (out); W-2/1099-NEC are payroll-provider (never generated here).

**Storage buckets:** `employee-docs` (private — W-4/W-9 signed PDFs, owner+self by `{user_id}/` folder, TINs), `job-photos` (public), `job-documents` (private — **corrected from public=true, 2026-07-29**), `bid-quotes` (private), `bug-screenshots` (private — 1-year signed URLs via submit-bug-report edge fn), `job-receipts` (private — receipt/lien images for `job_transactions`; on upload, TransactionModal auto-extracts vendor/amount/date/description via the `ai-extract-sub-invoice` Haiku fn's `{ bucket, path }` input shape)
...
### Auth & Roles
- Roles: `owner`, `project_manager`, `sales_rep`, `sub`, `client`, `crew` (enforced by the `profiles_role_check` TEXT CHECK — no enum). `sub`, `client`, and `crew` each get a dedicated full-screen app via early-return in `App.jsx` (SubPortal / ClientPortal / **CrewHomeScr**), bypassing the staff shell — crew never inherits staff nav/todos/schedule/financials. `crew` is invitable via `STAFF_ROLES` (Team → Add member).
- **CrewHomeScr** (`components/crew/CrewHomeScr.jsx`, TIME_CLOCK_ARC S1) — the hourly crew member's entire app: Clock In → job picker → (Switch Job)* → Clock Out, phone-first. Punch helpers in `supabase.js`: `sbClockIn`/`sbSwitchJob`/`sbClockOut`/`sbMyOpenEntry`/`sbMyEntriesToday`/`sbCrewJobs`. GPS one-shot via `lib/gps.js stampGPS()`; **a GPS failure never blocks a punch** (coords go null). `sbSwitchJob` calls the atomic `time_clock_switch` RPC (close + open in one transaction).
...
- **Master Agent `send_client_portal` verb (2026-07-02):** provisions the client login AND delivers a set-password email via Resend. Agent path uses `send_recovery: true` in the `create-client-login` call — a server-side random password is generated (never returned to caller), then `auth.admin.generateLink({ type: 'recovery', email, redirectTo: 'https://avenstone-app.vercel.app' })` issues a recovery link, and Resend emails it. The client lands on `SetPasswordScr` (`App.jsx` renders it when URL hash contains `#type=recovery`) to set their own password. The agent never elicits, displays, or stores a client password. UI path (`ClientLoginButton`) is unchanged — PM still sets password directly. `password` field is NOT in the agent's tool schema or REQUIRED_FIELDS. `send_client_portal` is in CONFIRM_TOOLS — the confirm card shows client name, email, and job address; nothing credential-shaped.
- **PASSWORD_RECOVERY landing (verified 2026-07-02):** `SetPasswordScr` exists at `src/components/auth/SetPasswordScr.jsx`. App.jsx line 302 renders it when `INVITE_TYPE === 'recovery'` (from URL hash `#type=recovery`). `redirectTo: 'https://avenstone-app.vercel.app'` is confirmed in Supabase's allowlist (same URL used by LoginScr's `resetPasswordForEmail`). The recovery link lands correctly.
- `send-client-link` is now unreferenced by live code paths (grep-confirmed 2026-07-02); candidate for pre-launch integrity audit removal.
...
- **Confirm card is the only commit point.** Every write verb in `CONFIRM_TOOLS` (currently 16: log_payment, log_receipt, submit_change_order, add_todo, create_job, notify_team_member, create_schedule_item, log_sub_invoice, log_sub_payment, approve_sub_invoice, upload_company_file, record_deposit, compose_draw, send_client_portal, create_document, send_document) returns `pending_action`. The agent never writes silently. New write tools must be added to this set unless deliberately excluded with a written reason. Total tools on the master surface: **30 (6 read + 24 write)**. Read tools (exempt from confirm): get_jobs, get_team, get_job_financials, get_schedule, get_open_todos, get_alerts. Write tools: 16 CONFIRM_TOOLS (listed above) + 8 auto-execute (add_note, advance_phase, update_phase, notify_team, update_job, add_contact, invite_person, add_knowledge). `send_client_portal` moved from auto-execute → CONFIRM_TOOLS 2026-07-02 (client-facing send; must be gated). `create_document` added 2026-07-18 (AGENT_DOCS — one verb, doc_type ∈ {invoice, delivery_acceptance, damage_waiver, material_receipt, lien_waiver, letter}; invoice hits the invoices ledger, lien_waiver pulls attorney-approved `waiver_templates` with an unconditional payment gate, letter is model-composed + previewed on the card). `send_document` added 2026-07-18 (AGENT_DOCS Slice 4 — emails any job document/estimate to a verbatim address as a 7-day link; owner/PM only). **NOTE (AVEN_MERGE B6.1): the tools now live in `ai-agent/masterCore.ts` (the merged fn's master surface), not the deprecated `ai-master-agent` slug.** Verified live from masterCore.ts TOOLS array 2026-07-18.
...
1. **Floor plan PDF — match MagicPlan output, pull dimensions cleanly.** This is the visible field deliverable. Sub-tasks (all SHIPPED as of 2026-07-18 — LIDAR_CLOSEOUT):
   - **Fixture/object rendering — SHIPPED (FIXTURE_RENDER 2026-07-18; verified on real scans in LIDAR_CLOSEOUT).** Swift serializes `room.objects` (toilet, bathtub, sink, stove, oven, refrigerator, dishwasher, washerDryer, storage); `pdf.js _drawFixture` renders each as a muted schematic oriented by the `_processAllRooms` rotation transform. Confirmed rendering correctly on real DB scans (single-room, 2-room bath, 5-room 13-fixture).
   - **Room-name-backwards UX bug — RESOLVED (2026-07-18).** JS side: SCAN_SCOPE_CAPTURE names each room at capture time in scan order (returned array already re-sorted via `scanIndex`), mini-map sub-task RETIRED. Native side: `matchNamesByArea` (RoomPlanPlugin.swift) now breaks area ties by nearest centroid, so two similar-area rooms can no longer swap names before JS sees them. Both halves of the swap risk are closed.
   - **Single-room PDF parity — SHIPPED (2026-07-18, `497c025`).** `_renderFloorPage` gated `worldMode` on raw `rooms[i].worldX`, so single-room scans lacking worldX fell to the legacy per-seg packing path (no glyphs/chain-dims/floor-tint/fixtures) even when `normFloor` existed. Now gated on `!!normFloor || rooms.some(worldX)` — normalizable single-room scans render the full world-space path. Legacy packing remains the fallback only for scans normalize cannot place. Verified via node/esbuild harness + real "Basement" scan.
   - **Incomplete-scan capture warning — SHIPPED (2026-07-18).** At each room finish, RoomPlanPlugin.swift detects a wall-ring gap > ~3 ft and offers Rescan / Keep before the room-type picker, so the rep fixes a hole on the spot rather than in the PDF.
...
5. **Lien waiver generation** — ✅ SHIPPED 2026-07-18 (AGENT_DOCS Slice 2). pdf-lib via `_shared/docRender.ts`; `create_document` doc_type `lien_waiver` + `_shared/agentDocs.ts` `createWaiver`. Statutory text lives as DATA in `waiver_templates` (tenant × state MO|KS × conditional × final = 8 seeded rows, editable without code), **attorney-reviewed and signed off 2026-07-18 (no redlines)**. Unconditional variants (`requires_payment_gate`) are hard-gated: the executor refuses to render until `payment_received=true`, and the confirm card shows an amount read-back (amountToWords) + explicit "payment received?" prompt. Auto-fills amount/claimant from a linked `sub_invoices`/`job_transactions` payment and links the saved file back via `sub_invoices.lien_waiver_file_id` / `job_transactions.lien_waiver_url`.
...
- `tools/apply_migration.js` — atomic apply + verify wrapper. Run via `npm run migrate <path>` from `avenstone-vite/`. Applies the SQL via Supabase Management API, auto-derives expected objects (CREATE TABLE / ADD COLUMN / CREATE INDEX / CREATE POLICY) and verifies each present in `information_schema` / `pg_policies` / `pg_indexes`, then reloads schema. Exit 0 = all objects confirmed, 1 = apply fail or missing object, 2 = usage/PAT error. Flags: `--verify <objects>` (explicit override), `--selftest` (no-write PAT + API smoke test), `--help`. PAT read from `C:/Users/Kalin/supabase-token.txt`. This is the canonical migration apply path — replace any curl-based apply with this.
  ...
  **Supabase MCP (installed at user scope, read-only):** use for `execute_sql` READS only — schema inspection, verification queries, ad-hoc lookups. The `--read-only` flag sandboxes `execute_sql` at the postgres session level (verified 2026-07-16) but does NOT block the Management API tools the server also exposes: `apply_migration`, `deploy_edge_function`, `create_branch`, `merge_branch`, `reset_branch`, `rebase_branch`. NEVER use those — migrations go through `npm run migrate` (atomic apply+verify+reload), edge function deploys go through GitHub Actions. Playwright MCP is also installed: use for live UI verification walks.
