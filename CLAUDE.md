...
- **Scope detail forms** — bathroom scope tags (full_remodel, tile_only, vanity_swap, paint_and_floor) show a per-room detail form in ScopeTab. Schemas stored in `scope_detail_schemas` table (JSONB, keyed by room_type + scope_tag). Rep-filled values stored in `job_room_scopes.scope_details` JSONB. `fixture_select` fields (vanity, door, toilet, countertop) emit fixed line items at takeoff time. Number fields (shower_wall_sf, shower_floor_sf, floor_tile_sf) feed material formula evaluation via `scope_detail` qty_basis in `takeoff_templates.scope_definition`. Other room types have no detail forms yet.
