...
- **Confirm card is the only commit point.** Every write verb in `CONFIRM_TOOLS` (currently 14: log_payment, log_receipt, submit_change_order, add_todo, create_job, notify_team_member, create_schedule_item, log_sub_invoice, log_sub_payment, approve_sub_invoice, upload_company_file, record_deposit, compose_draw, send_client_portal) returns `pending_action` from ai-master-agent. The agent never writes silently. New write tools must be added to this set unless deliberately excluded with a written reason. Total tools in ai-master-agent: **28 (6 read + 22 write)**. Read tools (exempt from confirm): get_jobs, get_team, get_job_financials, get_schedule, get_open_todos, get_alerts. Write tools: 14 CONFIRM_TOOLS (listed above) + 8 auto-execute (add_note, advance_phase, update_phase, notify_team, update_job, add_contact, invite_person, add_knowledge). `send_client_portal` moved from auto-execute → CONFIRM_TOOLS 2026-07-02 (client-facing send now provisions + emails a recovery link; must be gated). Verified live from ai-master-agent/index.ts TOOLS array 2026-06-16 (send_client_portal delta 2026-07-02).
