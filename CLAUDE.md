...
**Storage buckets:** `job-photos` (public), `job-documents` (private), `bid-quotes` (private), `bug-screenshots` (private — 1-year signed URLs via submit-bug-report edge fn), `job-receipts` (private — receipt/lien images for `job_transactions`; on upload, TransactionModal auto-extracts vendor/amount/date/description via the `ai-extract-sub-invoice` Haiku fn's `{ bucket, path }` input shape)
