...
| r | **Each January: swap in the new-year W-4 template** (`public/irs/fw4.pdf`) and re-run the field-name calibration (fill each field with its id, rasterize, re-map in `lib/irsForms.js`). The IRS revs the W-4 annually; stale field names silently mis-fill. | W-4 fills go wrong after the annual IRS revision | 2026-07-29 |
...
| S2b — fillable W-4/W-9, owner-sent + e-signed (employees + subs) | **SHIPPED 2026-07-29** | Migration `20260729120000` (`paperwork_requests`, employee doc pointers, **fixed job-documents public→private**, private `employee-docs` bucket owner+self). `lib/irsForms.js` fills official IRS PDFs client-side (field-fill verified by rasterization); `PaperworkCard` recipient fill-and-sign; owner Send in EmployeeModal + SubComplianceModal; `send-paperwork-request` + `record-paperwork-evidence` fns. **TIN lives only in the PDF — never DB data.** I-9 out (paper); no W-2/1099-NEC generation. |
