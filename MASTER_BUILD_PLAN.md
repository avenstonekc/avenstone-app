...
> **Starting position: SCOPE_CAPTURE_ENGINE P1B SHIPPED + LIVE-TESTED 2026-06-26. Scope-interview fires from typed Rooms field → project_type; asks bathroom checklist (9 fields, money-ordered) before pricing; fires modules on triggers; soft-gate force-draft marks scope_origin='incomplete'. Bathroom seed confirmed live-readable. B2.4 (Scope Risk) gate now clear (P1B = SCOPE_CAPTURE_ENGINE Phase 1 complete). Next action: Kalin to confirm bathroom gap-list candidates (see CLAUDE_MEMORY log 2026-06-26) → seed tuning, then P2 or B2.4.**
