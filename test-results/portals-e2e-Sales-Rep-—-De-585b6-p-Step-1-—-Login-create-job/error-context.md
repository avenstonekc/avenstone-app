...
- Name: portals-e2e.spec.js >> Sales Rep — Desktop (1280×800) >> [Sales Rep] Step 1 — Login + create job
...
Call log:
  ...
  - waiting for locator('.tabbar').first()
...
```ts
  137 |     userId = existing.id;
  138 |     await adminSB.auth.admin.updateUserById(userId, { password });
  139 |   } else {
  140 |     const { data, error } = await adminSB.auth.admin.createUser({
  141 |       email, password, email_confirm: true,
  142 |     });
  143 |     if (error) throw new Error(`Could not create ${email}: ${error.message}`);
  144 |     userId = data.user.id;
  145 |   }
