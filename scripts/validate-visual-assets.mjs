...
L.push('');
L.push('─── HOW TO FIX (Blake) — nothing uploads to the bucket until this exits 0 ───');
L.push('  • BAD SPECS: crop each square, 800×800 min. If a photo\'s short side is under 800px it is');
L.push('    too small — re-pull it from the manufacturer product page at FULL size, then crop square.');
L.push('  • MISNAMED: rename to the exact filename shown after the arrow.');
L.push('  • CLAUDE-NAMED FILES PRESENT: delete them — Claude generates those (e.g. bath_niche_recessed.png).');
L.push(`  • Re-run after each batch until READY = ${kalinTotal}/${kalinTotal} and this exits 0.`);
L.push('────────────────────────────────────────────────────────────────────────────');
L.push('');
