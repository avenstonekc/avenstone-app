...
L.push('');
L.push('─── HOW TO FIX — nothing uploads to the bucket until this exits 0 ───');
L.push('  Workflow (Kalin, after each of Blake\'s drops):');
L.push('    1) node scripts/fix-visual-assets.mjs <folder>   — auto-squares every file with short side');
L.push('       >= 800px to 800x800 (originals saved to _originals/); never upscales.');
L.push('    2) node scripts/validate-visual-assets.mjs <folder>   — this report.');
L.push('  • Whatever remains under BAD SPECS after the fix reads "short side < 800px" — that is BLAKE\'S');
L.push('    ONLY JOB: re-download each at FULL size from the manufacturer page (do NOT crop; the fixer squares it).');
L.push('  • MISNAMED: rename to the exact filename shown. CLAUDE-NAMED files: delete (e.g. bath_niche_recessed.png).');
L.push(`  • Repeat fix → validate until READY = ${kalinTotal}/${kalinTotal} and this exits 0.`);
L.push('────────────────────────────────────────────────────────────────────────────');
L.push('');
